unit SummerySalaryList_PersonelTotal;
...
type
  ...
  TSummerySalaryList_PersonelTotalF = class(Ttemplate2MDIF)
    BitBtn1: TBitBtn;
    BitBtn7: TBitBtn;
    BitBtn4: TBitBtn;
    BitBtn5: TBitBtn;
...
var
  SummerySalaryList_PersonelTotalF: TSummerySalaryList_PersonelTotalF;
...
procedure TSummerySalaryList_PersonelTotalF.UpdateFilter;
var
  SQLFld: string;
  i: Integer;
begin
  with qryPersonel do
    try
      ...
      begin
        SQLFld := SQLFld +
          Format(',SUM( CASE PersonelNO WHEN %d THEN Price ELSE 0 END) AS N%d',
          [qryPersonelPersonelNO.AsInteger, RecNo]);
        Next;
      end;
    finally
  ...
  begin
    Active := false;
    SQL.Text := 'SELECT ShowColumn, InfoName_L1, InfoName_L2';
    SQL.Add(SQLFld);
    SQL.Add(', SUM(Price) AS SumPrice ');
    SQL.Add('FROM Pay.RptComprasionSub(');
    SQL.Add(':ArchiveIDFrom , :ArchiveIDTo , 1 , 12 , :EmployTypeIDs  ,:LabelIDFrom  , :LabelIDTo  , :YearIDFrom , :YearIDTo, :YearsFrom , :YearsTo)AS RptComprasionSub_1');
    SQL.Add('WHERE (OfficeCode BETWEEN :FromOfficeCode AND :ToOfficeCode)');
    SQL.Add('AND (PersonelNO BETWEEN :FromPersonelNO AND :ToPersonelNO)');
    SQL.Add('AND (Mounth BETWEEN :MounthFrom AND :MounthTo)');
    SQL.Add('GROUP BY ShowColumn, InfoName_L1, InfoName_L2');
    ...
    Active := true;
    qryComprasion.FieldByName('ShowColumn').DisplayLabel := '—œÌ›';
    qryComprasion.FieldByName('InfoName_L1').DisplayLabel := '⁄‰Ê«‰';
  ...
  begin
    ...
    while not Eof do
    begin
      qryComprasion.FieldByName(Format('N%d', [RecNo])).DisplayLabel :=
        qryPersonelName_L1.AsString;
      TBCDField(qryComprasion.FieldByName(Format('N%d', [RecNo])))
        .currency := true;
      Next;
    end;
  end;
...
//  DBGrid1.Columns[1].Width := 80;
  ...
  begin
    DBGrid1.Columns[i].Width := Application.MainForm.Width div DBGrid1.Columns.Count;
  end;
...
procedure TSummerySalaryList_PersonelTotalF.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
...
procedure TSummerySalaryList_PersonelTotalF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TSummerySalaryList_PersonelTotalF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryComprasion);
end;

procedure TSummerySalaryList_PersonelTotalF.actSendToExcelExecute
  (Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
...
procedure TSummerySalaryList_PersonelTotalF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryComprasion);
end;

procedure TSummerySalaryList_PersonelTotalF.ppLabel5GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSummerySalaryList_PersonelTotalF.ppLabel10GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := Text + ' ' + IntToStr(qryComprasion.Parameters.ParamByName
...
procedure TSummerySalaryList_PersonelTotalF.ppLabel7GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TSummerySalaryList_PersonelTotalF.ppSystemVariable2GetText
...
procedure TSummerySalaryList_PersonelTotalF.actPrintExecute(Sender: TObject);
begin
  inherited;
  try
...
procedure TSummerySalaryList_PersonelTotalF.ppLabel2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSummerySalaryList_PersonelTotalF.ppLabel3GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSummerySalaryList_PersonelTotalF.actTreeChartExecute
  (Sender: TObject);
var
  Limit: String;
begin
...
procedure TSummerySalaryList_PersonelTotalF.AllW_GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
