...
type
  TSalaryList_ProjectF = class(Ttemplate2MDIF)
    Label1: TLabel;
    DBEdit1: TDBEdit;
    ...
    qryProjectCode: TADOQuery;
    srcProjectCode: TDataSource;
    ...
    qryProjectCodeProjectCode: TIntegerField;
    qryProjectCodeInfoName_L1: TStringField;
    ppReport1: TppReport;
    ...
    actShow: TAction;
    BitBtn3: TBitBtn;
    qryProjectCodeInfoName_L2: TStringField;
    qryProjectCodeProjectID: TIntegerField;
    DBGrid1: TCedarDbgrid;
    procedure FormShow(Sender: TObject);
    procedure qryProjectCodeAfterScroll(DataSet: TDataSet);
...
var
  SalaryList_ProjectF: TSalaryList_ProjectF;

implementation

uses DM, GlobalPro, filter_ADO, FilterClass_ADO, sort2, search2,
  searchCode_ADO, TreeChart,  Filter_ADO_Const;

{$R *.dfm}

procedure TSalaryList_ProjectF.UpdateFilter;
begin
...
procedure TSalaryList_ProjectF.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
  if actFilter.Execute then
    if myParams.FindParam('ProjectID') = nil then
      Close;
end;

procedure TSalaryList_ProjectF.qryProjectCodeAfterScroll(DataSet: TDataSet);
begin
  inherited;
  with qryComprasion do
  begin
    ...
    Parameters.ParamByName('ProjectID').Value :=
      qryProjectCode.Fieldbyname('ProjectID').AsInteger;
    Active := true;
  end;
end;

procedure TSalaryList_ProjectF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryComprasion);
end;

procedure TSalaryList_ProjectF.ppLabel7GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TSalaryList_ProjectF.ppSystemVariable2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList_ProjectF.ppLabel3GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList_ProjectF.ppLabel2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList_ProjectF.SpeedButton1Click(Sender: TObject);
var
  s: Boolean;
...
begin
  ...
  if s then
    qryProjectCode.Locate('ProjectID', Results[0], []);
end;

procedure TSalaryList_ProjectF.actChartExecute(Sender: TObject);
begin
  inherited;
  TreeChartF.showChart(qryComprasion, Self);
end;

procedure TSalaryList_ProjectF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList_ProjectF.actPrintExecute(Sender: TObject);
begin
  inherited;
  try
    qryComprasion.DisableControls;
    InitReportFile(ppReport1, 'SalaryList_ProjectF');
  finally
    qryComprasion.EnableControls;
  end;
end;

procedure TSalaryList_ProjectF.actSendToExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TSalaryList_ProjectF.Action1Execute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryComprasion);
end;

procedure TSalaryList_ProjectF.FormCreate(Sender: TObject);
begin
  inherited;
