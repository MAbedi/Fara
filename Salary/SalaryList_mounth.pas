...
type
  TSalaryList_mounthF = class(Ttemplate2MDIF)
    qrysalary: TADOQuery;
...
var
  SalaryList_mounthF: TSalaryList_mounthF;
...
{$R *.dfm}
procedure TSalaryList_mounthF.MakeSql;
var
  W:integer;
 sqltxt:string;
// SelectList : String ;
...
//----make qrysalary sql-----------------------------------
  ...
  with qrysalary do begin
    ...
    SQL.Add(' WHERE  (Pay.FixedCalculated.Mounth = :mounth) and  (Pay.FormsInfo.InfoID  BETWEEN :FromInfoID AND :ToInfoID) AND (Pay.FixedCalculated.PersonelNO BETWEEN :FromPersonelNo AND :ToPersonelNo)');
...
//----make qrysum sql-----------------------------------
  ...
  with qryDec do begin
    ...
    while W <10  do begin
      if not Eof  then begin
        sqltxt:=sqltxt+', sum( when salaryid='+Fieldbyname('FormInfoID').AsString +'  and  ShowlistKind =2  then price else 0 end ) as sumdec'+IntToStr( w) ;
        Next ;
        end
      else
...
procedure TSalaryList_mounthF.FormCreate(Sender: TObject);
begin
...
procedure TSalaryList_mounthF.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList_mounthF.FormDestroy(Sender: TObject);
begin
  inherited;
  SaveColWidth(DBGrid1);
end;

procedure TSalaryList_mounthF.FormResize(Sender: TObject);
begin
  inherited;
  SetColSize(DBGrid1,3,True);
end;

procedure TSalaryList_mounthF.ppLabel45GetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text:=APPBank.CompanyName
end;

procedure TSalaryList_mounthF.actSortExecute(Sender: TObject);
begin
  inherited;
    sort2F.ShowSort(qrysalary);
end;

procedure TSalaryList_mounthF.actSendtoExcelExecute(Sender: TObject);
begin
  inherited;
  SendDBGridToExcel(DBGrid1);
end;

procedure TSalaryList_mounthF.actPrintExecute(Sender: TObject);
begin
  inherited;
  try
...
procedure TSalaryList_mounthF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qrysalary);
end;

procedure TSalaryList_mounthF.ppLabel46GetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text:=GetPrintDate;
end;

procedure TSalaryList_mounthF.ppSystemVariable1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList_mounthF.actFilterExecute(Sender: TObject);
begin
  inherited;
  with TfilterF.Create2(Self,myParams) do begin
    try
      AddItem(DMf.adcSalary,'InfoID',' ÚäæÇä ÓÇÒãÇä','ßÏ',ftInteger,dvMinMax,'','',ciLookup,'SELECT    Pay.FormsInfo.InfoID, Pay.FormsInfo.InfoName_L1 FROM  Pay.FixedCalculated INNER JOIN  '+
      ...
      AddItem(DMf.adcSalary,'PersonelNO','ãÔÎÕÇÊ ÑÓäáí','ßÏ',ftInteger,dvMinMax,'','',ciLookup, 'SELECT     Pay.FixedCalculated.PersonelNO, Pay.PersonelInfo.name_L1 + '' '' + Pay.PersonelInfo.lastName_L1 AS name_l1 '+
...
procedure TSalaryList_mounthF.Updatefilter;
var
 i,j,w:integer;
begin
 with qrysalary do begin
...
procedure TSalaryList_mounthF.ToolButton1Click(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList_mounthF.N11Click(Sender: TObject);
begin
  inherited;
  InitReportFile('SalaryList_office.rtm');
end;

procedure TSalaryList_mounthF.InitReportFile(GeneralFileName: string);
var
  s:String;
begin
  s:=IncludeTrailingBackslash(ExtractFilePath(ParamStr(0))+'Report\');
    with ppReport1 do begin
      Template.FileName:=s+GeneralFileName;
       try
         if FileExists(Template.FileName) then  Template.LoadFromFile
                       else Warn('ÝÇíá Çí '+GeneralFileName+' íÇÝÊ äÔÏ.!þ');
            MakeReport;
            Print ;
       finally
       end;//try
...
procedure TSalaryList_mounthF.MakeReport;
begin
  try
    qrysalary.DisableControls;
...
procedure TSalaryList_mounthF.N21Click(Sender: TObject);
begin
  inherited;
  InitReportFile('SalaryList.rtm');
end;

//function TSalaryList_mounthF.CalcSumFileds(FiledName: String): Currency;
//begin
//  Result:=0;
//  try
...
procedure TSalaryList_mounthF.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TSalaryList_mounthF.DBGrid1ColEnter(Sender: TObject);
begin
  inherited;
 DBGrid2.SelectedIndex:=DBGrid1.SelectedIndex-3
end;

procedure TSalaryList_mounthF.ppLabel34GetText(Sender: TObject; var Text: String);
begin
  inherited;
