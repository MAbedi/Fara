{ -----------------------------------------------------------------------------
  Unit Name: RecallPersonelInfos
  Author:    M_A_H_M_O_O_D
  Purpose:
  History:
  ----------------------------------------------------------------------------- }
unit RecallPersonelInfos;
...
type
  ...
  TRecallPersonelInfosF = class(Ttemplate2MDIF)
...
var
  RecallPersonelInfosF: TRecallPersonelInfosF;

implementation

uses filter_ADO, FilterClass_ADO, GlobalPro, DM,

   mmessage, SalaryFunctions, dmgConst, FaraConsts,
  Filter_ADO_Const, search2, sort2;

{$R *.dfm}
// procedure TRecallPersonelInfosF.assignCombo;
// var
// qry: TADOQuery;
// begin
...
procedure TRecallPersonelInfosF.UpdateFilter;
begin
...
procedure TRecallPersonelInfosF.actFilterExecute(Sender: TObject);
begin
  inherited;
  ...
  begin
    try
      AddItem(DMF.adcSalary, 'FormType', '‰Ê⁄ «ÿ·«⁄« ', 'ﬂœ', ftInteger,
        ...
        'FROM Forms INNER JOIN Pay.FormTypes ON Forms.FormType = FormTypes.FormType',
        'SELECT Min(FormType),max(FormType)FROM Forms ');

      AddItem(DMF.adcSalary, 'number', '‘„«—Â', '‘„«—Â', ftInteger, dvMinMax, '',
        '', ciSimple, '', 'Select Min(FormNumber),max(FormNumber) from Forms ');
...
procedure TRecallPersonelInfosF.frServerName1cmbBankNameChange(Sender: TObject);
begin
  inherited;
...
procedure TRecallPersonelInfosF.FormDestroy(Sender: TObject);
begin
  inherited;
  dmgConstF.Free;
end;

procedure TRecallPersonelInfosF.actSendExelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid2);
end;

procedure TRecallPersonelInfosF.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TRecallPersonelInfosF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryTypesItems);
end;

procedure TRecallPersonelInfosF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryTypesItems);
end;

procedure TRecallPersonelInfosF.DBGrid2DblClick(Sender: TObject);
begin
  inherited;
  // ToggleCheck;
end;

procedure TRecallPersonelInfosF.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TRecallPersonelInfosF.DBGrid2MouseDown(Sender: TObject;
...
//
// procedure TRecallPersonelInfosF.ToggleCheck;
// begin
// with qryTypesItems do
// begin
// Edit;
...
procedure TRecallPersonelInfosF.actSelectAllExecute(Sender: TObject);
begin
  inherited;
  with qryTypesItems do
  begin
...
procedure TRecallPersonelInfosF.actSlelectInverseExecute(Sender: TObject);
begin
  inherited;
  with qryTypesItems do
  begin
...
procedure TRecallPersonelInfosF.actMoveExecute(Sender: TObject);
begin
  inherited;
  DMF.adcSalary.BeginTrans;
//  try
//    BigMessageProgBar('œ— Õ«· «‰ ﬁ«· ' + qryTypesItems.AsString + ' Â« ',
...
procedure TRecallPersonelInfosF.InsertRecipts;
var
  i: Integer; // ,FormIDi
...
//    sql.Add('WHERE FormID = ' + qryTypesItemsFormID.AsString);
//    sql.Add('AND ServerID = ' + qryTypesItems.FieldByName('ServerID').AsString);
 //   sql.Add('AND YearID = ' + qryTypesItems.FieldByName('YearID').AsString);
    Active := True;
    while not Eof do
    begin
      qryForms.Insert;
    ...
    GoProgressBar(qryTypesItems.Fields[0].AsString + '<>' +
      qryTypesItems.Fields[0].AsString);
    Active := False;
    sql.Text := 'SELECT * FROM FormItems';
    sql.Add('WHERE FormID = ' + qryTypesItems.Fields[0].AsString);
    sql.Add('AND ServerID = ' + qryTypesItems.FieldByName('ServerID').AsString);
    sql.Add('AND YearID = ' + qryTypesItems.FieldByName('YearID').AsString);
    Active := True;
    while not Eof do
...
procedure TRecallPersonelInfosF.actMoveBaseInfoExecute(Sender: TObject);
var
  i: Integer;
...
procedure TRecallPersonelInfosF.lslTableNameClick(Sender: TObject);
var
  i: Integer;
...
procedure TRecallPersonelInfosF.actShiftExecute(Sender: TObject);
begin
  inherited;
  if get_response
...
procedure TRecallPersonelInfosF.adcSrcAfterConnect(Sender: TObject);
begin
  inherited;
//  ChkServerID(adcSrc, actMove, actMoveBaseInfo)
end;

procedure TRecallPersonelInfosF.DBGrid1KeyPress(Sender: TObject; var Key: Char);
var
  tc: Cardinal;
const
  t0: Cardinal = 0;
