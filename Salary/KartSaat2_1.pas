...
uses
  ...
  ComCtrls, ToolWin, MyComboBoxUnit, System.ImageList, System.Actions;

type
  TKartSaat2_1F = class(Ttemplate2MDIF)
    memWarning: TMemo;
    BitBtn1: TBitBtn;
    actTransfer: TAction;
...
procedure TKartSaat2_1F.ReadPathLists;
var
  ListsPath:  String;
  srch:   TSearchRec;
begin
...
procedure TKartSaat2_1F.SpeedButton1Click(Sender: TObject);
var
  s:  String;
begin
  inherited;
...
procedure TKartSaat2_1F.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TKartSaat2_1F.FormDestroy(Sender: TObject);
begin
  inherited;
  ListFiles.Free;
...
function TKartSaat2_1F.extractListName(fileName: TFileName): String;
var
  m:  Byte;
  fname:  String;
begin
...
procedure TKartSaat2_1F.edtPathKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TKartSaat2_1F.actTransferExecute(Sender: TObject);
var
  F:  TextFile;
  k,pid,pName:  String;
...
begin
  ...
  try
    ...
    while not eof(f) do begin
      Readln(f,k);
      pid:=IntToStr(StrToIntDef(Trim(MidStr(k,1,10)),0));// ReadPersonelItemNo(k,2);
      if not qryPersonels.Locate('PersonelNo',pid,[]) then
        memWarning.Lines.Add('ﬂœ Å—”‰·Ì ' + pid + ' „⁄ »— ‰Ì” .˛')
      else begin
        if not Add2Function(k,pid) then
...
procedure TKartSaat2_1F.lstListFilesClick(Sender: TObject);
var
  i:  Integer;
begin
  inherited;
...
function TKartSaat2_1F.Add2Function(FunctionString, PersonelID: string): Boolean;
begin
  ...
  try
    with qryFunctions do begin
      ...
      FieldByName('JobCode').AsInteger:=StrToInt(MidStr(FunctionString,23,10));
      FieldByName('FunctionDay').AsInteger:= StrToInt(MidStr(FunctionString,40,5));
      FieldByName('FunctionTime').AsFloat:=StrToInt(MidStr(FunctionString,34,3))+
                       RoundTo( (StrToInt(MidStr(FunctionString,37,2))*1/60),-2);
...
procedure TKartSaat2_1F.Button1Click(Sender: TObject);
var
  i:  byte;
begin
  inherited;
...
procedure TKartSaat2_1F.ToolBar1Click(Sender: TObject);
begin
  inherited;
  MonthNo:=(Sender as TToolButton).tag;
end;

procedure TKartSaat2_1F.UpDateLists;
begin
  with qryFunctions do begin
    Active:=False;
...
procedure TKartSaat2_1F.ToolButton1Click(Sender: TObject);
begin
  inherited;
  MonthNo:=(Sender as TToolButton).tag;
  UpDateLists;
...
function TKartSaat2_1F.Add2FunctionItems(FunctionString,
...
procedure TKartSaat2_1F.qryFunctionsAfterScroll(DataSet: TDataSet);
begin
  inherited;
