...
type
  TrptInsouranceOtherF = class(Ttemplate2MDIF)
    ...
    qryInsouranceOtherPrice12: TBCDField;
    ...
    qryInsouranceOtherAmount12: TFloatField;
...
var
  rptInsouranceOtherF: TrptInsouranceOtherF;

implementation

uses DM, GlobalPro, sort2, search2, filter_ADO, FilterClass_ADO,
  SalaryFunctions, Mymostafa,
  mmessage;

{$R *.dfm}

procedure TrptInsouranceOtherF.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.FormDestroy(Sender: TObject);
begin
  inherited;
  SaveColWidth(DBGrid1);
end;

procedure TrptInsouranceOtherF.FormResize(Sender: TObject);
begin
  inherited;
  SetColSize(DBGrid1, 1, True);
end;

procedure TrptInsouranceOtherF.ppLblCompanyNameGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TrptInsouranceOtherF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryInsouranceOther);
end;

procedure TrptInsouranceOtherF.actSendtoExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TrptInsouranceOtherF.actPrintExecute(Sender: TObject);
begin
  inherited;
  PopList4Print.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TrptInsouranceOtherF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryInsouranceOther);
end;

procedure TrptInsouranceOtherF.ppLblPrintDateGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TrptInsouranceOtherF.ppSystemVariable1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TrptInsouranceOtherF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.Updatefilter;
begin
  inherited;
...
procedure TrptInsouranceOtherF.ToolButton1Click(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TrptInsouranceOtherF.AllClickClick(Sender: TObject);
begin
  inherited;
  with qryInsouranceOther do
    try
...
procedure TrptInsouranceOtherF.CmbArchiveIDChange(Sender: TObject);
begin
  inherited;
  Updatefilter
end;

function TrptInsouranceOtherF.MakeDisk(Tbl_Name, BimehFile: String): boolean;
var
  path: String;
  s: String;
  b1: boolean;
    ...
    begin
      ...
      begin
        ...
        if not CopyFile(pchar(path + BimehFile + '00.fra'),
          pchar(IncludeTrailingPathDelimiter(s) + BimehFile + '.mdb'), false) then
        begin
          WarnMsg := '«‘ﬂ«· œ— ﬂÅÌ ›«Ì·.';
          b1 := get_response(WarnMsg + ' œÊ»«—Â ”⁄Ì „Ìùﬂ‰Ìœø') = mrYes;
        end
...
procedure TrptInsouranceOtherF.Insertdisket(Tbl_Name: String);
begin
  if Tbl_Name = 'disket' then
    ...
    begin
      ...
      fieldbyname('td01').AsInteger := qryInsouranceOther.fieldbyname
        ('Amount12').AsInteger;
      fieldbyname('mb01').AsCurrency := qryInsouranceOther.fieldbyname
        ('Price12').AsCurrency;
...
procedure TrptInsouranceOtherF.actMakeDiskExecute(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.actMakeDisk_Tbl_ListExecute(Sender: TObject);
begin
  inherited;
...
procedure TrptInsouranceOtherF.ppDBText5GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TrptInsouranceOtherF.ppDBText1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := optP.Year;
...
procedure TrptInsouranceOtherF.Button2Click(Sender: TObject);
begin
  inherited;
