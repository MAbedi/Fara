...
type
  TSalaryList4Office2F = class(Ttemplate2MDIF)
    qrysalary: TADOQuery;
    ...
    qryOffice: TADOQuery;
    qryOfficeOfficeCode: TIntegerField;
    qryOfficeOfficeName_L1: TStringField;
  ...
  private
    ...
    function Getc_From(ParamName: string): Variant;
    function Getc_To(ParamName: string): Variant;
...
var
  SalaryList4Office2F: TSalaryList4Office2F;

implementation

uses DM, GlobalPro, sort2, search2, filter_ADO, FilterClass_ADO, StrUtils,
  main, FunctionItems, ListSalary, searchCode_ADO,
  SalaryFunctions, infoSMS, mdiMain;

{$R *.dfm}

function TSalaryList4Office2F.Getc_From(ParamName: string): Variant;
begin
  if myParams.FindParam(ParamName) <> nil then
    Result := GetcFrom(myParams.ParamValues[ParamName], ftInteger)
  else
    Result := -999999999;
end;

function TSalaryList4Office2F.Getc_To(ParamName: string): Variant;
begin
  if myParams.FindParam(ParamName) <> nil then
    Result := GetcTo(myParams.ParamValues[ParamName], ftInteger)
  else
    Result := 999999999;
end;

procedure TSalaryList4Office2F.MakeSql;
var
  W: Integer;
...
begin
  ...
  begin
    Active := False;
    Parameters.ParamByName('OfficeCodeFrom').Value := Getc_From('InfoID');
    // GetcFrom(myParams.ParamValues['InfoID'], ftInteger);
    Parameters.ParamByName('OfficeCodeTo').Value := Getc_To('InfoID');
    ...
    Parameters.ParamByName('LabelIDFrom').Value := Getc_From('LabelID');
    // GetcFrom(myParams.ParamValues['LabelID'], ftInteger);
    Parameters.ParamByName('LabelIDTo').Value := Getc_To('LabelID');
    ...
    Parameters.ParamByName('EmployTypeIDFrom').Value :=
      Getc_From('EmployTypeID');
    // GetcFrom(myParams.ParamValues['EmployTypeID'], ftInteger);
    Parameters.ParamByName('EmployTypeIDTo').Value := Getc_To('EmployTypeID');
    ...
    sqltxt := 'SELECT  FormsInfoOffice.InfoID , FormsInfoOffice.InfoName_L1 AS OfficeName, '
      ...
      ', Functions_Sum.FunctionDay, Functions_Sum.FunctionTime ';
  ...
  begin
    Active := False;
    SQL.Text := sqltxt;
    // SQL.Add('FROM Pay.FormsInfo RIGHT OUTER JOIN Pay.Interdicts INNER JOIN');
    // SQL.Add('Pay.PersonelInfo ON Pay.Interdicts.PersonelNo = PersonelInfo.PersonelNo RIGHT OUTER JOIN');
    // SQL.Add('Pay.FixedCalculated ON PersonelInfo.PersonelNo = FixedCalculated.PersonelNO ON FormsInfo.FormInfoID = Interdicts.OfficeCode');
    /// /////////////////////////////////////////////////
    ...
    SQL.Add('FixedCalculated.YearID = FunctionItemsList_1.YearID AND FixedCalculated.Mounth = FunctionItemsList_1.Mounth ');
    ...
    SQL.Add('ON FixedCalculated.Mounth = Functions_Sum.Mounth AND FixedCalculated.PersonelNO = PersonelInfo.PersonelNo ');
...
procedure TSalaryList4Office2F.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList4Office2F.addFunctionsItems;
begin
  ...
  if FunItems4Sum <> EmptyStr then
    FunItems4Sum := 'FunctionDay;FunctionTime;' + FunItems4Sum + ';'
  else
    FunItems4Sum := 'FunctionDay;FunctionTime;';
...
procedure TSalaryList4Office2F.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList4Office2F.FormDestroy(Sender: TObject);
begin
  inherited;
  Ts_Amount.Free;
end;

procedure TSalaryList4Office2F.ppLabel45GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList4Office2F.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qrysalary);
end;

procedure TSalaryList4Office2F.actSendtoExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TSalaryList4Office2F.actSMSExecute(Sender: TObject);
begin
  inherited;
  InfoSMSF.ShowSMS(qrysalary, 'Mobile')
end;

procedure TSalaryList4Office2F.actPrintExecute(Sender: TObject);
begin
  inherited;
  try
...
procedure TSalaryList4Office2F.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qrysalary);
end;

procedure TSalaryList4Office2F.ppLabel46GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TSalaryList4Office2F.ppSystemVariable1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList4Office2F.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList4Office2F.Updatefilter;
begin
  MakeSql;
  with qrysalary do
  begin
    ...
    Parameters.ParamByName('EmployTypeIDTo').Value := Getc_To('EmployTypeID');
    // GetcTo(myParams.ParamValues['EmployTypeID'], ftInteger);
    Parameters.ParamByName('jobCodeFrom').Value := Getc_From('jobCode');
    // GetcFrom(myParams.ParamValues['jobCode'], ftInteger);
    Parameters.ParamByName('jobCodeTo').Value := Getc_To('jobCode');
  ...
  begin
    ...
    begin
      Parameters.ParamByName('OfficeCodeFrom').Value := 0;
      Parameters.ParamByName('OfficeCodeTo').Value := 999999999;
    end;
    Active := True;
  end; // qryOffice
...
procedure TSalaryList4Office2F.ToolButton1Click(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList4Office2F.InitReportFile(GeneralFileName: string);
var
  ReportFooter: String;
begin
...
procedure TSalaryList4Office2F.MakeReport;
begin
  try
...
procedure TSalaryList4Office2F.ppLabel34GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TSalaryList4Office2F.actFunctionShowExecute(Sender: TObject);
begin
  inherited;
...
procedure TSalaryList4Office2F.actFishExecute(Sender: TObject);
var
  Month_No: Byte;
begin
  inherited;
...
procedure TSalaryList4Office2F.SpeedButton1Click(Sender: TObject);
var
  s: Boolean;
  Results: array [0 .. 1] of String;
begin
  inherited;
  s := searchCode_ADOF.SearchCode2(DMF.adcSalary, ' Ê«ÕœÂ«Ì ”«“„«‰Ì',
    'SELECT    FormsInfo.InfoID, FormsInfo.InfoName_L1 FROM Pay.FixedCalculated INNER JOIN  '
    + ' Pay.FormsInfo ON FixedCalculated.OfficeCode = FormsInfo.FormInfoID WHERE  formtype=12   '
    + ' GROUP BY  FormsInfo.InfoID,FormsInfo.InfoName_L1 ', ['ﬂœ', '‰«„ Ê«Õœ '],
    Results, [100, 100], alLeft);
  if s then
    qryOffice.Locate('OfficeCode', Results[0], []);
end;

procedure TSalaryList4Office2F.qryOfficeAfterScroll(DataSet: TDataSet);
begin
  inherited;
  with qrysalary do
  begin
...
procedure TSalaryList4Office2F.CmbArchiveIDChange(Sender: TObject);
begin
  inherited;
  Updatefilter
end;

procedure TSalaryList4Office2F.qrysalaryAfterOpen(DataSet: TDataSet);
var
  W: Integer;
  i: SmallInt;
begin
