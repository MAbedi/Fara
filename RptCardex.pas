...
type
  ...
  TRptCardexF = class(Ttemplate2MDIF)
...
procedure TRptCardexF.Enter(StoreID: Integer; StuffCode: Largeint;
  Ftype: Integer);
begin

  if ( { (mainF.Menu = mainF.Mnu2Store) and mainF.mnuActRptCardexF1.Visible) or
      ((mainF.Menu = mainF.Mnu13Store) and } mainF.mnuActRptCardexF1.Visible)
  then
    try
      Var_glb_NoFilter := True;
      CreateMDIForm2(TRptCardexF, RptCardexF, mainF, Ftype);
      RptCardexF.qryStuffs.AfterScroll := nil;
      if RptCardexF.frPersonID3F1.qryPersonID1D <> nil then
        RptCardexF.frPersonID3F1.qryPersonID1D.AfterScroll := nil;
      RptCardexF.cmbStore.OnClick := nil;
      RptCardexF.cmbStore.AddItem(StoreID.ToString, TObject(StoreID));
      RptCardexF.cmbStore.ItemIndex := RptCardexF.cmbStore.Items.IndexOfObject
        (TObject(StoreID));
      RptCardexF.cmbStoreClick(RptCardexF.cmbStore);
      // RptCardexF.qryStuffs.AfterScroll := RptCardexF.qryStuffsAfterScroll;
      // RptCardexF.frPersonID3F1.qryPersonID1D.AfterScroll :=
      // frPersonID3F1qryPersonID3AfterScroll;
      // RptCardexF.cmbStore.OnClick := RptCardexF.cmbStoreClick;
      if (StuffCode <> 0) and (RptCardexF.qryStuffs.Active) then
...
function TRptCardexF.LocateNewconst;
begin
  Result := False;
  With RptCardexF.qryStuffs do
    try
      DisableControls;
      AfterScroll := nil;
      First;
      ...
      qryStuffsAfterScroll(RptCardexF.qryStuffs);
      EnableControls;
    end;
end;

procedure TRptCardexF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TRptCardexF.UpdateList;
var
...
procedure TRptCardexF.FormShow(Sender: TObject);
begin
  inherited;
  actFilter.Execute;
...
procedure TRptCardexF.frPersonID3F1qryPersonID3AfterScroll(DataSet: TDataSet);
begin
  inherited;
  with RptCardexF do
    with qryRptTransactions do
...
procedure TRptCardexF.frPersonID3F1sbtnMasterClick(Sender: TObject);
begin
  inherited;
...
procedure TRptCardexF.cmbStoreClick(Sender: TObject);
begin
  inherited;
  RptCardexF.frPersonID3F1.qryPersonID1D.AfterScroll :=
    frPersonID3F1qryPersonID3AfterScroll;
  qryStuffs.AfterScroll := RptCardexF.qryStuffsAfterScroll;
...
procedure TRptCardexF.initCombo;
var
  StoreIdFrom: Integer;
begin
  with qryStore do
...
procedure TRptCardexF.qryStuffsAfterScroll(DataSet: TDataSet);
begin
  inherited;
  with RptCardexF do
...
procedure TRptCardexF.BitBtn2Click(Sender: TObject);
var
  Txt: String;
  b: Boolean;
...
procedure TRptCardexF.chkAccordingToClick(Sender: TObject);
begin
  inherited;
  UpdateList
end;

procedure TRptCardexF.chkSumClick(Sender: TObject);
begin
  inherited;
  DBGrid1.SetFooter4Sum([]);
end;

procedure TRptCardexF.actShowFormExecute(Sender: TObject);
begin
  inherited;
...
procedure TRptCardexF.actSendToExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TRptCardexF.updateRemain;
var
  c_BackLogEntity: Real48;
  c_BackLogWeight: Real48;
  c_BacklogPrice: Real48;
  c_BacklogSellPrice: Real48;
  b: Boolean;
begin
  c_BackLogEntity := 0;
  c_BackLogWeight := 0;
  ...
  begin
    try
      ...
      begin
        ...
        c_BacklogPrice := c_BacklogPrice + FieldByName('BacklogPrice').AsFloat;
        ...
        FieldByName('CalcuBackLogEntity').AsFloat :=
          RoundTo(c_BackLogEntity, opt.RoundEntity); // -5
        FieldByName('CalcuBackLogWeight').AsFloat :=
         RoundTo(c_BackLogWeight, opt.RoundWeight);

        // if (c_BackLogEntity <> 0) or (c_BackLogWeight <> 0) then
        // // 981102  disabel   and    1400/11/09 zarfBast Enabel
        // begin
        ...
        // c_BacklogSellPrice := 0;
        // end;
        // if qryRptTransactionsReciptNumber.AsInteger = 41 then
        // Warn('ffffff');
        // add2log(qryRptTransactionsReciptNumber.AsString);

        case MyEntityDisplayType of
          0, 2:
            if RoundTo(c_BackLogEntity, 0) <> 0 then
            begin
              // if c_BacklogSellPrice <> 0 then
              // FieldByName('UnitSellPriceBackLog').AsCurrency :=
              // c_BacklogSellPrice / c_BackLogEntity
              // else
          ...
          1, 3:
            ...
            begin
              // if c_BacklogSellPrice <> 0 then
              // FieldByName('UnitSellPriceBackLog').AsCurrency :=
              // c_BacklogSellPrice / c_BackLogWeight
              // else
              FieldByName('UnitSellPriceBackLog').AsFloat := c_BacklogPrice /
                c_BackLogWeight
            end
      ...
      StatusBar1.Panels[3].Text := FieldByName('CalcuBackLogEntity')
        .DisplayLabel + ' = ' + CurrToStrF(c_BackLogEntity, ffNumber, 4);
      StatusBar1.Panels[2].Text := FieldByName('CalcuBackLogWeight')
        .DisplayLabel + ' = ' + CurrToStrF(c_BackLogWeight, ffNumber, 4);
      if b then
      begin
        StatusBar1.Panels[1].Text := FieldByName('CalcuBackLogPrice')
          .DisplayLabel

          + ' = ' + CurrToStrF(c_BacklogPrice, ffCurrency, 0);

          if  opt.ActiveSellPrice2 then
        StatusBar1.Panels[0].Text := FieldByName('CalcuBackLogSellPrice')
          .DisplayLabel + ' = ' + CurrToStrF(c_BacklogSellPrice, ffCurrency, 0);
      end;
    end; // try
  end; // with
end;

procedure TRptCardexF.qryRptTransactionsAfterOpen(DataSet: TDataSet);
begin
  inherited;
  // with qrySum do
  // begin
...
procedure TRptCardexF.qryRptTransactionsBeforeInsert(DataSet: TDataSet);
begin
  inherited;
  Abort
end;

procedure TRptCardexF.setFieldColumns;
var
  b: Boolean;
begin
...
procedure TRptCardexF.actPrintExecute(Sender: TObject);
begin
  inherited;
  PopMenu.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TRptCardexF.ppLabel29GetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := GetPrintDate;
end;

procedure TRptCardexF.ppSystemVariable1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRptCardexF.ppLabel26GetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := APPBank.CompanyName;
end;

procedure TRptCardexF.ppLabel37GetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := cmbStore.Text;
end;

procedure TRptCardexF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryRptTransactions);
end;

procedure TRptCardexF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  try
...
procedure TRptCardexF.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  QuickSearch(Key, qryRptTransactions.FieldByName('ReciptNumber'));
end;

procedure TRptCardexF.DBNavigator1BeforeAction(Sender: TObject;
  Button: TNavigateBtn);
begin
  inherited;
  RptCardexF.frPersonID3F1.qryPersonID1D.AfterScroll :=
...
procedure TRptCardexF.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
  actShowForm.Execute
end;

procedure TRptCardexF.FormDestroy(Sender: TObject);
begin
  inherited;
  // SaveColWidth(DBGrid1);
end;

procedure TRptCardexF.AllC_l_i_c_k_Click(Sender: TObject);
begin
  inherited;
  ...
  try
    ...
    if (Sender as TMenuItem).Hint = EmptyStr then
      if FormType = 2 then
        InitReportFile(ppReport1, 'RptCardex1') // ppReport1.Print
      else
        InitReportFile(ppReport1, 'RptCardex2') // ppReport2.Print
    else
...
procedure TRptCardexF.actPrintVijehExecute(Sender: TObject);
begin
  inherited;
  DBGrid2PrintF.showGrid2Print(DBGrid1, FormType);
end;

procedure TRptCardexF.btnSdClick(Sender: TObject);
var
  i: Byte;
...
procedure TRptCardexF.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TRptCardexF.ppLblLimitDateGetText(Sender: TObject; var Text: String);
begin
  inherited;
