inherited RptEntiyStuff_UnitCodeF: TRptEntiyStuff_UnitCodeF
  Left = 193
  Top = 139
  Width = 803
  Height = 564
  Caption = #1711#1586#1575#1585#1588' '#1605#1608#1580#1608#1583#1610' '#1576#1585#1581#1587#1576' '#1603#1583' '#1605#1581#1604' '#1605#1589#1585#1601
  OldCreateOrder = True
  OnCreate = FormCreate
  ...
  object qryEntity: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT    dbo.Recipts.StoreID, dbo.ReciptItems.StuffCode, dbo.Un' +
        'its.UnitName,'
      
        '          Round(SUM(dbo.ReciptItems.InputEntity - dbo.ReciptItem' +
        's.OutputEntity),3) AS Entity,'
      
        '          Round(SUM(dbo.ReciptItems.InputWeight - dbo.ReciptItem' +
        's.OutputWeight),3) AS Weight,'
      
        '          SUM(dbo.ReciptItems.TotalInputPrice - dbo.ReciptItems.' +
        'TotalOutputPrice) AS Price,'
      
        '          dbo.ReciptItems.ProductModel, isnull(dbo.ReciptItems.U' +
        'seUnitID,0) as UseUnitID,'
      
        '          dbo.UseUnits.UseUnitName, dbo.LookUps.Name AS ModelNam' +
        'e, dbo.StuffCoding.c_StuffName'
      'FROM      dbo.ReciptItems INNER JOIN'
      
        '          dbo.Recipts ON dbo.ReciptItems.ReciptID = dbo.Recipts.' +
        'ReciptID INNER JOIN'
      
        '          dbo.ReciptTypes ON dbo.Recipts.ReciptType = dbo.Recipt' +
        'Types.ReciptType LEFT OUTER JOIN'
      
        '          dbo.UseUnits ON dbo.ReciptItems.UseUnitID = dbo.UseUni' +
        'ts.UseUnitID LEFT OUTER JOIN'
      '          dbo.Units INNER JOIN'
      
        '          dbo.StuffCoding ON dbo.Units.UnitCode = dbo.StuffCodin' +
        'g.n_UnitCode ON dbo.ReciptItems.StuffCode = dbo.StuffCoding.c_St' +
        'uffCode LEFT OUTER JOIN'
      
        '          dbo.LookUps ON dbo.ReciptItems.ProductModel = dbo.Look' +
        'Ups.LookUpID'
      'WHERE        (Recipts.ReciptDate BETWEEN :DateFrom  AND :DateTo)'
      
        '         AND (Recipts.StoreID BETWEEN :StoreIDFrom AND :StoreIDT' +
        'o )'
      
        '         AND (ReciptItems.StuffCode BETWEEN :StuffCodeFrom AND :' +
        'StuffCodeTo)'
      
        '         AND (Isnull(dbo.ReciptItems.UseUnitID,0) between :UseUn' +
        'itIDfrom and :UseUnitIDto)'
      
        '         AND (dbo.ReciptItems.ProductModel BETWEEN :ModelFrom an' +
        'd :ModelTo)'
      ...
      ''
        ...
        'ts.UnitName, dbo.ReciptItems.ProductModel, dbo.ReciptItems.UseUn' +
        'itID,'
      
        '          dbo.UseUnits.UseUnitName, dbo.LookUps.Name, dbo.StuffC' +
        'oding.c_StuffName'
      
        'HAVING   (SUM(dbo.ReciptItems.InputEntity - dbo.ReciptItems.Outp' +
        'utEntity) >= 0.0001 ) OR'
      
        '         (SUM(dbo.ReciptItems.InputEntity - dbo.ReciptItems.Outp' +
        'utEntity) <= - 0.0001) OR'
      
        '         (SUM(dbo.ReciptItems.InputWeight - dbo.ReciptItems.Outp' +
        'utWeight) >= 0.0001) OR'
        ...
        '         (SUM(dbo.ReciptItems.TotalInputPrice - dbo.ReciptItems.' +
        'TotalOutputPrice) >= 0.1) OR'
      
        '         (SUM(dbo.ReciptItems.TotalInputPrice - dbo.ReciptItems.' +
        'TotalOutputPrice) <= - 0.1)'
      '          '
      ''
      '')
    Left = 192
    ...
    object qryEntityUseUnitID: TIntegerField
      DisplayLabel = #1603#1583' '#1587#1601#1575#1585#1588
      FieldName = 'UseUnitID'
    end
    object qryEntityUseUnitName: TStringField
