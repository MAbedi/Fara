...
SELECT  SUBSTRING(dbo.MiladiTOShamsi(dte), 3, 10),  SUBSTRING(dbo.MiladiTOShamsi(dte), 3, 2) AS Y , SUBSTRING(dbo.MiladiTOShamsi(dte), 6, 2) AS M,
...
FROM
(SELECT dte FROM
(SELECT dte = dateadd(dd,i,'20160321') FROM
...
WHERE SUBSTRING(dbo.MiladiTOShamsi(dte), 3, 10) NOT IN(SELECT CalendarDate FROM Tsh.Calendar)
ORDER BY dte
------------ fill Calendar ----------------------------------------------------
