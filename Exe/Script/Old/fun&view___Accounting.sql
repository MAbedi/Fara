...
SET @SqlFinal =
       	'SELECT ALLSql.*,Fitful.FitfulIDc AS _CalBalance ,Fitful.FitfulIDc AS _rate,Fitful.FitfulIDc AS  _calcuRate'+
		               ',Fitful.FitfulIDc AS  _CurBalance FROM( '+CHAR(13)+@SqlFinal+CHAR(13)+' )AS ALLSql  CROSS JOIN Fitful '+CHAR(13)
...
begin
	...
	BEGIN
	---	IF (@tblName LIKE N'acc.Details' ) SET @TempJoin = ' AND D.CompanyCode = D. ' ELSE SET @TempJoin = ''
...
SELECT   serial
		...
		,CurrencyDebit 
		,CurrencyCredit
		,CurrencyDebit-CurrencyCredit Currencybalance
FROM	(
	 SELECT  DG.YearID
			,DG.Serial
			,D.ID
...
return
(
SELECT    DocGroups.YearId ,Documents.DetailCode,   Categories.TopicCode
, case when [documents].[detailcode]=0 then MoeenName_L1 else Details.DetailName_L1 end AS DetailName
, case when [documents].[detailcode]=0 then MoeenName_L2 else Details.DetailName_L2 end AS DetailName_L2
...
, Sum(Documents.debt) AS Allbed, Sum(Documents.Credit) AS Allbes
...
FROM       Acc.Documents as Documents INNER JOIN
              acc.DocGroups as DocGroups ON  Documents.Serial = DocGroups.Serial AND Documents.YearID =DocGroups.YearID 
                 AND Documents.CompanyCode = DocGroups.CompanyCode INNER JOIN
...
WHERE   (CASE WHEN @DocTypeCode1_Not = '' THEN 1 ELSE 0 END = 1 OR
        ...
        (Documents.DetailCode  BETWEEN  @DetailCodeFrom  AND  @DetailCodeto) AND
        (Documents.TopicCode  BETWEEN @AccCodeFrom AND @AccCodeTo ) AND
        ((CASE WHEN @Status1 = '' THEN 1 ELSE 0 END = 1 and (DocGroups.Status <> 0)) OR
        (DocGroups.Status in (Select part From [Acc].[SplitString] (@Status1,',')))) AND 
		(DocGroups.DocDate <= @DocDateTo)

GROUP BY DocGroups.YearId ,Documents.DetailCode, Details.DetailName_L1, Details.DetailName_L2  ,Categories.TopicCode, Categories.MoeenName_L1,
...
SELECT    DocGroups.YearId ,Documents.CTopicCode,   Categories.TopicCode
, case when [documents].[CTopicCode]=0 then MoeenName_L1 else CenterTopics.CTopicName_L1 end AS CTopicName_L1
, case when [documents].[CTopicCode]=0 then MoeenName_L2 else CenterTopics.CTopicName_L2 end AS CTopicName_L2 
, Sum( case when DocTypeCode = 1 then debt else 0 end) AS Firstdebt
, Sum( case when DocTypeCode=1 then Credit else 0 end) AS FirstCredit
...
FROM        Acc.Documents as Documents INNER JOIN
			 ...
			 ON  Documents.Serial = DocGroups.Serial AND Documents.YearID =DocGroups.YearID AND     
			Documents.CompanyCode =DocGroups.CompanyCode INNER JOIN
                      acc.CenterTopics as CenterTopics  ON Documents.CTopicCode = CenterTopics.CTopicCode INNER JOIN
...
WHERE  	(DocGroups.DocTypeCode in (Select part From [Acc].[SplitString] (@DocTypeCode1_Not,',') )) AND    	
		...
		(DocGroups.YearID  BETWEEN @FromYearID  AND  @TOYearID  )  AND
	             (DocGroups.PrimaryDocNo BETWEEN @PrimaryDocNoFrom AND @PrimaryDocNoTo)  AND 
 		(DocGroups.CompanyCode BETWEEN @CompanyCodeFrom AND @CompanyCodeTo) AND
                          ( Documents.CTopicCode  BETWEEN  @CTopicCodeFrom  AND  @CTopicCodeto) AND
                           ( Documents.TopicCode  BETWEEN @AccCodeFrom AND @AccCodeTo ) AND
                            ( DocGroups.Status in (Select part From [Acc].[SplitString] (@Status1,','))) AND
		DocGroups.DocDate <= @DocDateTo
GROUP BY DocGroups.YearId ,Documents.CTopicCode ,  CenterTopics.CTopicName_L1,  CenterTopics.CTopicName_L2  ,Categories.TopicCode, Categories.MoeenName_L1,
		 Categories.MoeenName_L2
)
...
return
(
SELECT     DocGroups.YearId , LEFT(Documents.TopicCode, @LenAccCode) AS AccCode, LEFT(Documents.TopicCode, @LenPrvAccCode) AS PrvAccCode
...
FROM acc.DocGroups as DocGroups INNER JOIN 
       (acc.Categories as Categories INNER JOIN acc.Documents as Documents ON Categories.TopicCode = Documents.TopicCode) ON     
       Documents.Serial = DocGroups.Serial AND Documents.YearID = DocGroups.YearID AND Documents.CompanyCode = DocGroups.CompanyCode
...
GROUP BY  DocGroups.YearId ,LEFT(Documents.TopicCode, @LenPrvAccCode) ,LEFT(Documents.TopicCode,@LenAccCode)

HAVING ( LEFT(Documents.TopicCode, @LenAccCode) BETWEEN @AccCodeFrom AND @AccCodeTo ) 
)
...
(	
@BudgetTopicFrom int, @BudgetTopicTo int, @DateFrom Varchar(10), @DateTo Varchar(10), @CompanyCode varchar(30), 
@AccTopicCodeFrom bigint, @AccTopicCodeTo bigint, @YearID int
)
-- =============================================
...
FROM	Acc.BudgetItems INNER JOIN
		Acc.Budgets ON Acc.BudgetItems.BudgetID = Acc.Budgets.BudgetID INNER JOIN
...
CREATE FUNCTION [Acc].[Rptjournal]  (@DocTypeCode varchar(1000)='' ,@CompanyCodeFrom integer=[0], @CompanyCodeTo integer=[2147483647] ,
...
SELECT   D.Serial, DG.SecondaryDocNo, DG.PrimaryDocNo , DG.DocTopic_L1,
                      DG.DocDate, D.TopicCode, C.MoeenName_L1, 
...
CREATE FUNCTION [Acc].[BudgetReportOnTopicCode]     ( @DocDateFrom Varchar(10), @DocDateTo Varchar(10), @YearIdFrom int, @YearIdTo int, @TopicCodeFrom bigint, @TopicCodeTo bigint )
RETURNS table AS  
return
(
...
FROM         (SELECT      D.TopicCode, Acc.Categories.MoeenName_L1, Acc.Categories.MoeenName_L2, Acc.Categories.BudgetPrice, 
                        ...
                        FROM         Acc.Documents D INNER JOIN
                                              ...
                                              Acc.DocGroups DG ON D.Serial = DG.Serial AND D.YearID = DG.YearID AND 
                                              D.CompanyCode = DG.CompanyCode
                        WHERE     (D.YearID BETWEEN @YearIdFrom AND @YearIdTo) AND (D.TopicCode BETWEEN @TopicCodeFrom AND @TopicCodeTo)
	...
	BEGIN
		...
		BEGIN
		    SET @Sql ='UPDATE '+@SourceDataBase+' SET DocNo='+RTRIM(LTRIM(@NewSecondaryDocNo))+' WHERE DocNo = '+RTRIM(LTRIM(@OldSecondaryDocNo))
				...
				IF (@SourceDataBase LIKE '%dbo.Recipts')  -- ReciptsRegulates
        		  SET @Sql = @SQL +'UPDATE ReciptsRegulates SET RegDocNo='+RTRIM(LTRIM(@NewSecondaryDocNo))+' WHERE RegDocNo = '+RTRIM(LTRIM(@OldSecondaryDocNo))
