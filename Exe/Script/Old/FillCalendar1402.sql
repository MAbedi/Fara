...
SELECT  SUBSTRING(dbo.MiladiTOShamsi(dte), 1, 10),  SUBSTRING(dbo.MiladiTOShamsi(dte), 1, 4) AS Y , SUBSTRING(dbo.MiladiTOShamsi(dte), 6, 2) AS M,
...
FROM
(SELECT dte FROM
(SELECT dte = dateadd(dd,i,'20230321') FROM
...
WHERE SUBSTRING(dbo.MiladiTOShamsi(dte), 1, 10) NOT IN(SELECT CalendarDate FROM Tsh.Calendar)
ORDER BY dte
------------ fill Calendar ----------------------------------------------------
