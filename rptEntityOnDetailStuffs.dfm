inherited rptEntityOnDetailStuffsF: TrptEntityOnDetailStuffsF
  Left = 348
  Top = 141
  ...
  object qryDetailStuffs: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT      ReciptItems_Stock.StoreID, ReciptItems_Stock.Control' +
        'Code, ReciptItems_Stock.StuffCode, dbo.StuffCoding.c_StuffName, ' +
        'dbo.Units.UnitName,'
        ...
        '                      ROUND(SUM(ReciptItems_Stock.InputEntity - ' +
        'ReciptItems_Stock.OutputEntity), 3) AS Entity,'
      
        '                      ROUND(SUM(ReciptItems_Stock.InputWeight - ' +
        'ReciptItems_Stock.OutputWeight), 3) AS Weight,'
      
        '                      SUM(ReciptItems_Stock.TotalInputPrice - Re' +
        'ciptItems_Stock.TotalOutputPrice) AS Price, MAX(ReciptItems_Stoc' +
        'k.PersonID1) AS PersonID1,StuffCoding.c_StuffTecInfo'
      ...
      ' INNER JOIN'
        ...
        'WHERE     (ReciptItems_Stock.EffectType IN (2, 4)) AND (ReciptIt' +
        'ems_Stock.ReciptState < 3)AND'
      
        '(ReciptItems_Stock.ReciptDate  BETWEEN :DateFrom AND :DateTo) AN' +
        'D (ReciptItems_Stock.StoreID  BETWEEN :StoreIDFrom  AND :StoreID' +
        'To) AND (ReciptItems_Stock.StuffCode  BETWEEN :StuffCodeFrom AND' +
        ' :StuffCodeTo) AND'
      
        ' (ReciptItems_Stock.ControlCode  BETWEEN :ControlCodeFrom AND :C' +
        'ontrolCodeTo)'
      'AND (dbo.StuffCoding.OwnerShipKind  in (:StuffKindActive))'
      
        'GROUP BY ReciptItems_Stock.StoreID, ReciptItems_Stock.StuffCode,' +
        ' dbo.StuffCoding.c_StuffName, dbo.Units.UnitName, ReciptItems_St' +
        'ock.StuffSize,'
      
        '                      ReciptItems_Stock.StuffDiameter, ReciptIte' +
        'ms_Stock.StuffAlloy, ReciptItems_Stock.ControlCode, ReciptItems_' +
        'Stock.StuffGrade,StuffCoding.c_StuffTecInfo'
      
        'HAVING      (SUM(ReciptItems_Stock.InputEntity - ReciptItems_Sto' +
        'ck.OutputEntity) >= 0.0001) OR'
      
        '                      (SUM(ReciptItems_Stock.InputEntity - Recip' +
        'tItems_Stock.OutputEntity) <= - 0.0001) OR'
      
        '                      (SUM(ReciptItems_Stock.InputWeight - Recip' +
        'tItems_Stock.OutputWeight) >= 0.0001) OR'
      
        '                      (SUM(ReciptItems_Stock.InputWeight - Recip' +
        'tItems_Stock.OutputWeight) <= - 0.0001) OR'
      
        '                      (SUM(ReciptItems_Stock.TotalInputPrice - R' +
        'eciptItems_Stock.TotalOutputPrice) >= 0.1) OR'
      
        '                      (SUM(ReciptItems_Stock.TotalInputPrice - R' +
        'eciptItems_Stock.TotalOutputPrice) <= - 0.1)'
      
        'ORDER BY ReciptItems_Stock.ControlCode, ReciptItems_Stock.StuffC' +
        'ode'
      '')
    Left = 192
    Top = 112
  ...
  object pm1: TPopupMenu
    ...
    Left = 488
    Top = 392
    object mniEntityOnDetailStuffs: TMenuItem
      Caption = #1670#1575#1662'1'
      Hint = 'rptEntityOnDetailStuffs'
      OnClick = mniAllClick1Click
    end
    object mniEntityOnDetailStuffs1: TMenuItem
      Caption = #1670#1575#1662'2'
      Hint = 'rptEntityOnDetailStuffs1'
      OnClick = mniAllClick1Click
    end
    object mniAllClick1: TMenuItem
      Caption = 'AllClick'
      Visible = False
      OnClick = mniAllClick1Click
    end
