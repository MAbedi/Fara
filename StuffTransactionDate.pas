...
type
  ...
  TStuffTransactionDateF = class(Ttemplate2MDIF)
    ...
    rptStuffTransactionDateprice1: TMenuItem;
    rptStuffTransactionDateprice2: TMenuItem;
    actPrintVijeh: TAction;
    BitBtn6: TBitBtn;
    rptStuffTransactionDateprice4: TMenuItem;
    BitBtn7: TBitBtn;
    actShow: TAction;
    StuffTransactionDateFBahayeTamamshode: TMenuItem;
    ...
    ppLabel10: TppLabel;
    rptStuffTransactionDateprice3: TMenuItem;
  ...
  private
    ...
    procedure MakeSql(IsRemanin: Boolean);
  public
...
var
  StuffTransactionDateF: TStuffTransactionDateF;
...
procedure TStuffTransactionDateF.Enter(StoreID: Integer; StuffCode: Largeint);
begin
  try
    Var_glb_NoFilter := True;
    CreateMDIForm2(TStuffTransactionDateF, StuffTransactionDateF, mainF);
    StuffTransactionDateF.qryStore.Locate('n_StoreID', StoreID, []);
    if (StuffTransactionDateF.qryTransAction.Active) then
...
function TStuffTransactionDateF.LocateNewconst;
begin
  Result := False;
  With StuffTransactionDateF.qryTransAction do
    try
      DisableControls;
      First;
...
procedure TStuffTransactionDateF.Panel2Enter(Sender: TObject);
begin
  inherited;
  THackGrid(DBGrid1).DoSave(rgpStore.ItemIndex.ToString);
end;

procedure TStuffTransactionDateF.actFilterExecute(Sender: TObject);
begin
  inherited;
  ...
  begin
    try
      ...
      AddItem(DMf.adcBSell, 'Date', 'تاريخ ', 'بتاريخ', ftDate, dvDefaults,
        ...
        QuotedStr(APPBank.StartYear), QuotedStr(APPBank.endYear));
...
procedure TStuffTransactionDateF.MakeSql(IsRemanin: Boolean);
var
  s1, E_W: String;
begin
  // Make Sql
  ...
  begin
    ...
    if IsRemanin then
    begin
      SQL.Add('SELECT 0 as Part,''مانده از قبل'' AS Titel , ');
    end
    else
    begin
      SQL.Add('SELECT 1 as Part,''گردش طی دوره'' AS Titel , ');
    end;
    ...
    if chkPersonID1D.Checked and not IsRemanin then
    begin
    ...
    if IsRemanin then
    begin
      SQL.Add('AND(R.ReciptDate < @ReciptDateFrom)');
      SQL.Add('AND(R.yearID = @YearIDFrom  )');
...
procedure TStuffTransactionDateF.UpdateFilter;
...
begin
  ...
  begin
    Active := False;
    SQL.Text := EmptyStr;
    SQL.Add('DECLARE @ReciptDateFrom char(10)= :ReciptDateFrom');
    SQL.Add('DECLARE @ReciptDateTo char(10)	 = :ReciptDateTo');
    SQL.Add('DECLARE @GroupIDFrom int		     = :GroupIDFrom');
    SQL.Add('DECLARE @GroupIDTo int		   	   = :GroupIDTo');
    SQL.Add('DECLARE @YearIDFrom int			   = :YearIDFrom');
    SQL.Add('DECLARE @YearIDTo int		   	   = :YearIDTo');
    SQL.Add('DECLARE @StoreIDFrom int			   = :StoreIDFrom');
    SQL.Add('DECLARE @StoreIDTo int			     = :StoreIDTo');
    SQL.Add('DECLARE @SellSemporiumFrom int  = :SellSemporiumFrom');
    SQL.Add('DECLARE @SellSemporiumTo int		 = :SellSemporiumTo');
    SQL.Add('DECLARE @StuffCodeFrom Bigint	 = :StuffCodeFrom');
    SQL.Add('DECLARE @StuffCodeTo bigint		 = :StuffCodeTo');
    SQL.Add('DECLARE @PersonID3From int		   = :PersonID3From');
    SQL.Add('DECLARE @PersonID3To int			   = :PersonID3To');
    SQL.Add('DECLARE @SecondTypeFrom int		 = :SecondTypeFrom');
    SQL.Add('DECLARE @SecondTypeTo int		   = :SecondTypeTo');
    SQL.Add('DECLARE @PersonID1dFrom int		 = :PersonID1dFrom');
    SQL.Add('DECLARE @PersonID1dTo int		   = :PersonID1dTo');
    ...
    case rgpStore.ItemIndex of
      1:
        begin
          SQL.Add('ORDER BY ABCD.StoreID,Part');
        end;
    else
      begin
        SQL.Add('ORDER BY ABCD.StuffCode,Part');
      end;
    end;
    ...
    ParamByName('ReciptDateFrom').Value :=
    ...
    lblLimit.Caption := 'از كد كالاي ' +
      ...
      'گردش كالا بتاريخ : ' + ParamByName('ReciptDateFrom').Value + ' تا ' +
      ParamByName('ReciptDateTo').Value;
  end; // with
  with qryStore do
  begin
...
procedure TStuffTransactionDateF.FormDestroy(Sender: TObject);
begin
  inherited;
  THackGrid(DBGrid1).DoSave((rgpStore.ItemIndex.ToString));
  P3ActAdd.Free;
...
procedure TStuffTransactionDateF.actSendToExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TStuffTransactionDateF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryTransAction);
end;

procedure TStuffTransactionDateF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryTransAction);
end;

procedure TStuffTransactionDateF.SpeedButton1Click(Sender: TObject);
...
procedure TStuffTransactionDateF.qryStoreAfterScroll(DataSet: TDataSet);
begin
  inherited;
  with qryTransAction do
  begin
...
procedure TStuffTransactionDateF.qryStoreBeforeScroll(DataSet: TDataSet);
begin
  inherited;
  THackGrid(DBGrid1).DoSave(rgpStore.ItemIndex.ToString);
end;

procedure TStuffTransactionDateF.qryTransActionAfterOpen(DataSet: TDataSet);
var
  i: Integer;
begin
  inherited;
  ...
  with qryTransAction do
  begin
      FieldByName('Titel').DisplayLabel := 'عنوان';
...
//  qryTransAction.FieldByName('InvSellPrice').Visible := opt.ActiveSellPrice2;
  ...
  DBGrid1.ColorDBGrid;
  setColumns2(DBGrid1, false, 'Part');

end;

procedure TStuffTransactionDateF.rgpStoreClick(Sender: TObject);
begin
  inherited;
...
procedure TStuffTransactionDateF.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
  actFilter.Execute;
...
procedure TStuffTransactionDateF.ppLblCompanyNameGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TStuffTransactionDateF.ppLabel2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TStuffTransactionDateF.ppSystemVariable2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TStuffTransactionDateF.ppSystemVariable1GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TStuffTransactionDateF.actPrintExecute(Sender: TObject);
begin
  inherited;
  PopMnuPrint.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TStuffTransactionDateF.ppLabel10GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TStuffTransactionDateF.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TStuffTransactionDateF.AllMenuItemClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TStuffTransactionDateF.chkPersonID1DClick(Sender: TObject);
begin
  inherited;
  UpdateFilter
end;

procedure TStuffTransactionDateF.DBGrid1DblClick(Sender: TObject);
var
  StoreID: Integer;
begin
  inherited;
...
procedure TStuffTransactionDateF.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TStuffTransactionDateF.actPrintVijehExecute(Sender: TObject);
begin
  inherited;
  DBGrid2PrintF.showGrid2Print(DBGrid1, 0);
end;

procedure TStuffTransactionDateF.actShowExecute(Sender: TObject);
begin
  inherited;
