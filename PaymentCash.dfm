inherited PaymentCashF: TPaymentCashF
  Left = 389
  Top = 180
  Caption = #1578#1606#1582#1608#1575#1607
  ClientHeight = 663
  ClientWidth = 985
  OnResize = FormResize
  ExplicitWidth = 993
  ExplicitHeight = 694
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel3: TPanel [0]
    Top = 55
    Width = 985
    Height = 567
    AutoSize = True
    BevelInner = bvNone
    BevelOuter = bvNone
    TabOrder = 1
    TabStop = True
    ExplicitTop = 55
    ExplicitWidth = 985
    ExplicitHeight = 567
    object pnlDelete: TPanel
      Left = 0
      Top = 460
      Width = 985
      Height = 23
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 9
      DesignSize = (
        985
        23)
      object spdSelectCheck: TSpeedButton
        Left = 5
        Top = 1
        Width = 23
        Height = 22
        Hint = #1601#1585#1575#1582#1608#1575#1606#1610' '#1670#1603#1607#1575
        Glyph.Data = {
          
          384A36384A38384A36364A37374A3B3D4A43434C4141466F6F6F}
        ParentShowHint = False
        ShowHint = True
        OnClick = spdSelectCheckClick
      end
      object SpeedButton16: TSpeedButton
        Left = 32
        Top = 1
        Width = 75
        Height = 22
        Action = DataSetItemDelette
        Glyph.Data = {
          
          0404040404040404040404040404040404040404040404040404}
        ParentShowHint = False
        ShowHint = True
      end
      object BitBtn2: TBitBtn
        Left = 894
        Top = -2
        Width = 79
        Height = 25
        Action = actCopyPaste
        Anchors = [akTop, akRight]
        Caption = #1603#1662#1610' '#1587#1591#1585
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        TabOrder = 0
      end
    end
    object pnlmasterAccount: TPanel
      Left = 0
      Top = 224
      Width = 985
      Height = 82
      Align = alTop
      AutoSize = True
      BevelOuter = bvNone
      Caption = '.............'
      TabOrder = 6
      TabStop = True
      object pnlCTopicCode3: TPanel
        Left = 0
        Top = 54
        Width = 985
        Height = 28
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 0
        object SpeedButton18: TSpeedButton
          Tag = 26
          AlignWithMargins = True
          Left = 197
          Top = 3
          Width = 23
          Height = 22
          Align = alLeft
          Caption = '...'
          OnClick = SpeedButton18Click
          ExplicitLeft = 198
          ExplicitTop = 0
        end
        object Label28: TLabel
          AlignWithMargins = True
          Left = 300
          Top = 3
          Width = 98
          Height = 22
          Align = alLeft
          Alignment = taRightJustify
          AutoSize = False
          Caption = #1603#1583' '#1578#1601#1589#1610#1604#1610'4'
          FocusControl = DBEdit10
          ExplicitLeft = 303
          ExplicitTop = 4
          ExplicitHeight = 13
        end
        object DBEdit10: TDBEdit
          Tag = 23
          AlignWithMargins = True
          Left = 226
          Top = 3
          Width = 68
          Height = 22
          Align = alLeft
          DataField = 'CTopicCode3'
          DataSource = srcForms
          TabOrder = 0
          ExplicitHeight = 21
        end
        object DBEdit12: TDBEdit
          AlignWithMargins = True
          Left = 3
          Top = 3
          Width = 188
          Height = 22
          TabStop = False
          Align = alLeft
          Color = 15132390
          DataField = '_CTopicName3_L1'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 1
          ExplicitHeight = 21
        end
      end
      object pnllTopicCodeDetailCode: TPanel
        Left = 599
        Top = 0
        Width = 386
        Height = 54
        Align = alRight
        AutoSize = True
        BevelOuter = bvNone
        TabOrder = 1
        object pnlTopicCode: TPanel
          Left = 0
          Top = 0
          Width = 386
          Height = 27
          Align = alTop
          AutoSize = True
          BevelOuter = bvNone
          TabOrder = 0
          TabStop = True
          object btnTopicCode: TSpeedButton
            AlignWithMargins = True
            Left = 186
            Top = 3
            Width = 23
            Height = 21
            Align = alRight
            Caption = '...'
            OnClick = btnTopicCodeClick
            ExplicitLeft = 18
            ExplicitTop = 5
            ExplicitHeight = 22
          end
          object lbl1: TLabel
            AlignWithMargins = True
            Left = 285
            Top = 3
            Width = 98
            Height = 21
            Align = alRight
            Alignment = taRightJustify
            AutoSize = False
            Caption = #1603#1583' '#1581#1587#1575#1576
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            ExplicitLeft = 329
          end
          object edt1: TDBEdit
            AlignWithMargins = True
            Left = 215
            Top = 3
            Width = 64
            Height = 21
            Align = alRight
            DataField = 'TopicCode'
            DataSource = srcForms
            TabOrder = 0
            OnKeyDown = ALLDBEditKeyDown
          end
          object edt2: TDBEdit
            AlignWithMargins = True
            Left = 3
            Top = 3
            Width = 177
            Height = 21
            TabStop = False
            Align = alClient
            Color = 15132390
            DataField = '_TopicCodeName_L1'
            DataSource = srcForms
            ReadOnly = True
            TabOrder = 1
          end
        end
        object pnlDetailCode: TPanel
          Left = 0
          Top = 28
          Width = 386
          Height = 26
          Align = alBottom
          AutoSize = True
          BevelOuter = bvNone
          TabOrder = 1
          TabStop = True
          object btnDetailCode: TSpeedButton
            AlignWithMargins = True
            Left = 186
            Top = 3
            Width = 23
            Height = 20
            Align = alRight
            Caption = '...'
            OnClick = btnDetailCodeClick
            ExplicitLeft = 10
            ExplicitTop = 5
            ExplicitHeight = 22
          end
          object lbl2: TLabel
            AlignWithMargins = True
            Left = 285
            Top = 3
            Width = 98
            Height = 20
            Align = alRight
            Alignment = taRightJustify
            AutoSize = False
            Caption = ' '#1603#1583' '#1578#1601#1589#1610#1604#1610
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            ExplicitLeft = 329
          end
          object edt3: TDBEdit
            AlignWithMargins = True
            Left = 215
            Top = 3
            Width = 64
            Height = 20
            Align = alRight
            DataField = 'DetailCode'
            DataSource = srcForms
            TabOrder = 0
            OnKeyDown = ALLDBEditKeyDown
            ExplicitHeight = 21
          end
          object edt4: TDBEdit
            AlignWithMargins = True
            Left = 3
            Top = 3
            Width = 177
            Height = 20
            TabStop = False
            Align = alClient
            Color = 15132390
            DataField = '_DetailsName_L1'
            DataSource = srcForms
            ReadOnly = True
            TabOrder = 1
            ExplicitHeight = 21
          end
        end
      end
      object pnlCTopicCode1CTopicCode2: TPanel
        Left = 0
        Top = 0
        Width = 402
        Height = 54
        Align = alLeft
        AutoSize = True
        BevelOuter = bvNone
        TabOrder = 2
        object pnlCTopicCode1: TPanel
          Left = 0
          Top = 0
          Width = 402
          Height = 28
          Align = alTop
          AutoSize = True
          BevelOuter = bvNone
          TabOrder = 0
          TabStop = True
          object btnCTopicCode: TSpeedButton
            AlignWithMargins = True
            Left = 199
            Top = 3
            Width = 23
            Height = 22
            Align = alRight
            Caption = '...'
            OnClick = btnCTopicCodeClick
            ExplicitLeft = 0
            ExplicitTop = 0
          end
          object lbl3: TLabel
            AlignWithMargins = True
            Left = 301
            Top = 3
            Width = 98
            Height = 22
            Align = alRight
            Alignment = taRightJustify
            AutoSize = False
            Caption = #1603#1583#1578#1601#1589#1610#1604#1610'2'
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            ExplicitLeft = 310
          end
          object edt5: TDBEdit
            AlignWithMargins = True
            Left = 228
            Top = 3
            Width = 67
            Height = 22
            Align = alRight
            DataField = 'CTopicCode'
            DataSource = srcForms
            TabOrder = 0
            OnKeyDown = ALLDBEditKeyDown
            ExplicitHeight = 21
          end
          object edt6: TDBEdit
            AlignWithMargins = True
            Left = 3
            Top = 3
            Width = 190
            Height = 22
            TabStop = False
            Align = alClient
            Color = 15132390
            DataField = '_CTopicName_L1'
            DataSource = srcForms
            ReadOnly = True
            TabOrder = 1
            ExplicitHeight = 21
          end
        end
        object pnlCTopicCode2: TPanel
          Left = 0
          Top = 26
          Width = 402
          Height = 28
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 1
          TabStop = True
          object btnCTopicCode2: TSpeedButton
            AlignWithMargins = True
            Left = 198
            Top = 3
            Width = 23
            Height = 22
            Align = alRight
            Caption = '...'
            OnClick = btnCTopicCode2Click
            ExplicitLeft = 197
          end
          object Label11: TLabel
            AlignWithMargins = True
            Left = 301
            Top = 3
            Width = 98
            Height = 22
            Align = alRight
            Alignment = taRightJustify
            AutoSize = False
            Caption = #1603#1583#1578#1601#1589#1740#1604#1740'3'
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            ExplicitLeft = 307
          end
          object DBEdit6: TDBEdit
            AlignWithMargins = True
            Left = 227
            Top = 3
            Width = 68
            Height = 22
            Align = alRight
            DataField = 'CTopicCode2'
            DataSource = srcForms
            TabOrder = 0
            OnKeyDown = ALLDBEditKeyDown
            ExplicitHeight = 21
          end
          object DBEdit3: TDBEdit
            AlignWithMargins = True
            Left = 3
            Top = 3
            Width = 189
            Height = 22
            TabStop = False
            Align = alClient
            Color = 15132390
            DataField = '_CTopicName2_L1'
            DataSource = srcForms
            ReadOnly = True
            TabOrder = 1
            ExplicitHeight = 21
          end
        end
      end
    end
    object pnlmasterBudject_AidInfo: TPanel
      Left = 0
      Top = 174
      Width = 985
      Height = 50
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 5
      TabStop = True
      object pnlmasterBudject: TPanel
        Left = 593
        Top = 0
        Width = 392
        Height = 50
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        TabStop = True
        DesignSize = (
          392
          50)
        object BtnBudgetCode: TSpeedButton
          Left = 192
          Top = 2
          Width = 23
          Height = 22
          Anchors = [akTop, akRight]
          Caption = '...'
          OnClick = BtnBudgetCodeClick
        end
        object Label7: TLabel
          Left = 291
          Top = 7
          Width = 98
          Height = 13
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          AutoSize = False
          Caption = #1603#1583' '#1576#1608#1583#1580#1607
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object btnProjectID: TSpeedButton
          Left = 192
          Top = 25
          Width = 23
          Height = 22
          Anchors = [akTop, akRight]
          Caption = '...'
          OnClick = btnProjectIDClick
        end
        object Label15: TLabel
          Left = 291
          Top = 29
          Width = 98
          Height = 13
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          AutoSize = False
          Caption = #1603#1583' '#1591#1585#1581
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit17: TDBEdit
          Left = 221
          Top = 3
          Width = 64
          Height = 21
          TabStop = False
          Anchors = [akTop, akRight]
          DataField = 'BudgetCode'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 0
          OnKeyDown = ALLDBEditKeyDown
        end
        object DBEdit15: TDBEdit
          Left = 221
          Top = 25
          Width = 64
          Height = 21
          TabStop = False
          Anchors = [akTop, akRight]
          DataField = 'ProjectID'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 1
          OnKeyDown = ALLDBEditKeyDown
        end
        object DBEdit16: TDBEdit
          Left = 9
          Top = 25
          Width = 177
          Height = 21
          TabStop = False
          Anchors = [akTop, akRight]
          Color = 15132390
          DataField = '_ProjectName'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 2
        end
        object DBEdit14: TDBEdit
          Left = 9
          Top = 3
          Width = 177
          Height = 21
          TabStop = False
          Align = alCustom
          Color = 15132390
          DataField = '_BudgetName'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 3
        end
      end
      object pnlAidInfo: TPanel
        Left = 0
        Top = 0
        Width = 593
        Height = 50
        Align = alClient
        BevelOuter = bvNone
        TabOrder = 1
        TabStop = True
        object LblAidInfoNo: TLabel
          Left = 306
          Top = 16
          Width = 33
          Height = 13
          Alignment = taRightJustify
          Caption = #1588#1605#1575#1585#1607' '
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object LblAidInfoDate: TLabel
          Left = 110
          Top = 17
          Width = 21
          Height = 13
          Alignment = taRightJustify
          Caption = #1578#1575#1585#1610#1582
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit19: TDBEdit
          Left = 199
          Top = 13
          Width = 96
          Height = 21
          TabStop = False
          DataField = 'AidInfoNo'
          DataSource = srcForms
          TabOrder = 0
        end
        object DBEdit21: TDBEdit
          Left = 5
          Top = 13
          Width = 99
          Height = 21
          TabStop = False
          DataField = 'AidInfoDate'
          DataSource = srcForms
          TabOrder = 1
        end
      end
    end
    object StatusBar1: TStatusBar
      Left = 0
      Top = 438
      Width = 985
      Height = 22
      BiDiMode = bdRightToLeft
      Panels = <
        item
          Width = 700
        end
        item
          Text = #1580#1605#1593' '#1603#1604
          Width = 300
        end>
      ParentBiDiMode = False
      SizeGrip = False
    end
    object Panel1: TPanel
      Left = 0
      Top = 483
      Width = 985
      Height = 22
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 10
      object pnlTopicCodeDetails: TPanel
        Left = 801
        Top = 0
        Width = 184
        Height = 22
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        object Label8: TLabel
          AlignWithMargins = True
          Left = 150
          Top = 3
          Width = 31
          Height = 13
          Align = alRight
          Alignment = taRightJustify
          Caption = #1581#1587#1575#1576
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit23: TDBEdit
          Left = 0
          Top = 0
          Width = 147
          Height = 22
          TabStop = False
          Align = alClient
          Color = 15132390
          DataField = '_TopicCodeName_L1'
          DataSource = srcFormItems
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 21
        end
      end
      object pnlDetailCodeDetail: TPanel
        Left = 617
        Top = 0
        Width = 184
        Height = 22
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 1
        object Label6: TLabel
          AlignWithMargins = True
          Left = 144
          Top = 3
          Width = 37
          Height = 13
          Align = alRight
          Alignment = taRightJustify
          Caption = #1578#1601#1589#1610#1604#1610
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit24: TDBEdit
          Left = 0
          Top = 0
          Width = 141
          Height = 22
          TabStop = False
          Align = alClient
          Color = 15132390
          DataField = '_DetailsName_L1'
          DataSource = srcFormItems
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 21
        end
      end
      object pnlCTopicCode1Detail: TPanel
        Left = 368
        Top = 0
        Width = 184
        Height = 22
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 2
        object Label3: TLabel
          AlignWithMargins = True
          Left = 138
          Top = 3
          Width = 43
          Height = 13
          Align = alRight
          Alignment = taRightJustify
          Caption = #1578#1601#1589#1610#1604#1610'2'
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit18: TDBEdit
          Left = 0
          Top = 0
          Width = 135
          Height = 22
          TabStop = False
          Align = alClient
          Color = 15132390
          DataField = '_CTopicName_L1'
          DataSource = srcFormItems
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 21
        end
      end
      object pnlCTopicCode2Detail: TPanel
        Left = 184
        Top = 0
        Width = 184
        Height = 22
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 3
        object Label4: TLabel
          AlignWithMargins = True
          Left = 138
          Top = 3
          Width = 43
          Height = 13
          Align = alRight
          Alignment = taRightJustify
          Caption = #1578#1601#1589#1740#1604#1740'3'
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit26: TDBEdit
          Left = 0
          Top = 0
          Width = 135
          Height = 22
          TabStop = False
          Align = alClient
          Color = 15132390
          DataField = '_CTopicName2_L1'
          DataSource = srcFormItems
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 21
        end
      end
      object pnlCTopicCode3Detail: TPanel
        Left = 0
        Top = 0
        Width = 184
        Height = 22
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 4
        object Label9: TLabel
          AlignWithMargins = True
          Left = 128
          Top = 3
          Width = 53
          Height = 13
          Align = alRight
          Alignment = taRightJustify
          Caption = #1605#1585#1603#1586#1607#1586#1610#1606#1607'3'
        end
        object DBEdit20: TDBEdit
          Left = 0
          Top = 0
          Width = 125
          Height = 22
          TabStop = False
          Align = alClient
          Color = 15132390
          DataField = '_CTopicName3_L1'
          DataSource = srcFormItems
          ReadOnly = True
          TabOrder = 0
          ExplicitHeight = 21
        end
      end
    end
    object Panel5: TPanel
      Left = 0
      Top = 87
      Width = 985
      Height = 29
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 2
      TabStop = True
      object lblReMainPerson: TLabel
        Left = 573
        Top = 0
        Width = 23
        Height = 13
        Align = alRight
        Caption = #1605#1575#1606#1583#1607
        Color = clHighlight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clGreen
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = True
      end
      object pnlCustomer1: TPanel
        Left = 596
        Top = 0
        Width = 389
        Height = 29
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        TabStop = True
        object lbcustomer1: TLabel
          Left = 288
          Top = 7
          Width = 98
          Height = 13
          Alignment = taRightJustify
          AutoSize = False
          Caption = #1603#1583' '#1608' '#1606#1575#1605' '#1662#1585#1583#1575#1582#1578' '#1603#1606#1606#1583#1607
        end
        object btnCustomerID1: TSpeedButton
          Tag = 1
          Left = 189
          Top = 2
          Width = 23
          Height = 22
          Caption = '...'
          OnClick = btnCustomerID1Click
        end
        object edtCustomerID1: TDBEdit
          Left = 218
          Top = 2
          Width = 64
          Height = 21
          Ctl3D = True
          DataField = 'CustomerID1'
          DataSource = srcForms
          ParentCtl3D = False
          TabOrder = 0
          OnDblClick = edtCustomerID1DblClick
          OnKeyDown = ALLDBEditKeyDown
        end
        object DBEdit11: TDBEdit
          Left = 6
          Top = 2
          Width = 177
          Height = 21
          TabStop = False
          Color = 15000804
          DataField = '_CustomerName'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 1
        end
      end
      object pnlCustomer2: TPanel
        Left = 0
        Top = 0
        Width = 402
        Height = 29
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 1
        TabStop = True
        object lbcustomer2: TLabel
          Left = 302
          Top = 7
          Width = 98
          Height = 13
          Alignment = taRightJustify
          Caption = #1603#1583' '#1608' '#1606#1575#1605' '#1662#1585#1583#1575#1582#1578' '#1603#1606#1606#1583#1607
        end
        object btnCustomerID2: TSpeedButton
          Tag = 2
          Left = 199
          Top = 2
          Width = 23
          Height = 22
          Caption = '...'
          OnClick = btnCustomerID1Click
        end
        object edtCustomerID2: TDBEdit
          Left = 228
          Top = 4
          Width = 67
          Height = 21
          Ctl3D = True
          DataField = 'CustomerID2'
          DataSource = srcForms
          ParentCtl3D = False
          TabOrder = 0
          OnDblClick = edtCustomerID2DblClick
          OnKeyDown = ALLDBEditKeyDown
        end
        object DBEdit7: TDBEdit
          Left = 3
          Top = 2
          Width = 190
          Height = 21
          TabStop = False
          Color = 15000804
          DataField = '_CustomerName2'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 1
        end
      end
    end
    object pnlSells: TPanel
      Left = 0
      Top = 58
      Width = 985
      Height = 29
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
      TabStop = True
      object pnlSellsEmporium: TPanel
        Left = 360
        Top = 0
        Width = 625
        Height = 29
        Align = alClient
        BevelOuter = bvNone
        TabOrder = 0
        TabStop = True
        DesignSize = (
          625
          29)
        object Label24: TLabel
          Left = 520
          Top = 6
          Width = 52
          Height = 13
          Anchors = [akTop, akRight]
          AutoSize = False
          Caption = #1605#1585#1603#1586' '#1601#1585#1608#1588
          FocusControl = dblkcbb_SellsEmporium
          ExplicitLeft = 528
        end
        object dblkcbb_SellsEmporium: TDBLookupComboBox
          Tag = 1
          Left = 321
          Top = 2
          Width = 193
          Height = 21
          Anchors = [akTop, akRight]
          DataSource = srcForms
          TabOrder = 0
        end
      end
      object pnlSellsMethod: TPanel
        Left = 0
        Top = 0
        Width = 360
        Height = 29
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 1
        TabStop = True
        object Label26: TLabel
          Left = 302
          Top = 6
          Width = 51
          Height = 13
          Caption = #1606#1581#1608#1607' '#1601#1585#1608#1588
          FocusControl = dblkcbb_SellsMethod
        end
        object dblkcbb_SellsMethod: TDBLookupComboBox
          Tag = 1
          Left = 3
          Top = 2
          Width = 292
          Height = 21
          DataSource = srcForms
          TabOrder = 0
        end
      end
    end
    object grpMasterNote: TGroupBox
      Left = 0
      Top = 505
      Width = 985
      Height = 62
      Align = alBottom
      Caption = #1578#1608#1590#1610#1581#1575#1578' '#1601#1585#1605' '
      TabOrder = 12
      object DBMemo2: TDBMemo
        Left = 2
        Top = 15
        Width = 981
        Height = 45
        Align = alClient
        DataField = 'FomNote'
        DataSource = srcForms
        TabOrder = 0
      end
    end
    object Panel4: TPanel
      Left = 0
      Top = 0
      Width = 985
      Height = 29
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 13
      object txtFormState: TDBText
        AlignWithMargins = True
        Left = 99
        Top = 3
        Width = 603
        Height = 23
        Align = alClient
        Alignment = taCenter
        DataField = 'FormState'
        DataSource = srcForms
        Font.Charset = ARABIC_CHARSET
        Font.Color = 8684799
        Font.Height = -15
        Font.Name = 'Tehran Light'
        Font.Style = [fsBold]
        ParentFont = False
        ExplicitLeft = 100
        ExplicitTop = 4
        ExplicitWidth = 91
        ExplicitHeight = 18
      end
      object txtDocDate: TDBText
        AlignWithMargins = True
        Left = 708
        Top = 6
        Width = 65
        Height = 13
        Hint = #1578#1575#1585#1740#1582' '#1587#1606#1583
        Margins.Top = 6
        Margins.Right = 10
        Align = alRight
        AutoSize = True
        DataField = 'DocDate'
        DataSource = srcForms
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtDocNo: TDBText
        AlignWithMargins = True
        Left = 786
        Top = 6
        Width = 52
        Height = 13
        Hint = #1588#1605#1575#1585#1607' '#1587#1606#1583
        Margins.Top = 6
        Margins.Right = 30
        Align = alRight
        AutoSize = True
        DataField = 'DocNo'
        DataSource = srcForms
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtFormState1: TDBText
        AlignWithMargins = True
        Left = 871
        Top = 6
        Width = 84
        Height = 13
        Margins.Top = 6
        Margins.Right = 30
        Align = alRight
        Alignment = taCenter
        AutoSize = True
        DataField = 'FormState'
        DataSource = srcForms
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object BitBtn11: TBitBtn
        Left = 0
        Top = 0
        Width = 96
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = actStateChange
        Align = alLeft
        Caption = #1578#1594#1740#1740#1585' '#1608#1590#1593#1740#1578
        TabOrder = 0
      end
    end
    object pnlPay_Topic: TPanel
      Left = 0
      Top = 145
      Width = 985
      Height = 29
      Align = alTop
      BevelOuter = bvNone
      Caption = ' '
      TabOrder = 4
      DesignSize = (
        985
        29)
      object LblTopicTypes: TLabel
        Left = 306
        Top = 3
        Width = 102
        Height = 13
        Alignment = taRightJustify
        Caption = #1605#1608#1590#1608#1593' '#1583#1585#1610#1575#1601#1578'/'#1662#1585#1583#1575#1582#1578
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object LblPayTypes: TLabel
        Left = 880
        Top = 8
        Width = 85
        Height = 13
        Anchors = [akTop, akRight]
        AutoSize = False
        BiDiMode = bdRightToLeftNoAlign
        Caption = #1606#1608#1593' '#1583#1585#1610#1575#1601#1578'/'#1662#1585#1583#1575#1582#1578
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentBiDiMode = False
        ParentFont = False
        ExplicitLeft = 888
      end
      object CmbPayTypes: TDBComboBox
        Left = 597
        Top = 4
        Width = 277
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'PayTypes'
        DataSource = srcForms
        TabOrder = 0
      end
      object CmbTopicTypes: TDBComboBox
        Left = 3
        Top = 0
        Width = 292
        Height = 21
        DataField = 'TopicTypes'
        DataSource = srcForms
        TabOrder = 1
      end
    end
    object DBGrid1: TCedarDbgrid
      Tag = 111
      Left = 0
      Top = 359
      Width = 985
      Height = 79
      Align = alClient
      Color = clCream
      DataSource = srcFormItems
      DynProps = <>
      Flat = True
      FooterParams.FillStyle = cfstGradientEh
      FooterParams.Font.Charset = DEFAULT_CHARSET
      FooterParams.Font.Color = clWindowText
      FooterParams.Font.Height = -11
      FooterParams.Font.Name = 'Tahoma'
      FooterParams.Font.Style = [fsBold]
      FooterParams.ParentFont = False
      FooterParams.VertLines = False
      GridLineParams.VertEmptySpaceStyle = dessNonEh
      HorzScrollBar.ExtraPanel.Visible = True
      IndicatorTitle.ShowDropDownSign = True
      IndicatorTitle.TitleButton = True
      OddRowColor = clGradientInactiveCaption
      Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
      RowHeight = 21
      SearchPanel.Enabled = True
      SearchPanel.FilterOnTyping = True
      SortLocal = True
      TabOrder = 8
      OnColEnter = DBGrid1ColEnter
      OnDblClick = actSeverableExecute
      OnEditButtonClick = DBGrid1EditButtonClick
      OnEnter = DBGrid1Enter
      OnExit = DBGrid1Exit
      OnKeyPress = DBGrid1KeyPress
      OnKeyUp = DBGrid1KeyUp
      ActiveSearchPanel = False
      SelectedSum = True
      SelectFooterIndex = 0
      Columns = <
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'Row'
          Footers = <>
          Title.TitleButton = True
          Width = 27
        end
        item
          BiDiMode = bdLeftToRight
          CellButtons = <>
          Color = 13431799
          DynProps = <>
          EditButtons = <>
          FieldName = 'CheckDate'
          Footers = <>
          Title.TitleButton = True
          Width = 32
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'CheckNumber'
          Footers = <>
          Title.TitleButton = True
          Width = 48
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'CustomerID2'
          Footers = <>
          Title.TitleButton = True
        end
        item
          CellButtons = <>
          Color = 15461355
          DynProps = <>
          EditButtons = <>
          FieldName = '_Cudtomername2'
          Footers = <>
          ReadOnly = True
          Title.TitleButton = True
          Width = 64
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'ItemNote'
          Footers = <>
          Title.TitleButton = True
          Width = 27
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'ItemAmount'
          Footers = <>
          Title.TitleButton = True
          Width = 26
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'ArzAmount'
          Footers = <>
          Title.TitleButton = True
          Width = 61
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'VatCo'
          Footers = <>
          Title.TitleButton = True
          Width = 30
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'VAT'
          Footers = <>
          Title.TitleButton = True
          Width = 64
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'ItemAmount_VAT'
          Footers = <>
          Title.TitleButton = True
          Width = 64
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'SeverableAmount'
          Footers = <>
          PickList.Strings = (
            #1582#1740#1585
            #1576#1604#1607)
          Title.TitleButton = True
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'ItemState'
          Footers = <>
          KeyList.Strings = (
            '1'
            '2'
            '1')
          PickList.Strings = (
            #1578#1575#1740#1740#1583
            #1593#1583#1605' '#1578#1575#1740#1740#1583)
          Title.TitleButton = True
          Width = 41
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'ItemStateComment'
          Footers = <>
          Title.TitleButton = True
          Width = 64
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'TopicCode'
          Footers = <>
          Title.TitleButton = True
          Width = 64
        end
        item
          Alignment = taRightJustify
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'DetailCode'
          Footers = <>
          Title.TitleButton = True
        end
        item
          Alignment = taRightJustify
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'CTopicCode'
          Footers = <>
          Title.TitleButton = True
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.DefaultAction = False
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'CTopicCode2'
          Footers = <>
          Title.TitleButton = True
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'CTopicCode3'
          Footers = <>
          Title.TitleButton = True
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'BudgetCode'
          Footers = <>
          Title.TitleButton = True
          Width = 45
        end
        item
          CellButtons = <>
          Color = 15461355
          DynProps = <>
          EditButtons = <>
          FieldName = '_BudgetCode'
          Footers = <>
          ReadOnly = True
          Title.TitleButton = True
          Width = 30
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'AidInfoNo'
          Footers = <>
          Title.TitleButton = True
        end
        item
          CellButtons = <>
          Color = 13431799
          DynProps = <>
          EditButtons = <>
          FieldName = 'AidInfoDate'
          Footers = <>
          Title.TitleButton = True
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'ProductCode'
          Footers = <>
          Title.TitleButton = True
          Width = 64
        end
        item
          ButtonStyle = cbsDropDown
          CellButtons = <>
          Color = 13236955
          DynProps = <>
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'CashWage'
          Footers = <>
          Title.TitleButton = True
          Width = 53
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'BankName'
          Footers = <>
          Width = 42
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'AccountNumber'
          Footers = <>
          Width = 32
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'AccountNumberNew'
          Footers = <>
          Width = 66
        end
        item
          ButtonStyle = cbsEllipsis
          CellButtons = <>
          DynProps = <>
          EditButton.Style = ebsEllipsisEh
          EditButton.Visible = True
          EditButtons = <>
          FieldName = 'ReciptItems_ID'
          Footers = <>
          Width = 83
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = '_ReciptItems_ID'
          Footers = <>
          Width = 100
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'PO'
          Footers = <>
          Width = 30
        end
        item
          CellButtons = <>
          DynProps = <>
          EditButtons = <>
          FieldName = 'CheckFor'
          Footers = <>
          Width = 82
        end>
      object RowDetailData: TRowDetailPanelControlEh
      end
    end
    object pnlRelatedRecipts: TPanel
      Left = 0
      Top = 29
      Width = 985
      Height = 29
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 0
      DesignSize = (
        985
        29)
      object btnRelatedRecipts: TSpeedButton
        Tag = 2
        Left = 781
        Top = 2
        Width = 23
        Height = 22
        Anchors = [akTop, akRight]
        Caption = '...'
        OnClick = btnRelatedReciptsClick
        ExplicitLeft = 789
      end
      object Label5: TLabel
        Left = 880
        Top = 7
        Width = 83
        Height = 13
        Alignment = taRightJustify
        Anchors = [akTop, akRight]
        AutoSize = False
        Caption = #1575#1585#1578#1576#1575#1591' '#1576#1575' '#1587#1601#1575#1585#1588#1575#1578
        ExplicitLeft = 888
      end
      object txt_RelatedRecipts: TDBText
        AlignWithMargins = True
        Left = 683
        Top = 3
        Width = 92
        Height = 13
        Anchors = [akTop, akRight]
        AutoSize = True
        Color = 15000804
        DataField = '_RelatedRecipts'
        DataSource = srcForms
        ParentColor = False
      end
      object DBEdit5: TDBEdit
        Left = 810
        Top = 4
        Width = 64
        Height = 21
        Anchors = [akTop, akRight]
        Ctl3D = True
        DataField = 'RelatedRecipts'
        DataSource = srcForms
        ParentCtl3D = False
        TabOrder = 0
        OnKeyDown = ALLDBEditKeyDown
      end
    end
    object pnlArz: TPanel
      Left = 0
      Top = 306
      Width = 985
      Height = 53
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 7
      object pnlArz1: TPanel
        Left = 0
        Top = 0
        Width = 985
        Height = 26
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 0
        object Label10: TLabel
          AlignWithMargins = True
          Left = 402
          Top = 3
          Width = 98
          Height = 20
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1606#1585#1582' '#1575#1585#1586
          FocusControl = edtArzRate
          ParentBiDiMode = False
          ExplicitLeft = 549
          ExplicitHeight = 22
        end
        object Label12: TLabel
          AlignWithMargins = True
          Left = 645
          Top = 3
          Width = 98
          Height = 20
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1605#1602#1583#1575#1585' '#1575#1585#1586
          FocusControl = edtArzAmount
          ParentBiDiMode = False
          ExplicitLeft = 731
          ExplicitHeight = 22
        end
        object Label13: TLabel
          AlignWithMargins = True
          Left = 884
          Top = 3
          Width = 98
          Height = 20
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1606#1608#1593' '#1575#1585#1586
          ParentBiDiMode = False
          ExplicitLeft = 888
          ExplicitHeight = 22
        end
        object edtArzAmount: TDBEdit
          AlignWithMargins = True
          Left = 506
          Top = 3
          Width = 133
          Height = 20
          Align = alRight
          DataField = 'ArzAmount'
          DataSource = srcForms
          TabOrder = 1
          ExplicitHeight = 21
        end
        object edtArzRate: TDBEdit
          AlignWithMargins = True
          Left = 267
          Top = 3
          Width = 129
          Height = 20
          Align = alRight
          DataField = 'ArzRate'
          DataSource = srcForms
          TabOrder = 2
          ExplicitHeight = 21
        end
        object CmbArzTypeID: TDBComboBox
          AlignWithMargins = True
          Left = 749
          Top = 3
          Width = 129
          Height = 21
          Align = alRight
          DataField = 'ArzTypeID'
          DataSource = srcForms
          TabOrder = 0
        end
      end
      object pnlArz2: TPanel
        Left = 0
        Top = 26
        Width = 985
        Height = 27
        Align = alClient
        BevelOuter = bvNone
        TabOrder = 1
        object Label14: TLabel
          AlignWithMargins = True
          Left = 645
          Top = 3
          Width = 98
          Height = 21
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1605#1602#1583#1575#1585' '#1578#1587#1593#1740#1585' '#1575#1585#1586
          FocusControl = edtArzAmountExchange
          ParentBiDiMode = False
          ExplicitLeft = 722
          ExplicitHeight = 25
        end
        object Label16: TLabel
          AlignWithMargins = True
          Left = 402
          Top = 3
          Width = 98
          Height = 21
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1606#1585#1582' '#1578#1587#1593#1740#1585' '#1575#1585#1586
          FocusControl = edtArzRateExchange
          ParentBiDiMode = False
          ExplicitLeft = 454
          ExplicitTop = 0
          ExplicitHeight = 25
        end
        object Label17: TLabel
          AlignWithMargins = True
          Left = 884
          Top = 3
          Width = 98
          Height = 21
          Align = alRight
          AutoSize = False
          BiDiMode = bdLeftToRight
          Caption = #1606#1608#1593' '#1578#1587#1593#1740#1585' '#1575#1585#1586
          ParentBiDiMode = False
          ExplicitLeft = 888
          ExplicitHeight = 25
        end
        object edtArzAmountExchange: TDBEdit
          AlignWithMargins = True
          Left = 506
          Top = 3
          Width = 133
          Height = 21
          Align = alRight
          DataField = 'ArzAmountExchange'
          DataSource = srcForms
          TabOrder = 1
        end
        object edtArzRateExchange: TDBEdit
          AlignWithMargins = True
          Left = 267
          Top = 3
          Width = 129
          Height = 21
          Align = alRight
          DataField = 'ArzRateExchange'
          DataSource = srcForms
          TabOrder = 2
        end
        object cmbArzTypeIDExchange: TDBComboBox
          AlignWithMargins = True
          Left = 749
          Top = 3
          Width = 129
          Height = 21
          Align = alRight
          DataField = 'ArzTypeIDExchange'
          DataSource = srcForms
          TabOrder = 0
        end
      end
    end
    object pnlCustomerID3_PO: TPanel
      Left = 0
      Top = 116
      Width = 985
      Height = 29
      Align = alTop
      Anchors = [akTop, akRight]
      BevelOuter = bvNone
      Caption = ' '
      TabOrder = 3
      object pnlCustomerID3: TPanel
        Left = 339
        Top = 0
        Width = 646
        Height = 29
        Align = alClient
        BevelOuter = bvNone
        Caption = ' '
        TabOrder = 0
        DesignSize = (
          646
          29)
        object lblCustomerID3: TLabel
          Left = 537
          Top = 8
          Width = 57
          Height = 13
          Anchors = [akTop, akRight]
          AutoSize = False
          BiDiMode = bdRightToLeftNoAlign
          Caption = #1603#1583' '#1605#1588#1578#1585#1610'3'
          FocusControl = edtCustomerID3
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentBiDiMode = False
          ParentFont = False
          ExplicitLeft = 888
        end
        object btnCustomerID3: TSpeedButton
          Left = 438
          Top = 3
          Width = 23
          Height = 22
          Hint = '26'
          Anchors = [akTop, akRight]
          Caption = '...'
          OnClick = btnCustomerID3Click
          ExplicitLeft = 789
        end
        object edtCustomerID3: TDBEdit
          Left = 467
          Top = 5
          Width = 64
          Height = 21
          Hint = '26'
          Anchors = [akTop, akRight]
          AutoSize = False
          DataField = 'CustomerID3'
          DataSource = srcForms
          TabOrder = 0
          OnKeyDown = edtCustomerID3KeyDown
        end
        object DBEdit2: TDBEdit
          Left = 255
          Top = 4
          Width = 177
          Height = 21
          TabStop = False
          Anchors = [akTop, akRight]
          Color = 15000804
          DataField = '_CustomerID3'
          DataSource = srcForms
          ReadOnly = True
          TabOrder = 1
        end
      end
      object pnlPO: TPanel
        Left = 0
        Top = 0
        Width = 339
        Height = 29
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 1
        DesignSize = (
          339
          29)
        object lblPO: TLabel
          Left = 302
          Top = 7
          Width = 14
          Height = 13
          Alignment = taRightJustify
          Caption = 'PO'
          FocusControl = edtPO
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object edtPO: TDBEdit
          Tag = 20
          Left = 3
          Top = 3
          Width = 292
          Height = 21
          Anchors = [akRight, akBottom]
          DataField = 'PO'
          DataSource = srcForms
          TabOrder = 0
        end
      end
    end
  end
  inherited PnlUnderButton: TPanel [1]
    Top = 622
    Width = 985
    BevelOuter = bvNone
    TabOrder = 2
    ExplicitTop = 622
    ExplicitWidth = 985
    DesignSize = (
      985
      41)
    object DBNavigator1: TDBNavigator [0]
      Left = 346
      Top = 9
      Width = 240
      Height = 25
      DataSource = srcAllForms
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      Anchors = [akLeft, akTop, akRight]
      Flat = True
      TabOrder = 6
    end
    object newPanel: TPanel [1]
      Left = 499
      Top = 0
      Width = 486
      Height = 41
      Align = alRight
      AutoSize = True
      BevelOuter = bvNone
      TabOrder = 4
      object BitBtn3: TBitBtn
        AlignWithMargins = True
        Left = 408
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = DataSetInsert1
        Align = alRight
        Caption = #1580#1583#1610#1583
        TabOrder = 0
      end
      object BitBtn4: TBitBtn
        AlignWithMargins = True
        Left = 327
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = DataSetEdit1
        Align = alRight
        Caption = #1608#1610#1585#1575#1610#1588
        TabOrder = 1
      end
      object BitBtn5: TBitBtn
        AlignWithMargins = True
        Left = 246
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = DataSetDelete1_
        Align = alRight
        Caption = #1581#1584#1601
        TabOrder = 2
      end
      object btnOtherMenu: TBitBtn
        AlignWithMargins = True
        Left = 84
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = actSeverable
        Align = alRight
        Caption = #1578#1601#1705#1610#1705#8207#1662#1585#1583#1575#1582#1578
        Layout = blGlyphRight
        Style = bsWin31
        TabOrder = 3
        OnMouseEnter = btnOtherMenuMouseEnter
        OnMouseLeave = btnOtherMenuMouseLeave
      end
      object BitBtn6: TBitBtn
        AlignWithMargins = True
        Left = 165
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = actGetExcel
        Align = alRight
        Caption = #1583#1585#1610#1575#1601#1578'Excel'#8207
        TabOrder = 4
      end
      object BitBtn13: TBitBtn
        AlignWithMargins = True
        Left = 3
        Top = 6
        Width = 75
        Height = 29
        Margins.Top = 6
        Margins.Bottom = 6
        Action = actPostOverallType
        Align = alRight
        Caption = 'actPostOverallType'
        Layout = blGlyphRight
        Style = bsWin31
        TabOrder = 5
      end
    end
    object okPanel: TPanel [2]
      Left = 336
      Top = 0
      Width = 163
      Height = 41
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 5
      DesignSize = (
        163
        41)
      object BitBtn7: TBitBtn
        Left = 87
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetPost1
        Anchors = [akTop, akRight]
        Caption = #1578#1575#1610#1610#1583
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        TabOrder = 0
      end
      object BitBtn8: TBitBtn
        Left = 7
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetCancel1
        Anchors = [akTop, akRight]
        Caption = #1575#1606#1589#1585#1575#1601
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        TabOrder = 1
      end
    end
    inherited BtnReject: TBitBtn
      Left = 3
      Top = 6
      Height = 29
      ExplicitLeft = 3
      ExplicitTop = 6
      ExplicitHeight = 29
    end
    object BitBtn1: TBitBtn
      AlignWithMargins = True
      Left = 246
      Top = 6
      Width = 71
      Height = 29
      Margins.Top = 6
      Margins.Bottom = 6
      Action = actSendToExcel
      Align = alLeft
      Caption = #1575#1585#1587#1575#1604#8207#1576#1607'Excel'
      TabOrder = 1
    end
    object BitBtn12: TBitBtn
      AlignWithMargins = True
      Left = 165
      Top = 6
      Width = 75
      Height = 29
      Margins.Top = 6
      Margins.Bottom = 6
      Action = actSearch_
      Align = alLeft
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 2
    end
    object BitBtn9: TBitBtn
      AlignWithMargins = True
      Left = 84
      Top = 6
      Width = 75
      Height = 29
      Margins.Top = 6
      Margins.Bottom = 6
      Action = actSort
      Align = alLeft
      Caption = #1578#1585#1578#1610#1576
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00067B1100FF00FF00FF00FF00FF00FF00}
      TabOrder = 3
    end
    object BitBtn10: TBitBtn
      AlignWithMargins = True
      Left = 323
      Top = 6
      Width = 75
      Height = 29
      Margins.Top = 6
      Margins.Bottom = 6
      Action = actPrint
      Align = alLeft
      Caption = #1670#1575#1662
      TabOrder = 7
    end
  end
  inherited Panel2: TPanel [2]
    Width = 985
    Height = 55
    BevelOuter = bvNone
    TabOrder = 0
    ExplicitWidth = 985
    ExplicitHeight = 55
    DesignSize = (
      985
      55)
    inherited ImgTemplate: TImage
      Left = 943
      ExplicitLeft = 751
    end
    inherited lblCaption: TLabel
      Left = 878
      Top = 15
      Height = 37
      ExplicitLeft = 878
      ExplicitTop = 15
    end
    inherited lblBaseDate: TLabel
      Left = 425
      ExplicitLeft = 397
    end
    object Label1: TLabel
      Left = 96
      Top = 10
      Width = 50
      Height = 13
      Alignment = taRightJustify
      Caption = #1588#1605#1575#1585#1607' '#1601#1585#1605
    end
    object SpeedButton5: TSpeedButton
      Left = 1
      Top = 6
      Width = 23
      Height = 20
      Caption = '...'
      OnClick = SpeedButton5Click
    end
    object Label2: TLabel
      Left = 96
      Top = 35
      Width = 41
      Height = 13
      Alignment = taRightJustify
      Caption = #1578#1575#1585#1610#1582' '#1601#1585#1605
    end
    object edtFormNumber: TDBEdit
      Left = 27
      Top = 7
      Width = 66
      Height = 19
      Ctl3D = False
      DataField = 'FormNumber'
      DataSource = srcForms
      ParentCtl3D = False
      TabOrder = 0
    end
    object edtFormDate: TDBEdit
      Left = 25
      Top = 32
      Width = 66
      Height = 21
      DataField = 'FormDate'
      DataSource = srcForms
      TabOrder = 1
    end
  end
  object aBigPanel: TPanel [3]
    Left = 131
    Top = 449
    Width = 233
    Height = 120
    Align = alCustom
    Anchors = [akLeft, akBottom]
    AutoSize = True
    BevelOuter = bvNone
    Caption = '0'
    Font.Charset = ARABIC_CHARSET
    Font.Color = clGreen
    Font.Height = -64
    Font.Name = 'Badr'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
    object horoofPanel: TPanel
      Left = 0
      Top = 96
      Width = 233
      Height = 24
      Align = alBottom
      BevelOuter = bvNone
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
  end
  inherited ActionList: TActionList
    Left = 167
    Top = 6
    inherited DataSetInsert1: TDataSetInsert
      DataSource = srcForms
    end
    inherited DataSetEdit1: TDataSetEdit
      DataSource = srcForms
    end
    inherited DataSetPost1: TDataSetPost
      DataSource = srcForms
    end
    inherited DataSetCancel1: TDataSetCancel
      ShortCut = 0
      DataSource = srcForms
    end
    inherited actSearch_: TAction
      OnExecute = actSearch_Execute
    end
    inherited DataSetDelete1_: TDataSetDelete
      DataSource = srcForms
    end
    object DataSetItemDelette: TDataSetDelete
      Category = 'Dataset'
      Caption = #1581#1584#1601' '#1585#1583#1610#1601
      DataSource = srcFormItems
    end
    object DataSetInsert2: TDataSetInsert
      Category = 'Dataset'
      Caption = #1580#1583#1610#1583
      DataSource = srcFormItems
    end
    object DataSetPost2: TDataSetPost
      Category = 'Dataset'
      Caption = 'DataSetPost2'
      DataSource = srcFormItems
    end
    object DataSetCancel2: TDataSetCancel
      Category = 'Dataset'
      Caption = 'DataSetCancel2'
      ShortCut = 27
      DataSource = srcFormItems
    end
    object actPrint: TAction
      Caption = #1670#1575#1662
      OnExecute = actPrintExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
    object actSendToExcel: TAction
      Caption = #1575#1585#1587#1575#1604#8207#1576#1607'Excel'
      OnExecute = actSendToExcelExecute
    end
    object actEdit: TAction
      Caption = #1608#1610#1585#1575#1610#1588
    end
    object actDelete1: TAction
    end
    object actselectCheck: TAction
      Caption = #1601#1585#1575#1582#1608#1575#1606#1610' '#1670#1603#1607#1575
      OnExecute = actselectCheckExecute
    end
    object actReportsearch: TAction
      Caption = #1580#1587#1578#1580#1608#1610' '#1711#1586#1575#1585#1588#1575#1578
      OnExecute = actReportsearchExecute
    end
    object actCopyPaste: TAction
      Caption = #1603#1662#1610' '#1587#1591#1585
      ImageIndex = 5
      ShortCut = 16397
      OnExecute = actCopyPasteExecute
    end
    object actSeverable: TAction
      Caption = #1578#1601#1705#1610#1705#8207#1662#1585#1583#1575#1582#1578
      OnExecute = actSeverableExecute
    end
    object actItemNote: TAction
      Caption = #1603#1662#1610' '#1588#1585#1581' '#1607#1575#1610' '#1575#1587#1578#1575#1606#1583#1575#1585#1583
      OnExecute = actItemNoteExecute
    end
    object actGetExcel: TAction
      Caption = #1583#1585#1610#1575#1601#1578'Excel'#8207
      OnExecute = actGetExcelExecute
    end
    object actViewFileF: TAction
      Caption = #1662#1610#1608#1587#1578' '#1608#1575#1587#1603#1606'...'
      OnExecute = actViewFileFExecute
    end
    object actWorkflowHistoryF: TAction
      Caption = #1578#1575#1585#1740#1582#1670#1607' '#1711#1585#1583#1588' '#1705#1575#1585
      ShortCut = 16471
      OnExecute = actWorkflowHistoryFExecute
    end
    object actStateChange: TAction
      Caption = #1578#1594#1740#1740#1585' '#1608#1590#1593#1740#1578
      OnExecute = actStateChangeExecute
    end
    object actWorkflow: TAction
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' '#1711#1585#1583#1588' '#1705#1575#1585
      OnExecute = actWorkflowExecute
    end
    object actMakeDocumentShow: TAction
      Caption = #1589#1583#1585#1608' '#1587#1606#1583
      ShortCut = 16452
      OnExecute = actMakeDocumentShowExecute
    end
    object actRequestCash: TAction
      Caption = #1575#1602#1604#1575#1605' '#1583#1585#1582#1608#1575#1587#1578' '#1608#1580#1607
      OnExecute = actRequestCashExecute
    end
    object actPostOverallType: TAction
      Caption = 'actPostOverallType'
      OnExecute = actPostOverallTypeExecute
    end
    object actCopyForm: TAction
      Caption = #1705#1662#1740' '#1601#1585#1605
      OnExecute = actCopyFormExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 237
    Top = 8
  end
  object qryForms: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    BeforeInsert = qryFormsBeforeInsert
    AfterInsert = qryFormsAfterInsert
    BeforeEdit = qryFormsBeforeEdit
    AfterEdit = qryFormItemsAfterEdit
    BeforePost = qryFormsBeforePost
    AfterPost = qryFormsAfterPost
    BeforeCancel = qryFormsBeforeCancel
    BeforeDelete = qryFormsBeforeDelete
    AfterDelete = qryFormsAfterDelete
    AfterScroll = qryFormsAfterScroll
    Parameters = <
      item
        Name = 'FormType'
        DataType = ftWord
        Precision = 3
        Size = 1
        Value = 2
      end
      item
        Name = 'YearID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT *'
      'FROM         Forms'
      'where FormType=:FormType'
      'and ( YearID = :YearID )'
      ''
      'ORDER BY  FormDate,FormNumber')
    Left = 644
    Top = 13
    object qryFormsOperatorID: TIntegerField
      FieldName = 'OperatorID'
    end
    object qryFormsFormID: TIntegerField
      FieldName = 'FormID'
    end
    object qryFormsFormType: TWordField
      FieldName = 'FormType'
    end
    object qryFormsFormNumber: TIntegerField
      FieldName = 'FormNumber'
    end
    object qryFormsFormDate: TStringField
      DisplayLabel = #1578#1575#1585#1610#1582' '#1601#1585#1605
      FieldName = 'FormDate'
      EditMask = '!9999/99/00;1;_'
      FixedChar = True
      Size = 10
    end
    object qryFormsCustomerID1: TIntegerField
      FieldName = 'CustomerID1'
      Required = True
      OnChange = qryFormsCustomerID1Change
    end
    object qryFormsCustomerID2: TIntegerField
      FieldName = 'CustomerID2'
      OnChange = qryFormsCustomerID2Change
    end
    object qryFormsAmount: TBCDField
      FieldName = 'Amount'
      Precision = 19
    end
    object qryFormsBudgetCode: TIntegerField
      FieldName = 'BudgetCode'
    end
    object qryFormsProjectID: TIntegerField
      FieldName = 'ProjectID'
    end
    object qryFormsServerID: TIntegerField
      FieldName = 'ServerID'
    end
    object qryFormsYearID: TIntegerField
      FieldName = 'YearID'
    end
    object qryFormsSellsEmporium: TIntegerField
      FieldName = 'SellsEmporium'
      OnChange = qryFormsSellsEmporiumChange
    end
    object qryFormsSellsMethod: TIntegerField
      FieldName = 'SellsMethod'
    end
    object qryFormsFomNote: TStringField
      FieldName = 'FomNote'
      Size = 255
    end
    object qryFormsAidInfoNo: TStringField
      FieldName = 'AidInfoNo'
      Size = 40
    end
    object qryFormsAidInfoDate: TStringField
      FieldName = 'AidInfoDate'
      EditMask = '!9999/99/00;1;_'
      FixedChar = True
      Size = 10
    end
    object qryFormsCustomerName2: TStringField
      DisplayLabel = #1605#1588#1582#1589#1575#1578' '#1583#1585#1610#1575#1601#1578' '#1603#1606#1606#1583#1607
      FieldName = 'CustomerName2'
      Size = 100
    end
    object qryForms_BudgetName: TStringField
      FieldKind = fkLookup
      FieldName = '_BudgetName'
      LookupDataSet = DmF.qryAccBudget
      LookupKeyFields = 'BudgetTopicID'
      LookupResultField = 'BudgetCaption_L1'
      KeyFields = 'BudgetCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_ProjectName: TStringField
      FieldKind = fkLookup
      FieldName = '_ProjectName'
      LookupDataSet = DmF.qryAccProject
      LookupKeyFields = 'ProjectID'
      LookupResultField = 'ProjectCaption_L1'
      KeyFields = 'ProjectID'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_CustomerName: TStringField
      FieldKind = fkLookup
      FieldName = '_CustomerName'
      LookupDataSet = qryCustomer1
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustName'
      KeyFields = 'CustomerID1'
      LookupCache = True
      Required = True
      Size = 150
      Lookup = True
    end
    object qryFormsDocNo: TIntegerField
      FieldName = 'DocNo'
    end
    object qryFormsDocDate: TStringField
      FieldName = 'DocDate'
      FixedChar = True
      Size = 10
    end
    object qryFormsModifyDate: TDateTimeField
      FieldName = 'ModifyDate'
    end
    object qryForms_CustomerName2: TStringField
      FieldKind = fkLookup
      FieldName = '_CustomerName2'
      LookupDataSet = qryCustomer2
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustName'
      KeyFields = 'CustomerID2'
      Size = 150
      Lookup = True
    end
    object qryFormsFormState: TWordField
      FieldName = 'FormState'
      OnGetText = qryFormsFormStateGetText
    end
    object qryFormsFirstUser: TStringField
      FieldName = 'FirstUser'
      Size = 50
    end
    object qryFormsCustomerID3: TIntegerField
      FieldName = 'CustomerID3'
      OnChange = qryFormsCustomerID3Change
    end
    object qryForms_CustomerID3: TStringField
      FieldKind = fkLookup
      FieldName = '_CustomerID3'
      LookupDataSet = qryCustomer3
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustName'
      KeyFields = 'CustomerID3'
      Size = 150
      Lookup = True
    end
    object qryFormsLastUser: TStringField
      FieldName = 'LastUser'
      Size = 50
    end
    object qryFormsRelatedRecipts: TStringField
      FieldName = 'RelatedRecipts'
      FixedChar = True
      Size = 27
    end
    object qryForms_RelatedRecipts: TStringField
      FieldKind = fkLookup
      FieldName = '_RelatedRecipts'
      LookupDataSet = qryRelatedRecipts
      LookupKeyFields = 'RelatedNo'
      LookupResultField = 'RelatedName'
      KeyFields = 'RelatedRecipts'
      FixedChar = True
      Size = 556
      Lookup = True
    end
    object qryFormsTopicCode: TLargeintField
      Tag = 3
      DisplayLabel = #1603#1583' '#1581#1587#1575#1576#1583#1575#1585#1610' '
      FieldName = 'TopicCode'
    end
    object qryFormsDetailCode: TStringField
      FieldName = 'DetailCode'
      OnChange = qryFormsTopicCodeChange
      EditMask = '999999999999'
      Size = 12
    end
    object qryFormsCTopicCode: TStringField
      FieldName = 'CTopicCode'
      OnChange = qryFormsTopicCodeChange
      EditMask = '999999999999'
      Size = 12
    end
    object qryFormsCTopicCode2: TStringField
      FieldName = 'CTopicCode2'
      OnChange = qryFormsTopicCodeChange
      EditMask = '999999999999'
      Size = 12
    end
    object qryFormsCTopicCode3: TStringField
      FieldName = 'CTopicCode3'
      OnChange = qryFormsTopicCodeChange
      EditMask = '999999999999'
      Size = 12
    end
    object qryForms_TopicCodeName: TStringField
      DisplayLabel = #1581#1587#1575#1576
      FieldKind = fkLookup
      FieldName = '_TopicCodeName_L1'
      LookupDataSet = DmF.qryAccTopicCode
      LookupKeyFields = 'TopicCode'
      LookupResultField = 'MoeenName'
      KeyFields = 'TopicCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_DetailsName: TStringField
      DisplayLabel = #1578#1601#1589#1610#1604#1610
      FieldKind = fkLookup
      FieldName = '_DetailsName_L1'
      LookupDataSet = DmF.qryAccDetailCode
      LookupKeyFields = 'DetailCode'
      LookupResultField = 'DetailName'
      KeyFields = 'DetailCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_CTopicName: TStringField
      DisplayLabel = #1605#1585#1603#1586#1607#1586#1610#1606#1607
      FieldKind = fkLookup
      FieldName = '_CTopicName_L1'
      LookupDataSet = DmF.qryAccCenterTopics
      LookupKeyFields = 'CTopicCode'
      LookupResultField = 'CTopicName'
      KeyFields = 'CTopicCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_CTopicName2_L1: TStringField
      DisplayLabel = #1578#1601#1589#1740#1604#1740'3'
      FieldKind = fkLookup
      FieldName = '_CTopicName2_L1'
      LookupDataSet = DmF.qryAccCenterTopics2
      LookupKeyFields = 'CTopicCode2'
      LookupResultField = 'CTopicName2'
      KeyFields = 'CTopicCode2'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryForms_CTopicName3: TStringField
      DisplayLabel = #1605#1585#1603#1586#1607#1586#1610#1606#1607'3'
      FieldKind = fkLookup
      FieldName = '_CTopicName3_L1'
      LookupDataSet = DmF.qryAccCenterTopics3
      LookupKeyFields = 'CTopicCode3'
      LookupResultField = 'CTopicName3'
      KeyFields = 'CTopicCode3'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormsReciptID: TIntegerField
      FieldName = 'ReciptID'
    end
    object qryFormsPayTypes: TIntegerField
      FieldName = 'PayTypes'
      OnChange = qryFormsPayTypesChange
      OnGetText = CmbALLGetText
      OnSetText = CmbALLSetText
    end
    object qryFormsTopicTypes: TIntegerField
      FieldName = 'TopicTypes'
      OnGetText = CmbALLGetText
      OnSetText = CmbALLSetText
    end
    object qryFormsArzRate: TFloatField
      FieldName = 'ArzRate'
    end
    object qryFormsArzAmount: TBCDField
      FieldName = 'ArzAmount'
      OnChange = qryFormsArzAmountChange
      Precision = 19
    end
    object qryFormsArzTypeID: TIntegerField
      FieldName = 'ArzTypeID'
      OnChange = qryFormsArzTypeIDChange
      OnGetText = CmbALLGetText
      OnSetText = CmbALLSetText
    end
    object qryForms_AccountKind: TIntegerField
      FieldKind = fkLookup
      FieldName = '_AccountKind'
      LookupDataSet = qryCustomer1
      LookupKeyFields = 'CustID'
      LookupResultField = 'AccountKind'
      KeyFields = 'CustomerID1'
      Lookup = True
    end
    object qryFormsArzAmountExchange: TBCDField
      FieldName = 'ArzAmountExchange'
      currency = True
      Precision = 19
    end
    object qryFormsArzRateExchange: TFloatField
      FieldName = 'ArzRateExchange'
    end
    object qryFormsArzTypeIDExchange: TIntegerField
      FieldName = 'ArzTypeIDExchange'
      OnChange = qryFormsArzTypeIDChange
      OnGetText = CmbALLGetText
      OnSetText = CmbALLSetText
    end
    object qryFormsID: TLargeintField
      FieldName = 'ID'
      ReadOnly = True
    end
    object qryFormsPO: TStringField
      FieldName = 'PO'
      Size = 30
    end
    object qryForms_CustomerNote1: TStringField
      FieldKind = fkLookup
      FieldName = '_CustomerNote1'
      LookupDataSet = qryCustomer1
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustomerNote'
      KeyFields = 'CustomerID1'
      Size = 100
      Lookup = True
    end
    object qryForms_CustomerNote2: TStringField
      FieldKind = fkLookup
      FieldName = '_CustomerNote2'
      LookupDataSet = qryCustomer2
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustomerNote'
      KeyFields = 'CustomerID2'
      Size = 100
      Lookup = True
    end
  end
  object srcForms: TDataSource
    AutoEdit = False
    DataSet = qryForms
    OnStateChange = srcFormsStateChange
    Left = 697
    Top = 14
  end
  object qryInitForm: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'Type'
        DataType = ftWord
        Precision = 3
        Size = 1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT        FormTypes.*, FormTypes_1.FormCaption AS _PostOvera' +
        'llTypeCaption'
      'FROM            FormTypes LEFT OUTER JOIN'
      
        '                         FormTypes AS FormTypes_1 ON FormTypes.P' +
        'ostOverallType = FormTypes_1.FormType'
      'WHERE        (FormTypes.FormType = :Type )')
    Left = 573
    Top = 6
  end
  object srcFormItems: TDataSource
    DataSet = qryFormItems
    Left = 151
    Top = 200
  end
  object ppReport1: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline1
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.PaperName = 'Letter'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 279000
    PrinterSetup.mmPaperWidth = 216000
    PrinterSetup.PaperSize = 1
    Template.FileName = 'D:\Projects\Fara\BSell\Exe\Report\rptPaymentCash.rtm'
    Units = utMillimeters
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'PDF'
    EmailSettings.ReportFormat = 'PDF'
    EmailSettings.ConnectionSettings.MailService = 'SMTP'
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RedirectURI = 'http://localhost'
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RedirectPort = 0
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RefreshTokenLifeSpan = 365
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RedirectURI = 'http://localhost'
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RedirectPort = 0
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RefreshTokenLifeSpan = 365
    EmailSettings.ConnectionSettings.EnableMultiPlugin = False
    EmailSettings.ConnectionSettings.ConnectionStatusInfo = [csiStatusBar]
    LanguageID = 'Default'
    OpenFile = False
    OutlineSettings.CreateNode = False
    OutlineSettings.CreatePageNodes = False
    OutlineSettings.Enabled = False
    OutlineSettings.Visible = False
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    ThumbnailSettings.ThumbnailSize = tsSmall
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.DigitalSignatureSettings.SignPDF = False
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PDFSettings.Layers = True
    PDFSettings.Outline = True
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    RTFSettings.AppName = 'ReportBuilder'
    RTFSettings.Author = 'ReportBuilder'
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    RTFSettings.Title = 'Report'
    TextFileName = '($MyDocuments)\Report.pdf'
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = False
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    CloudDriveSettings.DropBoxSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.DropBoxSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.DropBoxSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.DropBoxSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.DropBoxSettings.DirectorySupport = True
    CloudDriveSettings.DropBoxSettings.SharedResources = True
    CloudDriveSettings.GoogleDriveSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.GoogleDriveSettings.DirectorySupport = False
    CloudDriveSettings.GoogleDriveSettings.SharedResources = False
    CloudDriveSettings.OneDriveSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.OneDriveSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.OneDriveSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.OneDriveSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.OneDriveSettings.DirectorySupport = True
    CloudDriveSettings.OneDriveSettings.SharedResources = True
    Left = 547
    Top = 296
    Version = '23.0'
    mmColumnWidth = 0
    DataPipelineName = 'ppDBPipeline1'
    object ppHeaderBand1: TppHeaderBand
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 28046
      mmPrintPosition = 0
      object ppShape1: TppShape
        DesignLayer = ppDesignLayer2
        UserName = 'Shape1'
        mmHeight = 7938
        mmLeft = 0
        mmTop = 20108
        mmWidth = 203465
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel6: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label6'
        Border.mmPadding = 0
        Caption = #1578#1575#1585#1610#1582
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 181769
        mmTop = 21696
        mmWidth = 10583
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel7: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label7'
        Border.mmPadding = 0
        Caption = #1588#1585#1581
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 103452
        mmTop = 21696
        mmWidth = 60061
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel3: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label3'
        Border.mmPadding = 0
        Caption = #1585#1583#1610#1601
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 194469
        mmTop = 21696
        mmWidth = 8202
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel14: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label14'
        AutoSize = False
        Border.mmPadding = 0
        Caption = #1605#1576#1604#1594' '#1662#1585#1583#1575#1582#1578#1610' '#1585#1610#1575#1604
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 64294
        mmTop = 21696
        mmWidth = 37042
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel16: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label16'
        AutoSize = False
        Border.mmPadding = 0
        Caption = #1603#1583' '#1581#1587#1575#1576' '#1583#1585#1570#1605#1583
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5376
        mmLeft = 43392
        mmTop = 21696
        mmWidth = 19315
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel17: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label17'
        AutoSize = False
        Border.mmPadding = 0
        Caption = #1603#1583' '#1578#1601#1589#1610#1604#1610
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5376
        mmLeft = 21167
        mmTop = 21696
        mmWidth = 20902
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel18: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label18'
        AutoSize = False
        Border.mmPadding = 0
        Caption = #1603#1583' '#1578#1601#1589#1610#1604#1610'2'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 794
        mmTop = 21696
        mmWidth = 18785
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel19: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label19'
        Border.mmPadding = 0
        Caption = #1588#1605#1575#1585#1607
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Yagut'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 5292
        mmLeft = 165629
        mmTop = 21696
        mmWidth = 13758
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel2: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label2'
        OnGetText = ppLabel2GetText
        Border.mmPadding = 0
        Caption = #1588#1585#1603#1578' '#1591#1585#1581' '#1608' '#1662#1585#1583#1575#1586#1588' '#1601#1585#1575' '#1585#1575#1610#1575#1606#1607
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Jadid'
        Font.Size = 12
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 6350
        mmLeft = 77523
        mmTop = 2910
        mmWidth = 52123
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel10: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label10'
        Border.mmPadding = 0
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Zar'
        Font.Size = 14
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 8128
        mmLeft = 89165
        mmTop = 9525
        mmWidth = 28956
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel11: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label101'
        OnGetText = ppLabel11GetText
        Border.mmPadding = 0
        Caption = #1578#1575#1585#1610#1582' '#1670#1575#1662
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 6350
        mmLeft = 2646
        mmTop = 6085
        mmWidth = 13758
        BandType = 0
        LayerName = Foreground1
      end
      object ppSystemVariable1: TppSystemVariable
        DesignLayer = ppDesignLayer2
        UserName = 'SystemVariable1'
        OnGetText = ppSystemVariable1GetText
        Border.mmPadding = 0
        VarType = vtPageSet
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        mmHeight = 5821
        mmLeft = 2646
        mmTop = 12965
        mmWidth = 6879
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel12: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label12'
        Border.mmPadding = 0
        Caption = #1588#1605#1575#1585#1607' '#1601#1585#1605
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Zar'
        Font.Size = 10
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 5821
        mmLeft = 188119
        mmTop = 13229
        mmWidth = 14288
        BandType = 0
        LayerName = Foreground1
      end
      object ppDBText6: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText6'
        Border.mmPadding = 0
        DataField = 'FormNumber'
        DataPipeline = ppDBPipeline1
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 6435
        mmLeft = 165629
        mmTop = 13229
        mmWidth = 21960
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine3: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line3'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 63236
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine7: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line7'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 20373
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine8: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line8'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 42598
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine9: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line9'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 192882
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine10: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line10'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 179917
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine11: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line11'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 163777
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine12: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line12'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 8202
        mmLeft = 101600
        mmTop = 20108
        mmWidth = 3440
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel4: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label4'
        HyperLink = 'CustomerID1'
        HyperlinkColor = clBlack
        OnGetText = ppLblReMainGetText
        Border.mmPadding = 0
        Caption = #1605#1575#1606#1583#1607'CustomerID1'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Times New Roman'
        Font.Size = 18
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7143
        mmLeft = 29369
        mmTop = 5292
        mmWidth = 43921
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel5: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label5'
        HyperLink = 'CustomerID2'
        HyperlinkColor = clBlack
        OnGetText = ppLblReMainGetText
        Border.mmPadding = 0
        Caption = #1605#1575#1606#1583#1607' CustomerID2'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Times New Roman'
        Font.Size = 18
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7144
        mmLeft = 154517
        mmTop = 2910
        mmWidth = 45508
        BandType = 0
        LayerName = Foreground1
      end
      object ppDBCalcnum2alphabet: TppDBCalc
        DesignLayer = ppDesignLayer2
        UserName = 'DBCalcnum2alphabet'
        OnGetText = ppDBCalcnum2alphabetGetText
        Border.mmPadding = 0
        DataPipeline = ppDBPipeline2
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 5821
        mmLeft = 131234
        mmTop = 10848
        mmWidth = 17198
        BandType = 0
        LayerName = Foreground1
      end
      object plblRecordCount: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'plblRecordCount'
        OnGetText = plblRecordCountGetText
        Border.mmPadding = 0
        Caption = #1578#1593#1583#1575#1583
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 5821
        mmLeft = 140229
        mmTop = 2910
        mmWidth = 4498
        BandType = 0
        LayerName = Foreground1
      end
    end
    object ppDetailBand1: TppDetailBand
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 5027
      mmPrintPosition = 0
    end
    object ppFooterBand1: TppFooterBand
      Border.mmPadding = 0
      PrintOnLastPage = False
      mmBottomOffset = 0
      mmHeight = 11377
      mmPrintPosition = 0
      object plblAmount1Total_Price1: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'plblRemain1'
        OnGetText = plblRemainGetText
        Border.mmPadding = 0
        Caption = 'plblRemain'#1605#1575#1606#1583#1607' '#1583#1601#1578#1585
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Times New Roman'
        Font.Size = 18
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 7408
        mmLeft = 70115
        mmTop = 2910
        mmWidth = 49213
        BandType = 8
        LayerName = Foreground1
      end
      object plblCmbTopicTypes: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label15'
        OnGetText = plblCmbTopicTypesGetText
        Border.mmPadding = 0
        Caption = #1605#1608#1590#1608#1593' '#1583#1585#1610#1575#1601#1578'/'#1662#1585#1583#1575#1582#1578
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 3704
        mmLeft = 123031
        mmTop = 7673
        mmWidth = 25400
        BandType = 8
        LayerName = Foreground1
      end
      object plblCmbPayTypes: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'plblCmbPayTypes'
        OnGetText = plblCmbPayTypesGetText
        Border.mmPadding = 0
        Caption = #1606#1608#1593' '#1583#1585#1610#1575#1601#1578'/'#1662#1585#1583#1575#1582#1578
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 3440
        mmLeft = 152665
        mmTop = 1588
        mmWidth = 20902
        BandType = 8
        LayerName = Foreground1
      end
      object plblArz: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'plblCmbPayTypes1'
        OnGetText = plblArzGetText
        Border.mmPadding = 0
        Caption = #1606#1608#1593' '#1575#1585#1586
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        Transparent = True
        mmHeight = 5821
        mmLeft = 179917
        mmTop = 1852
        mmWidth = 6879
        BandType = 8
        LayerName = Foreground1
      end
    end
    object ppDesignLayers2: TppDesignLayers
      object ppDesignLayer2: TppDesignLayer
        UserName = 'Foreground1'
        LayerType = ltBanded
        Index = 0
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
  object ppDBPipeline1: TppDBPipeline
    DataSource = srcForms
    OpenDataSource = False
    RangeEnd = reCurrentRecord
    RangeBegin = rbCurrentRecord
    UserName = 'DBPipeline1'
    Left = 415
    Top = 248
    object ppDBPipeline1ppField1: TppField
      FieldAlias = 'OperatorID'
      FieldName = 'OperatorID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField2: TppField
      FieldAlias = 'FormID'
      FieldName = 'FormID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField3: TppField
      FieldAlias = 'FormType'
      FieldName = 'FormType'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField4: TppField
      FieldAlias = 'FormNumber'
      FieldName = 'FormNumber'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 3
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField5: TppField
      FieldAlias = 'FormDate'
      FieldName = 'FormDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 4
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField6: TppField
      FieldAlias = 'CustomerID1'
      FieldName = 'CustomerID1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 5
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField7: TppField
      FieldAlias = 'CustomerID2'
      FieldName = 'CustomerID2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 6
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField8: TppField
      FieldAlias = 'Amount'
      FieldName = 'Amount'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 7
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField9: TppField
      FieldAlias = 'BudgetCode'
      FieldName = 'BudgetCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 8
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField10: TppField
      FieldAlias = 'ProjectID'
      FieldName = 'ProjectID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 9
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField11: TppField
      FieldAlias = 'ServerID'
      FieldName = 'ServerID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 10
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField12: TppField
      FieldAlias = 'YearID'
      FieldName = 'YearID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 11
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField13: TppField
      FieldAlias = 'SellsEmporium'
      FieldName = 'SellsEmporium'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 12
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField14: TppField
      FieldAlias = 'SellsMethod'
      FieldName = 'SellsMethod'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 13
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField15: TppField
      FieldAlias = 'FomNote'
      FieldName = 'FomNote'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 14
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField16: TppField
      FieldAlias = 'AidInfoNo'
      FieldName = 'AidInfoNo'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 15
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField17: TppField
      FieldAlias = 'AidInfoDate'
      FieldName = 'AidInfoDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 16
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField18: TppField
      FieldAlias = 'CustomerName2'
      FieldName = 'CustomerName2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 17
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField19: TppField
      FieldAlias = '_BudgetName'
      FieldName = '_BudgetName'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 18
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField20: TppField
      FieldAlias = '_ProjectName'
      FieldName = '_ProjectName'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 19
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField21: TppField
      FieldAlias = '_CustomerName'
      FieldName = '_CustomerName'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 20
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField22: TppField
      FieldAlias = 'DocNo'
      FieldName = 'DocNo'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 21
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField23: TppField
      FieldAlias = 'DocDate'
      FieldName = 'DocDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 22
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField24: TppField
      FieldAlias = 'ModifyDate'
      FieldName = 'ModifyDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 23
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField25: TppField
      FieldAlias = '_CustomerName2'
      FieldName = '_CustomerName2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 24
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField26: TppField
      FieldAlias = 'FormState'
      FieldName = 'FormState'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 25
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField27: TppField
      FieldAlias = 'FirstUser'
      FieldName = 'FirstUser'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 26
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField28: TppField
      FieldAlias = 'CustomerID3'
      FieldName = 'CustomerID3'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 27
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField29: TppField
      FieldAlias = '_CustomerID3'
      FieldName = '_CustomerID3'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 28
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField30: TppField
      FieldAlias = 'LastUser'
      FieldName = 'LastUser'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 29
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField31: TppField
      FieldAlias = 'RelatedRecipts'
      FieldName = 'RelatedRecipts'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 30
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField32: TppField
      FieldAlias = '_RelatedRecipts'
      FieldName = '_RelatedRecipts'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 31
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField33: TppField
      FieldAlias = 'TopicCode'
      FieldName = 'TopicCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 32
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField34: TppField
      FieldAlias = 'DetailCode'
      FieldName = 'DetailCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 33
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField35: TppField
      FieldAlias = 'CTopicCode'
      FieldName = 'CTopicCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 34
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField36: TppField
      FieldAlias = 'CTopicCode2'
      FieldName = 'CTopicCode2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 35
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField37: TppField
      FieldAlias = 'CTopicCode3'
      FieldName = 'CTopicCode3'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 36
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField38: TppField
      FieldAlias = '_TopicCodeName_L1'
      FieldName = '_TopicCodeName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 37
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField39: TppField
      FieldAlias = '_DetailsName_L1'
      FieldName = '_DetailsName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 38
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField40: TppField
      FieldAlias = '_CTopicName_L1'
      FieldName = '_CTopicName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 39
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField41: TppField
      FieldAlias = '_CTopicName2_L1'
      FieldName = '_CTopicName2_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 40
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField42: TppField
      FieldAlias = '_CTopicName3_L1'
      FieldName = '_CTopicName3_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 41
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField43: TppField
      FieldAlias = 'ReciptID'
      FieldName = 'ReciptID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 42
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField44: TppField
      FieldAlias = 'PayTypes'
      FieldName = 'PayTypes'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 43
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField45: TppField
      FieldAlias = 'TopicTypes'
      FieldName = 'TopicTypes'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 44
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField46: TppField
      FieldAlias = 'ArzRate'
      FieldName = 'ArzRate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 45
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField47: TppField
      FieldAlias = 'ArzAmount'
      FieldName = 'ArzAmount'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 46
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField48: TppField
      FieldAlias = 'ArzTypeID'
      FieldName = 'ArzTypeID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 47
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField49: TppField
      FieldAlias = '_AccountKind'
      FieldName = '_AccountKind'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 48
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField50: TppField
      FieldAlias = 'ArzAmountExchange'
      FieldName = 'ArzAmountExchange'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 49
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField51: TppField
      FieldAlias = 'ArzRateExchange'
      FieldName = 'ArzRateExchange'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 50
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField52: TppField
      FieldAlias = 'ArzTypeIDExchange'
      FieldName = 'ArzTypeIDExchange'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 51
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField53: TppField
      FieldAlias = 'ID'
      FieldName = 'ID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 52
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField54: TppField
      FieldAlias = 'PO'
      FieldName = 'PO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 53
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField55: TppField
      FieldAlias = '_CustomerNote1'
      FieldName = '_CustomerNote1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 54
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline1ppField56: TppField
      FieldAlias = '_CustomerNote2'
      FieldName = '_CustomerNote2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 55
      Searchable = False
      Sortable = False
    end
  end
  object ppDBPipeline2: TppDBPipeline
    DataSource = srcFormItems
    OpenDataSource = False
    UserName = 'DBPipeline2'
    Left = 445
    Top = 312
    MasterDataPipelineName = 'ppDBPipeline1'
    object ppDBPipeline2ppField1: TppField
      FieldAlias = 'Row'
      FieldName = 'Row'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField2: TppField
      FieldAlias = 'CheckDate'
      FieldName = 'CheckDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField3: TppField
      FieldAlias = 'CheckNumber'
      FieldName = 'CheckNumber'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField4: TppField
      FieldAlias = 'CustomerID2'
      FieldName = 'CustomerID2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 3
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField5: TppField
      FieldAlias = '_Cudtomername2'
      FieldName = '_Cudtomername2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 4
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField6: TppField
      FieldAlias = 'ItemNote'
      FieldName = 'ItemNote'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 5
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField7: TppField
      FieldAlias = 'ItemAmount'
      FieldName = 'ItemAmount'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 6
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField8: TppField
      FieldAlias = 'VatCo'
      FieldName = 'VatCo'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 7
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField9: TppField
      FieldAlias = 'VAT'
      FieldName = 'VAT'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 8
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField10: TppField
      FieldAlias = 'ItemAmount_VAT'
      FieldName = 'ItemAmount_VAT'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 9
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField11: TppField
      FieldAlias = 'SeverableAmount'
      FieldName = 'SeverableAmount'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 10
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField12: TppField
      FieldAlias = 'ItemState'
      FieldName = 'ItemState'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 11
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField13: TppField
      FieldAlias = 'ItemStateComment'
      FieldName = 'ItemStateComment'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 12
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField14: TppField
      FieldAlias = 'CashWage'
      FieldName = 'CashWage'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 13
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField15: TppField
      FieldAlias = 'TopicCode'
      FieldName = 'TopicCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 14
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField16: TppField
      FieldAlias = 'DetailCode'
      FieldName = 'DetailCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 15
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField17: TppField
      FieldAlias = 'CTopicCode'
      FieldName = 'CTopicCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 16
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField18: TppField
      FieldAlias = 'CTopicCode2'
      FieldName = 'CTopicCode2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 17
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField19: TppField
      FieldAlias = 'CTopicCode3'
      FieldName = 'CTopicCode3'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 18
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField20: TppField
      FieldAlias = '_BudgetCode'
      FieldName = '_BudgetCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 19
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField21: TppField
      FieldAlias = 'BudgetCode'
      FieldName = 'BudgetCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 20
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField22: TppField
      FieldAlias = 'AidInfoNo'
      FieldName = 'AidInfoNo'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 21
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField23: TppField
      FieldAlias = 'AidInfoDate'
      FieldName = 'AidInfoDate'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 22
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField24: TppField
      FieldAlias = 'ProductCode'
      FieldName = 'ProductCode'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 23
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField25: TppField
      FieldAlias = 'FormItemID'
      FieldName = 'FormItemID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 24
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField26: TppField
      FieldAlias = 'FormID'
      FieldName = 'FormID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 25
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField27: TppField
      FieldAlias = 'YearID'
      FieldName = 'YearID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 26
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField28: TppField
      FieldAlias = 'ServerID'
      FieldName = 'ServerID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 27
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField29: TppField
      FieldAlias = 'BankName'
      FieldName = 'BankName'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 28
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField30: TppField
      FieldAlias = 'AccountNumber'
      FieldName = 'AccountNumber'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 29
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField31: TppField
      FieldAlias = 'City'
      FieldName = 'City'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 30
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField32: TppField
      FieldAlias = 'ProjectID'
      FieldName = 'ProjectID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 31
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField33: TppField
      FieldAlias = 'preFormItemID'
      FieldName = 'preFormItemID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 32
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField34: TppField
      FieldAlias = '_CTopicName2_L1'
      FieldName = '_CTopicName2_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 33
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField35: TppField
      FieldAlias = 'CheckType'
      FieldName = 'CheckType'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 34
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField36: TppField
      FieldAlias = '_CTopicName3_L1'
      FieldName = '_CTopicName3_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 35
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField37: TppField
      FieldAlias = '_CTopicName_L1'
      FieldName = '_CTopicName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 36
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField38: TppField
      FieldAlias = '_DetailsName_L1'
      FieldName = '_DetailsName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 37
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField39: TppField
      FieldAlias = '_TopicCodeName_L1'
      FieldName = '_TopicCodeName_L1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 38
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField40: TppField
      FieldAlias = '_Customer2Active'
      FieldName = '_Customer2Active'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 39
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField41: TppField
      FieldAlias = '_AccountNumber'
      FieldName = '_AccountNumber'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 40
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField42: TppField
      FieldAlias = '_BankId'
      FieldName = '_BankId'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 41
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField43: TppField
      FieldAlias = 'FirstUser'
      FieldName = 'FirstUser'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 42
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField44: TppField
      FieldAlias = 'LastUser'
      FieldName = 'LastUser'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 43
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField45: TppField
      FieldAlias = 'AccountNumberNew'
      FieldName = 'AccountNumberNew'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 44
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField46: TppField
      FieldAlias = 'ReciptItems_ID'
      FieldName = 'ReciptItems_ID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 45
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField47: TppField
      FieldAlias = '_ReciptItems_ID'
      FieldName = '_ReciptItems_ID'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 46
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField48: TppField
      FieldAlias = 'PO'
      FieldName = 'PO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 47
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField49: TppField
      FieldAlias = 'ArzAmount'
      FieldName = 'ArzAmount'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 48
      Searchable = False
      Sortable = False
    end
    object ppDBPipeline2ppField50: TppField
      FieldAlias = 'CheckFor'
      FieldName = 'CheckFor'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 49
      Searchable = False
      Sortable = False
    end
  end
  object qryFormItems: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterOpen = qryFormItemsAfterOpen
    BeforeInsert = qryFormItemsBeforeInsert
    AfterInsert = qryFormItemsAfterInsert
    BeforeEdit = qryFormItemsBeforeEdit
    AfterEdit = qryFormItemsAfterEdit
    BeforePost = qryFormItemsBeforePost
    AfterPost = qryFormItemsAfterPost
    BeforeDelete = qryFormItemsBeforeDelete
    AfterDelete = qryFormItemsAfterDelete
    AfterScroll = qryFormItemsAfterScroll
    Parameters = <
      item
        Name = 'FormID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 690
      end
      item
        Name = 'YearID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'ServerID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT     *, ItemAmount + VAT AS ItemAmount_VAT'
      'FROM           FormItems'
      'where (FormID=:FormID)'
      'and ( YearID = :YearID )'
      'and ( ServerID = :ServerID )')
    Left = 321
    Top = 6
    object qryFormItemsRow: TIntegerField
      Tag = 3
      DisplayLabel = #1585#1583#1610#1601
      FieldName = 'Row'
    end
    object qryFormItemsCheckDate: TStringField
      Tag = 3
      DisplayLabel = #1578#1575#1585#1610#1582
      FieldName = 'CheckDate'
      ImportedConstraint = '0'
      EditMask = '!9999/99/00;1;_'
      Size = 10
    end
    object qryFormItemsCheckNumber: TStringField
      Tag = 3
      DisplayLabel = #1588#1605#1575#1585#1607' '#1670#1603
      FieldName = 'CheckNumber'
      ImportedConstraint = '0'
      OnChange = qryFormItemsCheckNumberChange
      EditMask = '!999999999999999;0;_'
      Size = 20
    end
    object qryFormItemsCustomerID2: TIntegerField
      Tag = 3
      DisplayLabel = #1603#1583' '#1583#1585#1610#1575#1601#1578' '#1603#1606#1606#1583#1607
      FieldName = 'CustomerID2'
    end
    object qryFormItems_Cudtomername2: TStringField
      DisplayLabel = #1605#1588#1582#1589#1575#1578' '#1583#1585#1610#1575#1601#1578' '#1603#1606#1606#1583#1607
      FieldKind = fkLookup
      FieldName = '_Cudtomername2'
      LookupDataSet = qryCustomer2Detail
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustName'
      KeyFields = 'CustomerID2'
      LookupCache = True
      Size = 60
      Lookup = True
    end
    object qryFormItemsItemNote: TStringField
      Tag = 3
      DisplayLabel = #1588#1585#1581
      FieldName = 'ItemNote'
      Size = 255
    end
    object qryFormItemsItemAmount: TBCDField
      Tag = 3
      DisplayLabel = #1605#1576#1604#1594
      FieldName = 'ItemAmount'
      Required = True
      OnChange = qryFormItemsItemAmountChange
      currency = True
      Precision = 19
    end
    object qryFormItemsVatCo: TFloatField
      DisplayLabel = #1583#1585#1589#1583' '#1575#1585#1586#1588' '#1575#1601#1586#1608#1583#1607
      FieldName = 'VatCo'
      OnChange = qryFormItemsVatCoChange
    end
    object qryFormItemsVAT: TBCDField
      Tag = 3
      DisplayLabel = #1575#1585#1586#1588' '#1575#1601#1586#1608#1583#1607
      FieldName = 'VAT'
      currency = True
      Precision = 19
    end
    object qryFormItemsItemAmount_VAT: TBCDField
      DisplayLabel = #1605#1576#1604#1594' '#1576#1575' '#1575#1585#1586#1588' '#1575#1601#1586#1608#1583#1607
      FieldName = 'ItemAmount_VAT'
      ReadOnly = True
      currency = True
      Precision = 19
    end
    object qryFormItemsSeverableAmount: TWordField
      DisplayLabel = #1605#1576#1604#1594' '#1578#1601#1603#1610#1603#1610
      FieldName = 'SeverableAmount'
      OnGetText = AllPickListGetText
      OnSetText = AllPickListSetText
    end
    object qryFormItemsItemState: TWordField
      DisplayLabel = #1606#1608#1593' '#1578#1575#1611#1610#1610#1583
      FieldName = 'ItemState'
      OnChange = qryFormItemsItemStateChange
      OnGetText = AllPickListGetText
      OnSetText = AllPickListSetText
    end
    object qryFormItemsItemStateComment: TWideStringField
      DisplayLabel = #1588#1585#1581' '#1593#1583#1605' '#1578#1575#1610#1610#1583
      FieldName = 'ItemStateComment'
      Size = 200
    end
    object qryFormItemsCashWage: TBCDField
      DisplayLabel = #1705#1575#1585#1605#1586#1583
      FieldName = 'CashWage'
      currency = True
      Precision = 19
    end
    object qryFormItemsTopicCode: TLargeintField
      Tag = 3
      DisplayLabel = #1603#1583' '#1581#1587#1575#1576#1583#1575#1585#1610' '
      FieldName = 'TopicCode'
    end
    object qryFormItemsDetailCode: TStringField
      Tag = 3
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610' '#1581#1587#1575#1576#1583#1575#1585#1610
      FieldName = 'DetailCode'
      OnChange = qryFormItemsCheckNumberChange
      FixedChar = True
      Size = 12
    end
    object qryFormItemsCTopicCode: TStringField
      Tag = 3
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610'2'
      FieldName = 'CTopicCode'
      OnChange = qryFormItemsCheckNumberChange
      EditMask = '999999999999'
      FixedChar = True
      Size = 12
    end
    object qryFormItemsCTopicCode2: TStringField
      Tag = 3
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610'3'
      FieldName = 'CTopicCode2'
      OnChange = qryFormItemsCheckNumberChange
      EditMask = '999999999999'
      FixedChar = True
      Size = 12
    end
    object qryFormItemsCTopicCode3: TStringField
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610'4'
      FieldName = 'CTopicCode3'
      OnChange = qryFormItemsCheckNumberChange
      EditMask = '999999999999'
      FixedChar = True
      Size = 12
    end
    object qryFormItems_BudgetCode: TStringField
      DisplayLabel = #1576#1608#1583#1580#1607
      FieldKind = fkLookup
      FieldName = '_BudgetCode'
      LookupDataSet = DmF.qryAccBudget
      LookupKeyFields = 'BudgetTopicID'
      LookupResultField = 'BudgetCaption_L1'
      KeyFields = 'BudgetCode'
      Size = 200
      Lookup = True
    end
    object qryFormItemsBudgetCode: TIntegerField
      Tag = 3
      DisplayLabel = #1603#1583' '#1576#1608#1583#1580#1607
      FieldName = 'BudgetCode'
    end
    object qryFormItemsAidInfoNo: TStringField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1705#1605#1705#1740
      FieldName = 'AidInfoNo'
      Size = 40
    end
    object qryFormItemsAidInfoDate: TStringField
      DisplayLabel = #1578#1575#1585#1740#1582' '#1705#1605#1705#1740
      FieldName = 'AidInfoDate'
      EditMask = '9999/99/99'
      FixedChar = True
      Size = 10
    end
    object qryFormItemsProductCode: TLargeintField
      DisplayLabel = #1705#1583' '#1575#1605#1608#1575#1604#1740
      FieldName = 'ProductCode'
    end
    object qryFormItemsFormItemID: TIntegerField
      Tag = 3
      DisplayLabel = #1588#1605#1575#1585#1606#1583#1607
      FieldName = 'FormItemID'
    end
    object qryFormItemsFormID: TIntegerField
      FieldName = 'FormID'
    end
    object qryFormItemsYearID: TIntegerField
      FieldName = 'YearID'
    end
    object qryFormItemsServerID: TIntegerField
      FieldName = 'ServerID'
    end
    object qryFormItemsBankName: TStringField
      DisplayLabel = #1606#1575#1605' '#1576#1575#1606#1705
      FieldName = 'BankName'
    end
    object qryFormItemsAccountNumber: TStringField
      DisplayLabel = #1588#1593#1576#1607
      FieldName = 'AccountNumber'
      FixedChar = True
      Size = 25
    end
    object qryFormItemsCity: TStringField
      FieldName = 'City'
      FixedChar = True
    end
    object qryFormItemsProjectID: TIntegerField
      Tag = 3
      DisplayLabel = #1603#1583' '#1591#1585#1581
      FieldName = 'ProjectID'
    end
    object qryFormItemspreFormItemID: TIntegerField
      FieldName = 'preFormItemID'
    end
    object qryFormItems_CTopicName2_L1: TStringField
      DisplayLabel = #1578#1601#1589#1740#1604#1740'3'
      FieldKind = fkLookup
      FieldName = '_CTopicName2_L1'
      LookupDataSet = DmF.qryAccCenterTopics2
      LookupKeyFields = 'CTopicCode2'
      LookupResultField = 'CTopicName2'
      KeyFields = 'CTopicCode2'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormItemsCheckType: TWordField
      FieldName = 'CheckType'
    end
    object qryFormItems_CTopicName3_L1: TStringField
      DisplayLabel = #1605#1585#1603#1586#1607#1586#1610#1606#1607'3'
      FieldKind = fkLookup
      FieldName = '_CTopicName3_L1'
      LookupDataSet = DmF.qryAccCenterTopics3
      LookupKeyFields = 'CTopicCode3'
      LookupResultField = 'CTopicName3'
      KeyFields = 'CTopicCode3'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormItems_CTopicName_L1: TStringField
      DisplayLabel = #1605#1585#1603#1586#1607#1586#1610#1606#1607
      FieldKind = fkLookup
      FieldName = '_CTopicName_L1'
      LookupDataSet = DmF.qryAccCenterTopics
      LookupKeyFields = 'CTopicCode'
      LookupResultField = 'CTopicName'
      KeyFields = 'CTopicCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormItems_DetailsName_L1: TStringField
      DisplayLabel = #1578#1601#1589#1610#1604#1610
      FieldKind = fkLookup
      FieldName = '_DetailsName_L1'
      LookupDataSet = DmF.qryAccDetailCode
      LookupKeyFields = 'DetailCode'
      LookupResultField = 'DetailName'
      KeyFields = 'DetailCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormItems_TopicCodeName_L1: TStringField
      DisplayLabel = #1581#1587#1575#1576
      FieldKind = fkLookup
      FieldName = '_TopicCodeName_L1'
      LookupDataSet = DmF.qryAccTopicCode
      LookupKeyFields = 'TopicCode'
      LookupResultField = 'MoeenName'
      KeyFields = 'TopicCode'
      LookupCache = True
      Size = 150
      Lookup = True
    end
    object qryFormItems_Customer2Active: TIntegerField
      FieldKind = fkLookup
      FieldName = '_Customer2Active'
      LookupDataSet = qryCustomer2Detail
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustomerActive'
      KeyFields = 'CustomerID2'
      Lookup = True
    end
    object qryFormItems_AccountNumber: TStringField
      FieldKind = fkLookup
      FieldName = '_AccountNumber'
      LookupDataSet = qryCustomer2Detail
      LookupKeyFields = 'CustID'
      LookupResultField = 'AccountNumber'
      KeyFields = 'CustomerID2'
      Size = 50
      Lookup = True
    end
    object qryFormItems_BankId: TStringField
      FieldKind = fkLookup
      FieldName = '_BankId'
      LookupDataSet = qryCustomer2Detail
      LookupKeyFields = 'CustID'
      LookupResultField = 'BankId'
      KeyFields = 'CustomerID2'
      Size = 30
      Lookup = True
    end
    object qryFormItemsFirstUser: TStringField
      FieldName = 'FirstUser'
      Size = 50
    end
    object qryFormItemsLastUser: TStringField
      FieldName = 'LastUser'
      Size = 50
    end
    object qryFormItemsAccountNumberNew: TStringField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1581#1587#1575#1576
      FieldName = 'AccountNumberNew'
      Size = 60
    end
    object qryFormItemsReciptItems_ID: TLargeintField
      DisplayLabel = #1705#1583#1575#1585#1578#1576#1575#1591' '#1576#1575' '#1587#1601#1575#1585#1588#1575#1578
      FieldName = 'ReciptItems_ID'
    end
    object qryFormItems_ReciptItems_ID: TStringField
      DisplayLabel = #1575#1585#1578#1576#1575#1591' '#1576#1575' '#1587#1601#1575#1585#1588#1575#1578
      FieldKind = fkLookup
      FieldName = '_ReciptItems_ID'
      LookupDataSet = qryRelatedReciptItems
      LookupKeyFields = 'ID'
      LookupResultField = 'RelatedName'
      KeyFields = 'ReciptItems_ID'
      Size = 500
      Lookup = True
    end
    object qryFormItemsPO: TStringField
      FieldName = 'PO'
      Size = 30
    end
    object qryFormItemsArzAmount: TFloatField
      DisplayLabel = #1605#1602#1583#1575#1585' '#1575#1585#1586
      FieldName = 'ArzAmount'
      OnChange = qryFormItemsArzAmountChange
    end
    object qryFormItemsCheckFor: TStringField
      DisplayLabel = #1578#1608#1590#1740#1581#1575#1578' '#1705#1605#1705#1740
      FieldName = 'CheckFor'
      Size = 255
    end
  end
  object PopChoose: TPopupMenu
    AutoHotkeys = maManual
    Left = 361
    Top = 200
    object N13: TMenuItem
      Caption = #1601#1585#1575#1582#1608#1575#1606#1610' '#1670#1603#1607#1575' '#1575#1586' '#1583#1610#1587#1603#1578' '#1576#1575#1606#1603' '#1605#1604#1610
      OnClick = N13Click
    end
    object N12: TMenuItem
      Action = actselectCheck
    end
  end
  object popPrint: TPopupMenu
    AutoHotkeys = maManual
    Left = 187
    Top = 184
    object N11: TMenuItem
      Caption = #1670#1575#1662'1'
      object N9: TMenuItem
        Caption = #1601#1585#1605' '#1580#1575#1585#1610
        Hint = 'rptPaymentCash_Ctopic1,2.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
      object N10: TMenuItem
        Tag = 1
        Caption = #1607#1605#1607' '#1601#1585#1605' '#1607#1575
        Hint = 'rptPaymentCash_Ctopic1,2.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
    end
    object N21: TMenuItem
      Caption = #1670#1575#1662'2'
      object N14: TMenuItem
        Caption = #1601#1585#1605' '#1580#1575#1585#1610
        Hint = 'rptPaymentCash_Ctopic1.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
      object N15: TMenuItem
        Tag = 1
        Caption = #1607#1605#1607' '#1601#1585#1605' '#1607#1575
        Hint = 'rptPaymentCash_Ctopic1.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
    end
    object N31: TMenuItem
      Caption = #1670#1575#1662'3'
      object N16: TMenuItem
        Caption = #1601#1585#1605' '#1580#1575#1585#1610
        Hint = 'rptPaymentCash_withoutCtopic.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
      object N17: TMenuItem
        Tag = 1
        Caption = #1607#1605#1607' '#1601#1585#1605' '#1607#1575
        Hint = 'rptPaymentCash_withoutCtopic.rtm'
        OnClick = AllC_L_i_c_k_Click
      end
    end
    object N32: TMenuItem
      Tag = 1
      Caption = #1670#1575#1662' '#1576#1575' '#1603#1583' '#1583#1585#1610#1575#1601#1578' '#1603#1606#1606#1583#1607
      Hint = 'rptPaymentCash_Customer.rtm'
      OnClick = AllC_L_i_c_k_Click
    end
    object AllC_L_i_c_k_: TMenuItem
      Caption = 'Hint _ Tag'
      Visible = False
      OnClick = AllC_L_i_c_k_Click
    end
    object NReportFileName1: TMenuItem
      Caption = #1670#1575#1662' 1 '#1578#1606#1592#1610#1605#1575#1578
      OnClick = AllC_L_i_c_k_Click
    end
    object NReportFileName2: TMenuItem
      Caption = #1670#1575#1662' 2 '#1578#1606#1592#1610#1605#1575#1578
      OnClick = AllC_L_i_c_k_Click
    end
    object ItemState: TMenuItem
      Caption = #1670#1575#1662' '#1576#1585' '#1575#1587#1575#1587' '#1606#1608#1593' '#1578#1575#1611#1610#1610#1583
      Hint = 'rptPaymentCash_ItemState.rtm'
      OnClick = AllC_L_i_c_k_Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object NAllRecords: TMenuItem
      AutoCheck = True
      Caption = #1607#1605#1607' '#1601#1585#1605' '#1607#1575
    end
  end
  object qryRelatedRecipts: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      
        'SELECT     LTRIM(Recipts.ServerID) + '#39'-'#39' + LTRIM(Recipts.YearID)' +
        ' + '#39'-'#39' + LTRIM(Recipts.ReciptID) AS RelatedNo, '
      
        '                      ReciptTypes.ReciptCaption + '#39'-'#39' + LTRIM(Re' +
        'cipts.ReciptNumber) + '#39' - '#39' + Recipts.ReciptDate + '#39'-'#39' + ISNULL(' +
        'Recipts.ReciptNote, '#39#39') '
      '                      + '#39'-'#39' + Customers.CustName AS RelatedName'
      'FROM         Recipts INNER JOIN'
      
        '                      Customers ON Recipts.PersonID1 = Customers' +
        '.CustID INNER JOIN'
      
        '                      ReciptTypes ON Recipts.ReciptType = Recipt' +
        'Types.ReciptType'
      
        'WHERE     (Recipts.ReciptType IN ( 60, 61)) AND (Recipts.ReciptS' +
        'tate <> 3)')
    Left = 488
    Top = 8
  end
  object ppDBPipelineSumGrid1: TppDBPipeline
    UserName = 'DBPipelineSumGrid1'
    Left = 680
    Top = 280
  end
  object qryCustomer1: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT DISTINCT    top 1      CustID, CustName, AccountKind,'
      
        '           BankName, AccountNumber, BankId,BankReports, Mobile,C' +
        'ustomerActive,NationalID'
      ',CustomerNote'
      'FROM      Vu_CustomersGroups')
    Left = 507
    Top = 69
  end
  object qryCustomer2: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT   DISTINCT   top 1     CustID, CustName, Address, Tel, Fa' +
        'x, CustAccountNumber, Mobile ,CustomerCheckNote'
      ',CustomerActive,NationalID,CustomerNote'
      'FROM        Vu_CustomersGroups')
    Left = 490
    Top = 134
  end
  object qryCustomer3: TADOQuery
    AutoCalcFields = False
    Connection = DmF.adcBSell
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      
        'SELECT    top 1      CustID, CustName, ProjectID, BudgetID,Custo' +
        'merNote'
      'FROM Vu_CustomersGroups')
    Left = 494
    Top = 199
  end
  object qryCustomer2Detail: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT  DISTINCT      CustID, CustName, Address, Tel, Fax, Mobil' +
        'e,CustomerActive'
      
        ', AccountNumber, BankId,BankReports, Mobile,CustomerActive,Natio' +
        'nalID'
      ''
      'FROM         Vu_CustomersGroups')
    Left = 581
    Top = 222
  end
  object qryAllForms: TADOQuery
    Connection = DmF.adcBSell
    LockType = ltBatchOptimistic
    AfterScroll = qryAllFormsAfterScroll
    Parameters = <>
    SQL.Strings = (
      'SELECT        FormID, ServerID, YearID, FormType,ParentFormID'
      'FROM            Forms'
      'WHERE        (FormType = 0) AND (YearID = 1)')
    Left = 603
    Top = 74
    object qryAllFormsFormID: TIntegerField
      Tag = 3
      FieldName = 'FormID'
      ImportedConstraint = '0'
    end
    object qryAllFormsServerID: TIntegerField
      Tag = 3
      FieldName = 'ServerID'
    end
    object qryAllFormsYearID: TIntegerField
      Tag = 3
      FieldName = 'YearID'
    end
    object qryAllFormsCustomerID1: TIntegerField
      Tag = 3
      FieldName = 'CustomerID1'
    end
    object qryAllFormsCustomerID2: TIntegerField
      Tag = 3
      FieldName = 'CustomerID2'
    end
    object qryAllFormsCustomerID3: TIntegerField
      FieldName = 'CustomerID3'
    end
  end
  object srcAllForms: TDataSource
    AutoEdit = False
    DataSet = qryAllForms
    Left = 778
    Top = 20
  end
  object qryRelatedReciptItems: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      'SELECT   *                         '
      'FROM        Vu_RelatedReciptTypesItem')
    Left = 400
    Top = 96
  end
  object ProcReMainPerson: TADOStoredProc
    Connection = DmF.adcBSell
    ProcedureName = 'ReMainPerson'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        Attributes = [paNullable]
        DataType = ftBCD
        Direction = pdReturnValue
        Precision = 19
        Value = Null
      end
      item
        Name = '@FormID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@ServerID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@YearID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@FormDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 20
        Value = Null
      end
      item
        Name = '@PersonID'
        Attributes = [paNullable]
        DataType = ftString
        Size = 20
        Value = Null
      end>
    Left = 181
    Top = 74
  end
end
