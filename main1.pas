...
uses
  ...
  IdFTP, DMnipc, System.ImageList, Vcl.ImgList, IdURI,   System.UITypes, Taxs, LMDSysInfo,
  CedarMainMenu;
...
type
  ...
  TmainF = class(TForm)
    ...
    mnuBuyWithCost: TMenuItem;
    MenuItem731: TMenuItem;
    MenuItem732: TMenuItem;
    MenuItem733: TMenuItem;
  ...
  public
    ...
    procedure AddListRecipts(sFName: string);
...
var
  ...
  sysFormat: Array [1 .. 4] of ShortString;
...
uses GlobalPro, mdiMain, CustGroups, Stores, CompanyBankAccounts,
  ...
  vjForm1, SelectYear, RestoreDataBase, options,
  ...
  RptControlCardex2, Pakings, ExcelConflict, CountingDetails,
  ...
  rptInsouranceOther,
  ListBimehNew, PersonelLeave, DecExtOnMounth, Mission,
...
var
  // SubSystemID, i: Integer;
  i: Integer;
begin
  // SubSystemID := mdiMainF.MainFrame.GetSubSystemID(Self.Menu.Name);
  // SubSystemID := mdiMainF.MainFrame.GetActiveIndex;
  // Operators2F.FillListByMenu(Self.Menu, SYSID, SubSystemID)
  ...
  try
    ...
    // if opta.AccActive then
    // Operators2F.AddMenu(mainF.Mnu17Acc, 'حسابداري مالی', 0);
    // if opta.BudgetActive then
    // Operators2F.AddMenu(mainF.Mnu18Budget, 'بودجه و اعتبارات', 1);
    /// /    if opta.AuditActive then
    // //    Operators2F.AddMenu(mainF.mnuAudit, 'صورتهاي مالي', 2);
    // if opta.FinancialActive then
    // Operators2F.AddMenu(mainF.Mnu19Finalcial, 'حسابرس و كنترلها', 3);
    // if opta.PortainActive then
    // Operators2F.AddMenu(mainF.Mnu20Portion, 'تسهيم هزينه', 4);
    // if opta.SpecialActive then
    // Operators2F.AddMenu(mainF.Mnu21SpecialReport, 'گزارشات خاص', 5);
    // if opta.ManegeActive then
    // Operators2F.AddMenu(mainF.mnuMange, 'مدير سيستم', 6);
    // if opta.CurrencyActive then
    // Operators2F.AddMenu(mainF.mnuCurrencyReport, 'گزارشات ارزی', 7);
...
begin
  FreeDesktopAlert;
  lbl1.Tag := lbl1.Tag + 1;
  lbl1.Caption := IntToStr(lbl1.Tag);
...
begin
  NewItem := TMenuItem.Create(Self);
  // NewItem.Name:=MnuItemPlace.Name+NewAction.Hint;
  NewItem.Action := NewAction;
...
begin
  try
    ...
    (Sender as TJvDesktopAlert).StyleHandler.DisplayDuration := 1;
  end;
...
procedure TmainF.AddListRecipts(sFName: string);
begin
  var_MultiType_Boolean := False;
  var_MultiType_Boolean := mainF.tsListRecipts.IndexOf(sFName) > -1;
  mainF.tsListRecipts.Add(sFName);
end;
...
begin
  ...
  with qry do
    try
      ...
      SQL.Add('WHERE (ID = :ID )');
      Parameters.ParamByName('ID').Value := (Sender as TJvDesktopAlert).Tag;
      Active := True;
      Edit;
  ...
  begin
    ...
    begin
      ...
      NewItem.Name := MnuItem.Name + Action_Name.Name + Fields[0].AsString +
        IntToStr(MainMnuId * (MainMnuId + 1));
      // s4NameUnique;
