...
uses
  ...
  ExtCtrls, Buttons, Mask, DB, ADODB, DBCtrls, zAPIBalloon, Menus, Provider,
  DBClient, Xmlxform, xmldom, CheckDoc, System.ImageList, System.Actions,
  DBGridEhGrouping, ToolCtrlsEh, DBGridEhToolCtrls, DynVarsEh, EhLibVCL,
  GridsEh, DBAxisGridsEh, DBGridEh, CedarDbGrid;

type
  TAssetsMakeDocumentF = class(Ttemplate2MDIF)
    Panel1: TPanel;
    Panel5: TPanel;
    actMakeDoc: TAction;
    actShowDoc: TAction;
    ...
    Label8: TLabel;
    Label9: TLabel;
    edt_TopicName: TDBEdit;
    edt_DetailName: TDBEdit;
    edt_CtopicName1: TDBEdit;
    edt_CtopicName2: TDBEdit;
    edt_CtopicName3: TDBEdit;
...
var
  AssetsMakeDocumentF: TAssetsMakeDocumentF;

implementation

uses DM, shamsiDate, GlobalPro, Math, StrUtils, search2, sort2,
  mmessage, FaraConsts, filter_ADO, FilterClass_ADO, CheckDocument,
  Filter_ADO_Const;
{$R *.dfm}

procedure TAssetsMakeDocumentF.UpdateFilter;
begin
...
procedure TAssetsMakeDocumentF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TAssetsMakeDocumentF.initCombos;
begin
  // cmbGroups.Clear;
  ...
  CmbDocType.ItemIndex := CmbDocType.Items.IndexOfObject
    (TObject(10 { opt.DefaultDocType } ));
end;

procedure TAssetsMakeDocumentF.InitForm;
var
  sqltxt: string;
begin
...
procedure TAssetsMakeDocumentF.FormCreate(Sender: TObject);
begin
  inherited;
  initCombos;
  InitForm;
...
procedure TAssetsMakeDocumentF.actMakeDocExecute(Sender: TObject);
var
  S: String;
begin
  inherited;
...
procedure TAssetsMakeDocumentF.FormDestroy(Sender: TObject);
begin
  inherited;
...
procedure TAssetsMakeDocumentF.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
  actFilter.Execute;
...
procedure TAssetsMakeDocumentF.cmbGroupsChange(Sender: TObject);
begin
  inherited;
  actShowDoc.Execute
end;

procedure TAssetsMakeDocumentF.RdGKindDocClick(Sender: TObject);
begin
  inherited;
  actShowDoc.Execute
end;

procedure TAssetsMakeDocumentF.TabControl1Change(Sender: TObject);
begin
  inherited;
  with qryExportDoc do
  begin
    case TabControl1.TabIndex of
      ...
      2:
        begin
          Active := False;
          Sql.Text := Format(fOrgSql,
            [QuotedStr('بابت ثبت تجدید ارزیابی تا تاریخ' + opt.BaseDate),
            'Assets.MakeRevalidateDoc', QuotedStr(opt.BaseDate) + ' , ' +
            IntToStr(APPBank.Year)]);
          Active := True;
          MskDocTopic.Text := 'بابت ثبت سند حسابداري   تجدید ارزیابی ';
        end;
...
procedure TAssetsMakeDocumentF.actErrorListExecute(Sender: TObject);
var
  S: String;
  i: Integer;
begin
  ...
  begin
    ...
    try
      ...
      if ValidateDocument(mskDocDate.Text, APPBank.Year, qryExportDoc, nil,
...
procedure TAssetsMakeDocumentF.lslWarningsClick(Sender: TObject);
var
  i: Integer;
begin
  inherited;
...
procedure TAssetsMakeDocumentF.actSendExelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TAssetsMakeDocumentF._actSearchExecute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryExportDoc);
end;

procedure TAssetsMakeDocumentF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryExportDoc);
end;

procedure TAssetsMakeDocumentF.actSendXmlExecute(Sender: TObject);
var
  XtrToDb: String;
  XtrToXml: String;
  Xml: String;
...
procedure TAssetsMakeDocumentF.qryExportDocAfterOpen(DataSet: TDataSet);
begin
  inherited;
  StatusBar1.Panels[0].Text := 'مانده :‏' +
    CurrToStr(DBGrid1.GetValueFieldSum('Debt') - DBGrid1.GetValueFieldSum
    ('Credit'))
end;

procedure TAssetsMakeDocumentF.qryExportDocBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  // ShowQryParam(qryExportDoc);
end;

function TAssetsMakeDocumentF.MackDocs;
var
  LastSn: Integer;
begin
  LastSn := GetANewCode(Self.Name,
    ...
    begin
      ...
      if FindField('SourceDataBase') <> nil then
        FieldByName('SourceDataBase').AsString :=
          IfThen(optSub.Subsys[18] = '1', 'Assets.DocExports',
          APPBank.Name + '.Assets.DocExports');
        ...
        if FindField('SourceKey') <> nil then
          FieldByName('SourceKey').AsString := 'Serial = ' +
            ...
            APPBank.Year.ToString.Trim;
      Post;
    end;

    try
      DMF.adcBsell.Execute
        (Format('assets.InsertDocExports %D , %S , %D , %d , %d ,%d , %d ,%d ',
        ...
        StrToInt64(VarToStr(qryExportDoc.Parameters.ParamByName
        ('SellsEmporiumFrom').Value)),
        StrToInt64(VarToStr(qryExportDoc.Parameters.ParamByName
        ('SellsEmporiumTo').Value))]))
    except
      Result := False;
    end;
...
procedure TAssetsMakeDocumentF.actSearchExecute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryExportDoc);
end;

// procedure TMakeDocumentF.qryExportDocBeforeOpen(DataSet: TDataSet);
// begin
// inherited;
