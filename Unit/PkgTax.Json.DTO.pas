unit PkgTax.Json.DTO;

interface

uses System.Classes, System.Json, Rest.Json, System.Generics.Collections,
  Rest.JsonReflect;
...
type
  ...
  protected
    ...
    function ObjectList<T: class>(var aList: TObjectList<T>; aSource: TArray<T>)
      : TObjectList<T>;
  public
  ...
  private
    ...
    class procedure PrettyPrintPair(aJSONValue: TJSONPair;
      aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
    class procedure PrettyPrintJSON(aJSONValue: TJsonValue;
      aOutputStrings: TStrings; Indent: Integer = 0); overload;
    class procedure PrettyPrintArray(aJSONValue: TJSONArray;
      aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
  protected
...
begin
  inherited;
  FOptions := [joDateIsUTC, joDateFormatISO8601, joIgnoreEmptyStrings]; //
end;
...
class procedure TJsonDTO.PrettyPrintJSON(aJSONValue: TJsonValue;
  aOutputStrings: TStrings; Indent: Integer);
var
  i: Integer;
  Ident: Integer;
begin
  ...
  begin
    ...
    for i := 0 to TJSONObject(aJSONValue).Count - 1 do
      PrettyPrintPair(TJSONObject(aJSONValue).Pairs[i], aOutputStrings,
        i = TJSONObject(aJSONValue).Count - 1, Ident);

    aOutputStrings.Add(StringOfChar(' ', Ident) + '}');
  end
  else if aJSONValue is TJSONArray then
    PrettyPrintArray(TJSONArray(aJSONValue), aOutputStrings,
      i = TJSONObject(aJSONValue).Count - 1, Ident)
  else
    aOutputStrings.Add(StringOfChar(' ', Ident) + aJSONValue.ToString);
end;

class procedure TJsonDTO.PrettyPrintArray(aJSONValue: TJSONArray;
  aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
var
  i: Integer;
begin
  ...
  begin
    ...
    if i < aJSONValue.Count - 1 then
      aOutputStrings[aOutputStrings.Count - 1] :=
        aOutputStrings[aOutputStrings.Count - 1] + ',';
  end;
...
class procedure TJsonDTO.PrettyPrintPair(aJSONValue: TJSONPair;
  aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
const
  TEMPLATE = '%s:%s';
var
...
begin
  ...
  try
    ...
    Line := Format(TEMPLATE, [aJSONValue.JsonString.ToString,
      Trim(NewList.Text)]);
  finally
    NewList.Free;
  end;
...
begin
  ...
  try
    ...
    begin
      aValue := aValue.Trim;
      if (aValue = '') and not Assigned(JSONValue) or (aValue <> '') and
        Assigned(JSONValue) and JSONValue.Null then
        Exit
      else
...
function TArrayMapper.List<T>(var aList: TList<T>; aSource: TArray<T>)
  : TList<T>;
begin
  if aList = nil then
  begin
...
function TArrayMapper.ObjectList<T>(var aList: TObjectList<T>;
  aSource: TArray<T>): TObjectList<T>;
var
  Element: T;
begin
  if aList = nil then
...
procedure TArrayMapper.RefreshArray<T>(aSource: TList<T>;
  var aDestination: TArray<T>);
begin
  if aSource <> nil then
...
type
  ...
  public
    function ObjectsConverter(Data: TObject; Field: string)
      : TListOfObjects; override;
  end;
...
begin
  RttiProperty := ctx.GetType(Data.ClassInfo)
    .GetProperty(Copy(Field, 2, MAXINT));
...
begin
  inherited Create(ctObjects, rtObjects, TGenericListFieldInterceptor,
    nil, false);
end;
...
var
  ...
  Date: TDateTime;
begin
  Date := RttiContext.GetType(Data.ClassType).GetField(Field).GetValue(Data)
    .AsType<TDateTime>;
  if Date = 0 then
    Result := string.Empty
  else
...
procedure TSuppressZeroDateInterceptor.StringReverter(Data: TObject;
  Field, Arg: string);
var
  RttiContext: TRttiContext;
  Date: TDateTime;
