inherited rptReciptSumOnPersonID1F: TrptReciptSumOnPersonID1F
  Left = 370
  Top = 197
  ...
  inherited Panel3: TPanel
    ...
    object SumGrid1: TSumGrid
      ...
      MasterGrid = DBGrid1
      ShowZero = True
      FieldsName = 'RequestedEntity;RequestedWeight;entity;weight;Price;'
    end
  end
  ...
  object qryCustomers: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT DISTINCT Recipts.PersonID1 AS PersonID1, CASE WHEN Custom' +
        'ers.CustName LIKE N'#39'%'#39'+ ISNULL(Customers.CustFirstName, '#39#39') +'#39'%'#39 +
        ' THEN Customers.CustName'
      
        ' ELSE Customers.CustName + '#39' '#39' + ISNULL(Customers.CustFirstName,' +
        ' '#39#39') END AS CustName'
      'FROM            Recipts LEFT OUTER JOIN'
      
        '                         ReciptItems ON Recipts.YearID = ReciptI' +
        'tems.YearID AND Recipts.ServerID = ReciptItems.ServerID AND Reci' +
        'pts.ReciptID = ReciptItems.ReciptID LEFT OUTER JOIN'
      
        '                         Customers ON Recipts.PersonID1 = Custom' +
        'ers.CustID'
      ''
      
        'WHERE     (Customers.CustomerGrpID BETWEEN :CustomerGrpIDFrom AN' +
        'D :CustomerGrpIDTo)'
  ...
  object qryRecipt_Items: TADOQuery
    ...
    SQL.Strings = (
        ...
        'SELECT  ReciptItems.StuffCode, StuffCoding.c_StuffName, Units.Un' +
        'itName,'
      
        '      sum(  ReciptItems.InputEntity + ReciptItems.OutputEntity) ' +
        'AS Entity,'
      
        '      sum(ReciptItems.InputWeight + ReciptItems.OutputWeight) AS' +
        ' Weight,'
      
        '      sum( ReciptItems.TotalInputPrice + ReciptItems.TotalOutput' +
        'Price) AS Price'
      ', Recipts.PersonID1 AS PersonID1, Customers.CustName'
      
        ', SUM(ReciptItems.RequestedEntity) AS RequestedEntity, SUM(Recip' +
        'tItems.RequestedWeight)  AS RequestedWeight'
      ''
      ''
      'FROM    Stores RIGHT OUTER JOIN'
      
        '        Recipts ON Stores.n_StoreID = Recipts.StoreID  RIGHT OUT' +
        'ER JOIN'
      '        ReciptItems LEFT OUTER JOIN'
      '        Units RIGHT OUTER JOIN'
      
        '        StuffCoding ON Units.UnitCode = StuffCoding.n_UnitCode O' +
        'N ReciptItems.StuffCode = StuffCoding.c_StuffCode ON'
      
        '        Recipts.ReciptID = ReciptItems.ReciptID AND Recipts.Serv' +
        'erID = ReciptItems.ServerID AND Recipts.YearID = ReciptItems.Yea' +
        'rID'
      ''
      'LEFT OUTER JOIN'
      '        CustomersGroup RIGHT OUTER JOIN'
      
        '        Customers ON CustomersGroup.CustomerGrpID = Customers.Cu' +
        'stomerGrpID ON ReciptItems.PersonID1 = Customers.CustID'
      ''
      ' INNER JOIN'
      ...
      ''
        ...
        '       AND (Recipts.PersonID1 BETWEEN :PersonID1From AND :Person' +
        'ID1To)'
      
        '       AND (Recipts.PersonID2 BETWEEN :PersonID2From AND :Person' +
        'ID2To)'
      ...
      ''
        ...
        'group by  ReciptItems.StuffCode, StuffCoding.c_StuffName, Units.' +
        'UnitName, Recipts.PersonID1, Customers.CustName')
    Left = 216
    Top = 233
    ...
    object qryRecipt_ItemsUnitName: TStringField
      Tag = 3
  ...
  object ppReport1: TppReport
    ...
    Template.FileName = 
      'D:\Projects\Fara\BSell\Exe\Report\rptReciptSumOnCustomersItem.rt' +
      'm'
