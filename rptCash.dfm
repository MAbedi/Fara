inherited rptCashF: TrptCashF
  ...
  object qryDetail: TADOQuery
    ...
    SQL.Strings = (
      ...
      'UNION ALL'
        ...
        'SELECT 1, '#39#1705#1604' '#1576#1575#1606#1705#1607#1575' '#1608' '#1583#1585#1740#1575#1601#1578#1607#1575#39' AS Caption,Vu_CustomersGroups.C' +
        'ustName ,CAST(SUM(bed  - bes ) AS MONEY) AS Balance ,@Bank'
      'FROM'#9'Vu_CustomersGroups INNER JOIN'
      
        #9#9'dbo.allaccount(1,2,10,-1,  0 , 99999 , '#39'0000/00/00'#39', :DateTo ,' +
        ' :DateFromRemain , :YearIDFrom   , :YearIDTo'
      
        '    , 0 , 99999 , 0 ,  99999 , DEFAULT  )allaccount  ON Vu_Custo' +
        'mersGroups.CustID = allaccount.customerid1'
      ''
      'WHERE     (Vu_CustomersGroups.GroupType=1)'
      ''
      'GROUP BY customerid1  , Vu_CustomersGroups.CustName'
      'UNION ALL'
      ...
      'FROM'#9'Vu_CustomersGroups INNER JOIN'
        ...
        #9#9'dbo.allaccount(1,2,10,-1,  0 , 99999 , '#39'0000/00/00'#39', :Date1To ' +
        ', :DateFromRemain1 , @YearIDFrom   , @YearIDTo'
  ...
  object qryMaster: TADOQuery
    ...
    Parameters = <>
    SQL.Strings = (
      'SELECT DISTINCT LEFT(checkdate,7) AS Code,'
      'CASE SUBSTRING(checkdate,6,2) WHEN '#39'01'#39' THEN '#39#1601#1585#1608#1585#1583#1740#1606#39
      ' WHEN '#39'02'#39' THEN '#39#1575#1585#1583#1740#1576#1607#1588#1578#39
      ' WHEN '#39'03'#39' THEN '#39#1582#1585#1583#1575#1583#39
      ' WHEN '#39'04'#39' THEN '#39#1578#1740#1585#39
      ' WHEN '#39'05'#39' THEN '#39#1605#1585#1583#1575#1583#39
      ' WHEN '#39'06'#39' THEN '#39#1588#1607#1585#1740#1608#1585#39
      ' WHEN '#39'07'#39' THEN '#39#1605#1607#1585#39
      ' WHEN '#39'08'#39' THEN '#39#1570#1576#1575#1606#39
      ' WHEN '#39'09'#39' THEN '#39#1570#1584#1585#39
      ' WHEN '#39'10'#39' THEN '#39#1583#1740#39
      ' WHEN '#39'11'#39' THEN '#39#1576#1607#1605#1606#39
      ' WHEN '#39'12'#39' THEN '#39#1575#1587#1601#1606#1583#39' END+LEFT(CheckDate,4) AS Mounth'
      'FROM FormItems'
      'WHERE (LEN(checkdate)=10)'
      '    AND ( LEFT(checkdate,7) >= '#39'@DateFrom'#39' )'
      'ORDER BY  LEFT(checkdate,7)'
      ''
      '')
    Left = 560
    Top = 104
  ...
  object cliDetail: TClientDataSet
    Aggregates = <>
    Params = <>
