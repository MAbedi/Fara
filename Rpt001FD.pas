...
uses
  ...
  DBGridEh, CedarDbGrid, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet,
  FireDAC.Comp.Client;

type
  TRpt001FDF = class(Ttemplate2MDIF)
    Panel1: TPanel;
    ...
    qryReciptspriceWithDef: TFMTBCDField;
    qryReciptspriceWithDef2: TFMTBCDField;
    ...
    qryReciptsTotallSellPrice: TFMTBCDField;
    qryReciptsPayebelPrice: TFMTBCDField;
    qryReciptsPayebelPrice2: TFMTBCDField;
    ...
    qryReciptsModifyDate: TSQLTimeStampField;
    qryReciptsInsertDate: TSQLTimeStampField;
    qryReciptsReciptDateMiladi: TSQLTimeStampField;
    qryReciptsDATE_OF_SEND_TO_TAX: TSQLTimeStampField;
    qryReciptsDeficitAddDec: TCurrencyField;
    qryReciptsDeficitValue: TCurrencyField;
    qryReciptsTaxValue: TCurrencyField;
    qryReciptsStandardRate: TCurrencyField;
    qryReciptsRegPrice: TCurrencyField;
    qryReciptsArzAmount: TFloatField;
    qryReciptsArticle: TCurrencyField;
    qryReciptsWage: TCurrencyField;
    qryReciptsScoria: TCurrencyField;
    qryReciptsLoanPayment: TCurrencyField;
    qryReciptsRialsEqual: TCurrencyField;
    qryReciptsprice: TFMTBCDField;
  ...
  private
    formType: Integer;
    RecallTypeOption, Settings: Largeint;
...
begin
  ...
  if (User.PowerUser) or b then
    ...
    begin
      ...
      CreateMDIForm2(TRpt001fdF, Rpt001fdF, mainF, FieldByName('ReciptType')
        .AsInteger);
      if Rpt001fdF.qryRecipts.Active then
        Rpt001fdF.qryRecipts.Locate('ReciptID;ServerID;YearID',
...
begin
  ...
  begin
    ...
    begin
      ...
      with TCurrencyField.Create(qryRecipts) do
      begin
...
procedure TRpt001FDF.actFilterExecute(Sender: TObject);
var
...
procedure TRpt001FDF.ppLblShowLimitPlace1GetText(Sender: TObject;
  var Text: String);
begin
  Text := LblShowLimitPlace1.Caption;
end;

procedure TRpt001FDF.ppLblShowLimitPlace2GetText(Sender: TObject;
  var Text: String);
begin
  Text := LblShowLimitPlace2.Caption;
end;

procedure TRpt001FDF.UpdateList;
var
  S: String;
begin
  ...
  begin
    ...
    if formType = 0 then
    begin
      ParamByName('ReciptTypeFrom').Value := 0;
      ParamByName('ReciptTypeTo').Value := 9999;
      // SQL.Add(' :TypeChecked ');
    end
    else
    begin
      ParamByName('ReciptTypeFrom').Value := formType;
      ParamByName('ReciptTypeTo').Value := formType;
    end;
    ...
    ParamByName('numberFrom').Value :=
      GetcFrom(myParams.ParamValues['number'], ftInteger);
    ParamByName('numberto').Value :=
    ...
    begin
      ...
      ParamByName('CustomerGrpIDFrom').Value :=
        GetcFrom(myParams.ParamValues['CustomerGrpID'], ftInteger);
      ParamByName('CustomerGrpIDTo').Value :=
    ...
    if qryInitQry.FieldByName('MasirActive').AsInteger = 1 then
    begin
      ParamByName('MasirIDFrom').Value :=
        GetcFrom(myParams.ParamValues['MasirID'], ftInteger);
      ParamByName('MasirIDTo').Value :=
    ...
    if qryInitQry.FieldByName('Person5Active').AsInteger = 1 then
    begin
      ParamByName('PersonID5From').Value :=
        GetcFrom(myParams.ParamValues['PersonID5'], ftInteger);
      ParamByName('PersonID5To').Value :=
    ...
    if UseUnitM then
    begin
      ParamByName('UseUnitIDFrom').Value :=
        GetcFrom(myParams.ParamValues['UseUnitID'], ftInteger);
      ParamByName('UseUnitIDTo').Value :=
        GetcTo(myParams.ParamValues['UseUnitID'], ftInteger);
    end
    else
    begin
      ParamByName('UseUnitIDFrom').Value := 0;
      ParamByName('UseUnitIDTo').Value := 9999;
    end;
    ...
    begin
      ...
      ParamByName('SellsEmporiumTo').Value := 9999;
    end;
    if SellsMethodActive then
    begin
      ParamByName('SellsMethodFrom').Value :=
        GetcFrom(myParams.ParamValues['SellsMethod'], ftInteger);
      ParamByName('SellsMethodTo').Value :=
        GetcTo(myParams.ParamValues['SellsMethod'], ftInteger);
    end
    else
    begin
      ParamByName('SellsMethodFrom').Value := 0;
      ParamByName('SellsMethodTo').Value := 9999;
    end;

    if AidInfoActive then
    begin
      ParamByName('AidNumberFrom').Value :=
        GetcFrom(myParams.ParamValues['AidNumber'], ftFMTBcd );
      ParamByName('AidNumberTo').Value :=
        GetcTo(myParams.ParamValues['AidNumber'], ftFMTBcd );

      ParamByName('AidDateFrom').Value :=
        GetcFrom(myParams.ParamValues['AidDate'], ftDate);
      ParamByName('AidDateTo').Value :=
    ...
    begin
      SQL.Text := StringReplace(SQL.Text, ':AidNumberFrom', '',
        [rfReplaceAll]);
      SQL.Text := StringReplace(SQL.Text, ':AidNumberTo', '999999999999999999999999999999999999',
        [rfReplaceAll]);
    ...
    if ReciptsRowActive then
    begin
      ParamByName('ReciptsRowFrom').Value :=
        GetcFrom(myParams.ParamValues['ReciptsRow'], ftInteger);
      ParamByName('ReciptsRowTo').Value :=
        GetcTo(myParams.ParamValues['ReciptsRow'], ftInteger);
    end
    else
    begin
      ParamByName('ReciptsRowFrom').Value := 0;
      ParamByName('ReciptsRowTo').Value := 9999;
    end;

    if opt.DefaultDateActive then
    begin
      ParamByName('DefaultDateFrom').Value :=
        GetcFrom(myParams.ParamValues['DefaultDate'], ftDate);
      ParamByName('DefaultDateTo').Value :=
        GetcTo(myParams.ParamValues['DefaultDate'], ftDate);
    end
    else
    begin
      ParamByName('DefaultDateFrom').Value := '0000/00/00';
      ParamByName('DefaultDateTo').Value := '9999/99/99';
    end;
    ...
    LblShowLimitPlace2.Caption := 'محدوده تاريخ فرم : ' + ' از ' +
      ParamByName('DateFrom').Value + ' تا ' + ParamByName
      ('DateTo').Value;
    Active := True;
  end; // with
...
procedure TRpt001FDF.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
  actFilter.Execute;
...
procedure TRpt001FDF.actSendToExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TRpt001FDF.actShowFormExecute(Sender: TObject);
begin
  inherited;
  ShowReciptTypes(qryRecipts, mainF);
end;

procedure TRpt001FDF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qryRecipts);
end;

procedure TRpt001FDF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qryRecipts);
end;

procedure TRpt001FDF.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
  actShowForm.Execute
end;

procedure TRpt001FDF.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
var
  aDataSet: TDataSet;
begin
...
procedure TRpt001FDF.FormDestroy(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.FormResize(Sender: TObject);
begin
  inherited;
  // SetColSize(DBGrid1, 6, True, IntToStr(formType));
end;

procedure TRpt001FDF.actReciptState1Execute(Sender: TObject);
var
  i, c: Integer;
...
procedure TRpt001FDF.qryReciptsAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TRpt001FDF.actReciptState2Execute(Sender: TObject);
var
  i, c: Integer;
  S, Descriptions: string;
...
procedure TRpt001FDF.qryReciptsReciptStateGetText(Sender: TField;
...
procedure TRpt001FDF.RadioGroup1Click(Sender: TObject);
begin
  inherited;
  case RadioGroup1.ItemIndex of
    0:
      begin
        ...
        qryRecipts.IndexFieldNames  := 'RelatedID';
      end;
    1:
    ...
    2:
      begin
        ...
        qryRecipts.IndexFieldNames := EmptyStr;
      end;
  end;
  qryRecipts.Filtered := qryRecipts.Filter <> EmptyStr;
end;

procedure TRpt001FDF.ppLblCompanyNameGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := APPBank.CompanyName
end;

procedure TRpt001FDF.ppSysVarPageNumberGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := GetPageNumberString(Text)
end;

procedure TRpt001FDF.ppLblPrintDateGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := GetPrintDate
end;

procedure TRpt001FDF.ppLblFormNameGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := Text + Caption
end;

procedure TRpt001FDF.ppDBTxtStateGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := GetReciptState(qryRecipts.FieldByName('ReciptState').AsInteger)
end;

procedure TRpt001FDF.actPrintExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.AutoStateChange;
var
...
begin
  ...
  try
    qryUpDate.Params := qryRecipts.Params;
...
procedure TRpt001FDF.MenuItem3Click(Sender: TObject);
begin
  inherited;
  try
    mnuAllCheck.Checked := False;
...
procedure TRpt001FDF.MenuItem4Click(Sender: TObject);
begin
  inherited;
  try
    try
...
procedure TRpt001FDF.ppImage1DrawCommandCreate(Sender, aDrawCommand: TObject);
var
  S: String;
begin
  inherited;
...
procedure TRpt001FDF.ppLblCaptionGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := qryInitQry.FieldByName('ReciptCaption').AsString
end;

procedure TRpt001FDF.ppLblEconomicNumberGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.ppSysVarPageNumGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := GetPageNumberString(Text);
end;

procedure TRpt001FDF.qryItemsCalcFields(DataSet: TDataSet);
begin
  inherited;
  qryItems.FieldByName('_radif').AsInteger := abs(DataSet.RecNo);
end;

procedure TRpt001FDF.mnuAllSecondTypeClick(Sender: TObject);
var
  i, c: Integer;
  S: string;
  procedure sUPDATE;
...
procedure TRpt001FDF.MnuCheckMoaadiyanClick(Sender: TObject);
var
  aTax: TMainTax;
...
procedure TRpt001FDF.mnuEntityClick(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.TaxStateMenu;
begin
...
procedure TRpt001FDF.MnuTaxMoaadiyanClick(Sender: TObject);
var
  i: Integer;
begin
  inherited;
...
procedure TRpt001FDF.mnuWeightClick(Sender: TObject);
begin
  inherited;
  mnuEntity.Checked := not mnuWeight.Checked
end;

procedure TRpt001FDF.N7Click(Sender: TObject);
begin
  inherited;
  try
    mnuAllCheck.Checked := False;
...
procedure TRpt001FDF.ReciptState3_5_4(Caption: string; ReciptState: Integer);
var
  i, c: Integer;
  S, Descriptions: string;
  procedure sUPDATE;
...
procedure TRpt001FDF.rgTaxClick(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.actReciptState3Execute(Sender: TObject);
begin
  inherited;
  ReciptState3_5_4('باطل', 3)
end;

procedure TRpt001FDF.actReciptState4Execute(Sender: TObject);
begin
  inherited;
  ReciptState3_5_4('رزرو', 4)
end;

procedure TRpt001FDF.actReciptState5Execute(Sender: TObject);
begin
  inherited;
  ReciptState3_5_4('پیش نویس', 5)
end;

procedure TRpt001FDF.BtnReciptStateClick(Sender: TObject);
begin
  inherited;
  PopMnuReciptState.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TRpt001FDF.btnTaxClick(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.chkCorrelateReciptTypeClick(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.chkFooter4SumClick(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.PerformFileNameClick(Sender: TObject);
begin
  inherited;
  try
    SeletedPrint(ppDBPipeline1, DBGrid1);
...
procedure TRpt001FDF.mnu4allClick(Sender: TObject);
begin
  inherited;
...
function TRpt001FDF.MakeSQLInsert(ReciptIDS, Descriptions: string;
  qryRecipts: TDataSet): string;
begin
...
procedure TRpt001FDF.actReciptState0Execute(Sender: TObject);
var
  i, c: Integer;
  S, Descriptions: string;
  procedure sUPDATE;
...
procedure TRpt001FDF.ppDBTextStuffSizeGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := Text + '';
end;

procedure TRpt001FDF.ppLabelnum2alphabetGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.ppLabelnum2alphabetPayebelGetText(Sender: TObject;
  var Text: String);
var
...
procedure TRpt001FDF.ppLblSecondTypeGetText(Sender: TObject; var Text: String);
begin
  inherited;
  Text := ''; // ComboType.Text;
...
procedure TRpt001FDF.logoPrint(Sender: TObject);
var
  S: String;
begin
  inherited;
...
procedure TRpt001FDF.ppLabelTotalOutputPriceGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.ppDBTxtPayablePriceGetText(Sender: TObject;
  var Text: String);
var
  PayablePrice: currency;
begin
...
procedure TRpt001FDF.ppLabelOutputWeightGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.ppLabelOutputEntityGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TRpt001FDF.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  QuickSearch(Key, qryReciptsReciptNumber);
end;

procedure TRpt001FDF.DBGrid1TitleClick(Column: TColumnEh);
var
  i: Integer;
begin
  inherited;
...
procedure TRpt001FDF.actCorrelateReciptExecute(Sender: TObject);
var
  i, id: Largeint;
begin
  inherited;
...
procedure TRpt001FDF.callstpNewReciptsCorrelate;
begin
...
procedure TRpt001FDF.actNewReciptsCorrelatePriceExecute(Sender: TObject);
begin
  inherited;
  if get_response
...
procedure TRpt001FDF.actSortRunExecute(Sender: TObject);
begin
  inherited;
  popSortRun.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TRpt001FDF.allSort1Click(Sender: TObject);
begin
  inherited;
  allSortRecipts(qryRecipts, qryInitQry, (Sender as TMenuItem).Tag)
end;

procedure TRpt001FDF.BitBtn3Click(Sender: TObject);
begin
  inherited;
  PopMnuOther.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
end;

procedure TRpt001FDF.actPrintVijehExecute(Sender: TObject);
begin
  inherited;
  DBGrid2PrintF.showGrid2Print(DBGrid1, formType);
end;

procedure TRpt001FDF.actUpdateExecute(Sender: TObject);
var
  rid: Integer;
begin
  inherited;
...
procedure TRpt001FDF.actViewFileFExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.actwebNipcCheckExecute(Sender: TObject);
var
  // pp: TJsonDTO;
  // S: string;
  i: Integer;
...
procedure TRpt001FDF.actwebNipcExecute(Sender: TObject);
var
  // pp: TJsonDTO;
  i: Integer;
begin
...
procedure TRpt001FDF.actDocNoExecute(Sender: TObject);
begin
  inherited;
  ClearDoc(DBGrid1, qryRecipts);
end;

procedure TRpt001FDF.actDocumentUpDateExecute(Sender: TObject);
begin
  inherited;
  DocumentUpDateF.Enter(qryRecipts, fkRcipts);
end;

procedure TRpt001FDF.actDBGridSelectAllExecute(Sender: TObject);
begin
  inherited;
  try
    qryRecipts.AfterScroll := nil;
...
procedure TRpt001FDF.actRecallSpecialReciptsFExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.actReciptDeleteExecute(Sender: TObject);
var
  i: Integer;
  S: string;
begin
...
procedure TRpt001FDF.actDelReciptsDeficitsExecute(Sender: TObject);
var
  i: Integer;
  S: string;
begin
...
procedure TRpt001FDF.ppLabel__StandardRateGetText(Sender: TObject;
  var Text: String);
var
  c: currency;
begin
...
procedure TRpt001FDF.ppLbl__TaxGetText(Sender: TObject; var Text: String);
var
  cloned: TADOQuery;
  c: currency;
begin
...
procedure TRpt001FDF.ppLbl_TotalStandardRateGetText(Sender: TObject;
  var Text: String);
var
  cloned: TADOQuery;
  c: currency;
...
procedure TRpt001FDF.ppLbl_TotalStandardRateAlphaGetText(Sender: TObject;
  var Text: String);
var
  cloned: TADOQuery;
  c: currency;
...
procedure TRpt001FDF.qryReciptsBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  // GetSumGridFields(DBGrid1, SumGrid2);
end;

procedure TRpt001FDF.qryReciptsModifyDateGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
  inherited;
...
procedure TRpt001FDF.ppLblState2ZeroDrawCommandCreate(Sender,
...
procedure TRpt001FDF.ppLblReMainOldGetText(Sender: TObject; var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.ppLblReMainGetText(Sender: TObject; var Text: String);
begin
  inherited;
...
procedure TRpt001FDF.plblTotallSellPriceGetText(Sender: TObject;
  var Text: String);
var
  PayablePrice: currency;
begin
...
procedure TRpt001FDF.plblTotallSellPricAlphaGetText(Sender: TObject;
  var Text: String);
var
  PayablePrice: currency;
begin
...
procedure TRpt001FDF.actMakeDocumentExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt001FDF.actManagGridColumnsExecute(Sender: TObject);
begin
  inherited;
  // ManagGridColumnsF.ShowColumns(DMf.adcBSell, DBGrid1, formType)
end;

procedure TRpt001FDF.ppDBCalcnum2alphabetGetText(Sender: TObject;
  var Text: String);
var
  c: currency;
begin
...
procedure TRpt001FDF.actAppendixExecute(Sender: TObject);
var
  T: TThread;
begin
  inherited;
...
procedure TRpt001FDF.ProcCheckExistsFile;
var
  DoWhile: Boolean;
begin
  With qryRecipts do
...
procedure TRpt001FDF.actCalcCostStorageExecute(Sender: TObject);
var
  i, c: Integer;
  S: string;
  procedure sUPDATE;
...
procedure TRpt001FDF.actCancellationFixesExecute(Sender: TObject);
var
  i, c: Integer;
  S, Descriptions: string;
  procedure sUPDATE;
...
begin
  inherited;
  if Pos(VarToStr(qryRecipts.ParamByName('States').Value), '3') < 1
  then
  begin
...
procedure TRpt001FDF.FilterONServerID;
begin
...
procedure TRpt001FDF.actCorrelateConversionCoExecute(Sender: TObject);
var
...
begin
  ...
  begin
    SelectStoreF.GetStore(sID1, 'انبار براي ثبت ', '', MyEntityDisplayType);
    if sID1.code = 0 then
      Exit
    else
      StoreID := sID1.code;
...
procedure TRpt001FDF.actReciptsRowExecute(Sender: TObject);
var
...
procedure TRpt001FDF.actTransmittalExecute(Sender: TObject);
var
  i, c: Integer;
  S: string;
  procedure sUPDATE;
...
procedure TRpt001FDF.finallyqryRecipts;
var
  i: Integer;
begin
  With qryRecipts do
...
procedure TRpt001FDF.DBGrid1Enter(Sender: TObject);
begin
  inherited;
  qryRecipts.AfterScroll := nil
end;

procedure TRpt001FDF.ppDBTxtPayableSellPriceGetText(Sender: TObject;
var Text: String);
var
  PayablePrice: currency;
begin
...
procedure TRpt001FDF.ppReport1AfterPrint(Sender: TObject);
begin
  inherited;
  qryItems.Filtered := False;
end;

procedure TRpt001FDF.plblAllStuffCodingGetText(Sender: TObject; var Text: string);
begin
  inherited;
...
procedure TRpt001FDF.plblAllReadBankConfigGetText(Sender: TObject;
var Text: String);
begin
  inherited;
