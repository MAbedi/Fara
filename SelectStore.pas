...
begin
  ...
  SelectStoreF.Caption := 'انتخاب ' + myCaption;
  try
    ...
    begin
      ...
      begin
        ...
        // محدودیت بر اساس جدول UsersStoreReciptTypes (StoreID + ReciptType)
        Parameters.ParamByName('ReciptType').Value := ReciptType;

        Active := True;
        if IsEmpty then
          raise Exception.Create('هيچ ' + myCaption +
            'ي در سيستم براي شما تعريف نشده است.');
        ListStores.Clear;
        while not eof do
        begin
