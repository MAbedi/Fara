inherited CommitmentsF: TCommitmentsF
  Left = 0
  Top = 47
  Caption = #1578#1593#1607#1583#1575#1578
  ClientHeight = 707
  ClientWidth = 991
  Position = poDesigned
  OnResize = FormResize
  ExplicitWidth = 999
  ExplicitHeight = 738
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlUnderButton: TPanel
    Top = 666
    Width = 991
    TabOrder = 3
    ExplicitTop = 666
    ExplicitWidth = 991
    object okPanel: TPanel [0]
      Left = 485
      Top = 1
      Width = 252
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 1
      DesignSize = (
        252
        39)
      object BitBtn9: TBitBtn
        Left = 169
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetPost1
        Anchors = [akTop, akRight]
        Caption = #1578#1575#1610#1610#1583
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        TabOrder = 1
      end
      object BitBtn10: TBitBtn
        Left = 92
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetCancel1
        Anchors = [akTop, akRight]
        Caption = #1575#1606#1589#1585#1575#1601
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        TabOrder = 0
      end
    end
    inherited BtnReject: TBitBtn
      TabOrder = 2
    end
    object newPanel: TPanel
      Left = 737
      Top = 1
      Width = 253
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 0
      DesignSize = (
        253
        39)
      object BitBtn6: TBitBtn
        Left = 168
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetInsert1
        Anchors = [akTop, akRight]
        Caption = #1580#1583#1610#1583
        TabOrder = 2
      end
      object BitBtn7: TBitBtn
        Left = 91
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetEdit1
        Anchors = [akTop, akRight]
        Caption = #1608#1610#1585#1575#1610#1588
        TabOrder = 1
      end
      object BitBtn8: TBitBtn
        Left = 14
        Top = 7
        Width = 75
        Height = 25
        Action = DataSetDelete1_
        Anchors = [akTop, akRight]
        Caption = #1581#1584#1601
        TabOrder = 0
      end
    end
    object BitBtn11: TBitBtn
      Left = 314
      Top = 8
      Width = 75
      Height = 25
      Action = actPrint
      Anchors = [akLeft, akBottom]
      Caption = #1670#1575#1662
      TabOrder = 6
    end
    object BitBtn12: TBitBtn
      Left = 237
      Top = 8
      Width = 75
      Height = 25
      Action = actExcel
      Anchors = [akLeft, akBottom]
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' Excel'
      TabOrder = 5
    end
    object BitBtn13: TBitBtn
      Left = 160
      Top = 8
      Width = 75
      Height = 25
      Action = actSearch_
      Caption = #1580#1587#1578#1580#1608
      Glyph.Data = {
        
        8C00CEA59C00C69C9400FF00FF00FF00FF00FF00FF00FF00FF00}
      TabOrder = 4
    end
    object BitBtn14: TBitBtn
      Left = 83
      Top = 8
      Width = 75
      Height = 25
      Action = actSort
      Caption = #1578#1585#1578#1610#1576
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00067B1100FF00FF00FF00FF00FF00FF00}
      TabOrder = 3
    end
    object DBNavigator1: TDBNavigator
      Left = 468
      Top = 9
      Width = 96
      Height = 23
      DataSource = srcMaster
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      Anchors = [akLeft, akBottom]
      Flat = True
      TabOrder = 7
    end
  end
  inherited Panel2: TPanel
    Width = 991
    Height = 49
    TabOrder = 0
    ExplicitWidth = 991
    ExplicitHeight = 49
    inherited ImgTemplate: TImage
      Left = 933
      ExplicitLeft = 720
    end
    inherited lblCaption: TLabel
      Left = 883
      Height = 29
      ExplicitLeft = 883
    end
  end
  inherited Panel3: TPanel
    Top = 49
    Width = 991
    Height = 598
    Anchors = [akTop, akRight]
    TabOrder = 1
    ExplicitTop = 49
    ExplicitWidth = 991
    ExplicitHeight = 598
    object Panel6: TPanel
      Left = 2
      Top = 2
      Width = 987
      Height = 63
      Align = alTop
      BevelOuter = bvLowered
      TabOrder = 0
      object PnlCompany: TPanel
        Left = 375
        Top = 1
        Width = 611
        Height = 61
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        object lbl1: TLabel
          Left = 532
          Top = 8
          Width = 56
          Height = 13
          Caption = #1608#1575#1581#1583' '#1575#1580#1585#1575#1740#1740
        end
        object lbl6: TLabel
          Left = 532
          Top = 35
          Width = 39
          Height = 13
          Caption = #1606#1608#1593' '#1578#1593#1607#1583
        end
        object Label1: TLabel
          Left = 170
          Top = 8
          Width = 58
          Height = 13
          Caption = #1608#1590#1593#1740#1578' '#1578#1593#1607#1583
        end
        object DBText5: TDBText
          Left = 109
          Top = 6
          Width = 53
          Height = 16
          Alignment = taRightJustify
          AutoSize = True
          BiDiMode = bdLeftToRight
          DataField = 'Status'
          DataSource = srcMaster
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHighlight
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentBiDiMode = False
          ParentFont = False
        end
        object btn1: TBitBtn
          Tag = 1
          Left = 296
          Top = 6
          Width = 23
          Height = 20
          Caption = '...'
          TabOrder = 0
          OnClick = btn1Click
        end
        object DBEdit12: TDBEdit
          Left = 322
          Top = 6
          Width = 204
          Height = 19
          TabStop = False
          Color = 14342874
          Ctl3D = False
          DataField = '_ComponyName'
          DataSource = srcMaster
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 2
        end
        object dblkcbbInterfaceID: TDBLookupComboBox
          Left = 322
          Top = 32
          Width = 204
          Height = 21
          DataField = 'InterfaceID'
          DataSource = srcMaster
          KeyField = 'InterfaceID'
          ListField = 'InterfaceCaption_L1'
          ListSource = srcInterface
          TabOrder = 1
        end
      end
      object Panel5: TPanel
        Left = 1
        Top = 1
        Width = 248
        Height = 61
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 1
        object lbl4: TLabel
          Left = 161
          Top = 8
          Width = 50
          Height = 13
          Caption = #1588#1605#1575#1585#1607' '#1601#1585#1605
        end
        object lbl5: TLabel
          Left = 161
          Top = 35
          Width = 41
          Height = 13
          Caption = #1578#1575#1585#1610#1582' '#1601#1585#1605
        end
        object edtBudgetSerial: TDBEdit
          Left = 48
          Top = 6
          Width = 105
          Height = 21
          DataField = 'BudgetSerial'
          DataSource = srcMaster
          TabOrder = 1
        end
        object DBEdit6: TDBEdit
          Left = 48
          Top = 32
          Width = 105
          Height = 21
          Color = clInfoBk
          DataField = 'BudgetDate'
          DataSource = srcMaster
          TabOrder = 2
        end
        object btn2: TBitBtn
          Left = 22
          Top = 6
          Width = 23
          Height = 20
          Caption = '...'
          TabOrder = 0
          TabStop = False
          OnClick = btn2Click
        end
      end
    end
    object grp1: TGroupBox
      Left = 2
      Top = 65
      Width = 987
      Height = 72
      Align = alTop
      Caption = '   '#1578#1608#1590#1740#1581#1575#1578'   '
      TabOrder = 1
      object dbredtNote_L1: TDBRichEdit
        Left = 2
        Top = 15
        Width = 983
        Height = 55
        Align = alClient
        DataField = 'Note_L1'
        DataSource = srcMaster
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        TabOrder = 0
        Zoom = 100
      end
    end
    object Panel1: TPanel
      Left = 2
      Top = 564
      Width = 987
      Height = 32
      Align = alBottom
      TabOrder = 3
      object PnlNewItem: TPanel
        Left = 721
        Top = 1
        Width = 265
        Height = 30
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 1
        DesignSize = (
          265
          30)
        object BitBtn16: TBitBtn
          Left = 16
          Top = 2
          Width = 75
          Height = 25
          Action = DataSetDelete1
          Anchors = [akTop, akRight]
          Caption = #1581#1584#1601' '#1585#1583#1610#1601
          TabOrder = 2
        end
        object BitBtn15: TBitBtn
          Left = 96
          Top = 2
          Width = 75
          Height = 25
          Action = DataSetEdit_
          Caption = #1608#1740#1585#1575#1740#1588' '#1585#1583#1740#1601
          TabOrder = 1
        end
        object BitBtn5: TBitBtn
          Left = 175
          Top = 2
          Width = 75
          Height = 25
          Action = DataSetInsert_
          Caption = #1575#1601#1586#1608#1583#1606' '#1585#1583#1740#1601
          TabOrder = 0
        end
      end
      object PnlOkItem: TPanel
        Left = 516
        Top = 1
        Width = 205
        Height = 30
        Align = alRight
        BevelOuter = bvNone
        TabOrder = 0
        object BitBtn17: TBitBtn
          Left = 114
          Top = 2
          Width = 75
          Height = 25
          Action = DataSetPost_
          Caption = #1578#1575#1740#1740#1583
          TabOrder = 0
        end
        object BitBtn18: TBitBtn
          Left = 31
          Top = 2
          Width = 75
          Height = 25
          Action = DataSetCancel_
          Caption = #1575#1606#1589#1585#1575#1601
          TabOrder = 1
        end
      end
    end
    object Panel4: TPanel
      Left = 2
      Top = 137
      Width = 987
      Height = 427
      Align = alClient
      TabOrder = 2
      object Panel7: TPanel
        Left = 154
        Top = 1
        Width = 832
        Height = 425
        Align = alRight
        Alignment = taRightJustify
        BevelEdges = [beLeft, beTop, beRight]
        BevelOuter = bvNone
        TabOrder = 0
        object Label9: TLabel
          Left = 178
          Top = 219
          Width = 38
          Height = 13
          Caption = #1606#1608#1593' '#1605#1576#1604#1594
        end
        object Label7: TLabel
          Left = 754
          Top = 137
          Width = 71
          Height = 13
          Caption = #1606#1608#1593' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
        end
        object Label8: TLabel
          Left = 754
          Top = 178
          Width = 67
          Height = 13
          Caption = #1705#1583' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
        end
        object Label6: TLabel
          Left = 457
          Top = 219
          Width = 73
          Height = 13
          Caption = #1606#1602#1583#1740'/'#1594#1740#1585' '#1606#1602#1583#1740
        end
        object Label5: TLabel
          Left = 754
          Top = 219
          Width = 39
          Height = 13
          Caption = #1606#1608#1593' '#1578#1593#1607#1583
        end
        object Label2: TLabel
          Left = 754
          Top = 14
          Width = 40
          Height = 13
          Caption = #1705#1583' '#1576#1608#1583#1580#1607
        end
        object Label3: TLabel
          Left = 754
          Top = 55
          Width = 62
          Height = 13
          Caption = #1705#1583' '#1605#1581#1604' '#1578#1575#1605#1740#1606
        end
        object Label4: TLabel
          Left = 754
          Top = 96
          Width = 45
          Height = 13
          Caption = #1705#1583' '#1588#1582#1589
        end
        object Label11: TLabel
          Left = 754
          Top = 260
          Width = 20
          Height = 13
          Caption = #1605#1576#1604#1594
        end
        object Label10: TLabel
          Left = 598
          Top = 260
          Width = 43
          Height = 13
          Caption = #1576#1607' '#1581#1585#1608#1601':'
        end
        object Label12: TLabel
          Left = 754
          Top = 301
          Width = 40
          Height = 13
          Caption = #1578#1608#1590#1740#1581#1575#1578
        end
        object Label13: TLabel
          Left = 754
          Top = 369
          Width = 62
          Height = 13
          Caption = #1588#1605#1575#1585#1607' '#1705#1605#1705#1740
        end
        object Label14: TLabel
          Left = 458
          Top = 369
          Width = 53
          Height = 13
          Caption = #1578#1575#1585#1740#1582' '#1705#1605#1705#1740
        end
        object DBText1: TDBText
          Left = 79
          Top = 176
          Width = 562
          Height = 17
          Color = clBtnFace
          DataField = '_costName'
          DataSource = srcItems
          ParentColor = False
        end
        object DBText2: TDBText
          Left = 237
          Top = 15
          Width = 404
          Height = 17
          DataField = '_BudjectName'
          DataSource = srcItems
        end
        object DBText3: TDBText
          Left = 237
          Top = 56
          Width = 404
          Height = 17
          DataField = '_Companyname'
          DataSource = srcItems
        end
        object DBText4: TDBText
          Left = 237
          Top = 97
          Width = 404
          Height = 17
          DataField = '_CustomerName'
          DataSource = srcItems
        end
        object Label15: TLabel
          Left = 571
          Top = 258
          Width = 22
          Height = 13
          Caption = #1589#1601#1585
          Color = clBtnFace
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHighlight
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentColor = False
          ParentFont = False
        end
        object DBEdit3: TDBEdit
          Left = 683
          Top = 52
          Width = 65
          Height = 21
          DataField = 'CompanyCode'
          DataSource = srcItems
          TabOrder = 1
          OnKeyDown = DBEdit3KeyDown
        end
        object DBEdit9: TDBEdit
          Left = 683
          Top = 175
          Width = 65
          Height = 21
          DataField = '_reciptNumber'
          DataSource = srcItems
          TabOrder = 4
          OnKeyDown = DBEdit9KeyDown
        end
        object BitBtn3: TBitBtn
          Tag = 2
          Left = 653
          Top = 92
          Width = 29
          Height = 22
          Caption = '...'
          TabOrder = 12
          TabStop = False
          OnClick = BitBtn3Click
        end
        object BitBtn2: TBitBtn
          Tag = 1
          Left = 653
          Top = 51
          Width = 29
          Height = 22
          Caption = '...'
          TabOrder = 13
          TabStop = False
          OnClick = BitBtn2Click
        end
        object BitBtn1: TBitBtn
          Left = 653
          Top = 10
          Width = 29
          Height = 22
          Caption = '...'
          TabOrder = 14
          TabStop = False
          OnClick = BitBtn1Click
        end
        object DBEdit1: TDBEdit
          Left = 683
          Top = 11
          Width = 65
          Height = 21
          DataField = 'BudgetTopicID'
          DataSource = srcItems
          TabOrder = 0
          OnKeyDown = DBEdit1KeyDown
        end
        object DBEdit7: TDBEdit
          Left = 683
          Top = 93
          Width = 65
          Height = 21
          DataField = 'CustomerID'
          DataSource = srcItems
          TabOrder = 2
          OnKeyDown = DBEdit7KeyDown
        end
        object DBEdit10: TDBEdit
          Left = 653
          Top = 257
          Width = 95
          Height = 21
          DataField = 'Bed'
          DataSource = srcItems
          TabOrder = 8
          OnChange = DBEdit10Change
        end
        object DBEdit11: TDBEdit
          Left = 653
          Top = 366
          Width = 95
          Height = 21
          DataField = 'AidNum'
          DataSource = srcItems
          TabOrder = 10
        end
        object DBEdit13: TDBEdit
          Left = 355
          Top = 366
          Width = 95
          Height = 21
          DataField = 'AidDate'
          DataSource = srcItems
          TabOrder = 11
        end
        object DBComboBox2: TDBComboBox
          Left = 653
          Top = 216
          Width = 95
          Height = 21
          Style = csDropDownList
          DataField = 'Ctype'
          DataSource = srcItems
          Items.Strings = (
            #1575#1593#1578#1576#1575#1585#1740
            #1601#1575#1602#1583' '#1575#1593#1578#1576#1575#1585)
          TabOrder = 5
          OnEnter = DBComboBox2Enter
        end
        object DBComboBox3: TDBComboBox
          Left = 355
          Top = 216
          Width = 95
          Height = 21
          Style = csDropDownList
          DataField = 'Cashtype'
          DataSource = srcItems
          Items.Strings = (
            #1606#1602#1583#1740
            #1594#1740#1585' '#1606#1602#1583#1740)
          TabOrder = 6
          OnEnter = DBComboBox3Enter
        end
        object DBComboBox4: TDBComboBox
          Left = 78
          Top = 216
          Width = 95
          Height = 21
          Style = csDropDownList
          DataField = 'Bes'
          DataSource = srcItems
          Items.Strings = (
            #1575#1601#1586#1575#1740#1588#1740
            #1705#1575#1607#1588#1740)
          TabOrder = 7
          OnEnter = DBComboBox4Enter
        end
        object BitBtn4: TBitBtn
          Left = 653
          Top = 174
          Width = 29
          Height = 22
          Caption = '...'
          TabOrder = 15
          TabStop = False
          OnClick = BitBtn4Click
        end
        object DBMemo1: TDBMemo
          Left = 78
          Top = 299
          Width = 670
          Height = 46
          DataField = 'ItemNote_L1'
          DataSource = srcItems
          TabOrder = 9
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 583
          Top = 133
          Width = 165
          Height = 21
          DataField = 'CostType'
          DataSource = srcItems
          KeyField = 'ReciptType'
          ListField = 'ReciptCaption'
          ListSource = srcLookUp
          TabOrder = 3
          OnEnter = DBLookupComboBox1Enter
        end
      end
      object Panel8: TPanel
        Left = 1
        Top = 1
        Width = 153
        Height = 425
        Align = alClient
        BevelInner = bvRaised
        BevelKind = bkSoft
        BevelOuter = bvNone
        ParentBackground = False
        TabOrder = 1
        object Label16: TLabel
          Left = 1
          Top = 1
          Width = 147
          Height = 14
          Align = alTop
          Alignment = taCenter
          Caption = #1608#1590#1593#1740#1578' '#1578#1593#1607#1583#1575#1578' '#1602#1576#1604#1740
          Color = clHighlight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHighlightText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentColor = False
          ParentFont = False
          Transparent = True
          ExplicitWidth = 120
        end
        object DBGrid2: TDBGrid
          Left = 1
          Top = 15
          Width = 147
          Height = 379
          Margins.Left = 5
          Margins.Right = 5
          Margins.Bottom = 5
          TabStop = False
          Align = alClient
          Color = clCream
          Ctl3D = True
          DataSource = srcHistory
          DrawingStyle = gdsGradient
          GradientEndColor = clSkyBlue
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = [fsBold]
          Columns = <
            item
              Expanded = False
              FieldName = 'status'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'Cash'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CashOut'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'SumCash'
              Visible = True
            end>
        end
        object StatusBar2: TStatusBar
          Left = 1
          Top = 394
          Width = 147
          Height = 26
          Panels = <
            item
              Text = #1605#1575#1606#1583#1607' '#1578#1593#1607#1583' '#1578#1587#1576#1578' '#1576#1607' '#1607#1586#1740#1606#1607':   '
              Width = 270
            end
            item
              Text = #1605#1575#1606#1583#1607' '#1575#1593#1578#1576#1575#1585':   '
              Width = 20
            end>
        end
      end
    end
    object DBGrid1: TDBGrid
      Tag = 111
      Left = 2
      Top = 137
      Width = 987
      Height = 427
      TabStop = False
      Align = alClient
      Color = clCream
      DataSource = srcItems
      DrawingStyle = gdsGradient
      FixedColor = clSkyBlue
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      ReadOnly = True
      TabOrder = 4
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'BudgetTopicID'
          Visible = True
        end
        item
          Expanded = False
          FieldName = '_BudjectName'
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CompanyCode'
          Width = 73
          Visible = True
        end
        item
          Expanded = False
          FieldName = '_Companyname'
          Width = 91
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Cashtype'
          Title.Alignment = taCenter
          Width = 84
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Ctype'
          Width = 44
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Bes'
          Title.Alignment = taCenter
          Width = 63
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Bed'
          Title.Alignment = taCenter
          Width = 50
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ItemNote_L1'
          Width = 49
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CustomerID'
          Visible = True
        end
        item
          Expanded = False
          FieldName = '_CustomerName'
          Width = 78
          Visible = True
        end
        item
          Expanded = False
          FieldName = '_CostTypeName'
          Width = 82
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CostCode'
          Width = 102
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'AidDate'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'AidNum'
          Width = 83
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ItemNote_L2'
          Visible = False
        end>
    end
  end
  object StatusBar1: TStatusBar [3]
    Left = 0
    Top = 647
    Width = 991
    Height = 19
    Panels = <
      item
        Width = 300
      end
      item
        Width = 200
      end>
  end
  inherited ActionList: TActionList
    Left = 744
    Top = 0
    inherited DataSetInsert1: TDataSetInsert
      DataSource = srcMaster
    end
    inherited DataSetEdit1: TDataSetEdit
      DataSource = srcMaster
    end
    inherited DataSetPost1: TDataSetPost
      DataSource = srcMaster
    end
    object DataSetDelete1: TDataSetDelete [4]
      Category = 'Dataset'
      Caption = #1581#1584#1601' '#1585#1583#1610#1601
      DataSource = srcItems
    end
    inherited DataSetCancel1: TDataSetCancel
      DataSource = srcMaster
    end
    object DataSetInsert_: TDataSetInsert [6]
      Category = 'Dataset'
      Caption = #1575#1601#1586#1608#1583#1606' '#1585#1583#1740#1601
      Hint = 'Insert'
      DataSource = srcItems
    end
    object actPrint: TAction [7]
      Caption = #1670#1575#1662
      OnExecute = actPrintExecute
    end
    object actExcel: TAction [8]
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' Excel'
      OnExecute = actExcelExecute
    end
    inherited actSearch_: TAction
      OnExecute = actSearch_Execute
    end
    object actSort: TAction [10]
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
    inherited DataSetDelete1_: TDataSetDelete
      DataSource = srcMaster
    end
    object DataSetEdit_: TDataSetEdit
      Category = 'Dataset'
      Caption = #1608#1740#1585#1575#1740#1588' '#1585#1583#1740#1601
      Hint = 'Edit'
      DataSource = srcItems
    end
    object DataSetPost_: TDataSetPost
      Category = 'Dataset'
      Caption = #1578#1575#1740#1740#1583
      Hint = 'Post'
      ImageIndex = 7
      DataSource = srcItems
    end
    object DataSetCancel_: TDataSetCancel
      Category = 'Dataset'
      Caption = #1575#1606#1589#1585#1575#1601
      Hint = 'Cancel'
      ImageIndex = 8
      DataSource = srcItems
    end
  end
  inherited ImageList1: TImageList
    Left = 672
    Top = 1
    Bitmap = {
      
      000000000000}
  end
  object qryMaster: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    AfterInsert = qryMasterAfterInsert
    BeforePost = qryMasterBeforePost
    AfterPost = qryMasterAfterPost
    AfterCancel = qryMasterAfterCancel
    BeforeDelete = qryMasterBeforeDelete
    AfterDelete = qryMasterAfterDelete
    AfterScroll = qryMasterAfterScroll
    Parameters = <>
    Left = 232
    Top = 59
    object qryMasterBudgetSerial: TIntegerField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1601#1585#1605
      FieldName = 'BudgetSerial'
      Required = True
    end
    object qryMasterBudgetDate: TStringField
      DisplayLabel = #1578#1575#1585#1610#1582' '#1601#1585#1605
      FieldName = 'BudgetDate'
      Required = True
      EditMask = '!9999/99/00;1;_'
      FixedChar = True
      Size = 10
    end
    object qryMasterAccCode: TIntegerField
      DisplayLabel = #1603#1583' '#1605#1593#1740#1606
      FieldName = 'AccCode'
    end
    object qryMasterDetailCode: TIntegerField
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610
      FieldName = 'DetailCode'
    end
    object qryMasterCTopic1: TIntegerField
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610'2'#39'1'
      FieldName = 'CTopic1'
    end
    object qryMasterCTopic2: TIntegerField
      DisplayLabel = #1603#1583' '#1578#1601#1589#1610#1604#1610'3'
      FieldName = 'CTopic2'
    end
    object qryMasterCompanyCode: TIntegerField
      FieldName = 'CompanyCode'
    end
    object qryMaster_ComponyName: TStringField
      FieldKind = fkLookup
      FieldName = '_ComponyName'
      LookupDataSet = qryComponyName
      LookupKeyFields = 'CompanyCode'
      LookupResultField = 'CompanyName_L1'
      KeyFields = 'CompanyCode'
      LookupCache = True
      Size = 100
      Lookup = True
    end
    object qryMasterBudgetID: TIntegerField
      FieldName = 'BudgetID'
    end
    object qryMasterInterfaceID: TIntegerField
      DisplayLabel = #1606#1608#1593' '#1578#1593#1607#1583
      FieldName = 'InterfaceID'
      Required = True
    end
    object qryMasterNote_L1: TStringField
      FieldName = 'Note_L1'
      Size = 200
    end
    object qryMasterNote_L2: TStringField
      FieldName = 'Note_L2'
      Size = 200
    end
    object qryMasterYearID: TIntegerField
      FieldName = 'YearID'
    end
    object qryMasterStatus: TWordField
      FieldName = 'Status'
      OnGetText = qryMasterStatusGetText
      OnSetText = qryMasterStatusSetText
    end
  end
  object srcMaster: TDataSource
    DataSet = qryMaster
    OnStateChange = srcMasterStateChange
    Left = 273
    Top = 57
  end
  object qryItems: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterInsert = qryItemsAfterInsert
    BeforePost = qryItemsBeforePost
    AfterPost = qryItemsAfterPost
    Parameters = <>
    Left = 328
    Top = 290
    object qryItemsBudgetItemID: TIntegerField
      FieldName = 'BudgetItemID'
    end
    object qryItemsBudgetID: TIntegerField
      FieldName = 'BudgetID'
    end
    object qryItemsItemSerial: TIntegerField
      FieldName = 'ItemSerial'
    end
    object qryItemsBudgetTopicID: TIntegerField
      Tag = 3
      DisplayLabel = #1603#1583' '#1576#1608#1583#1580#1607
      FieldName = 'BudgetTopicID'
      Required = True
      OnChange = qryItemsBudgetTopicIDChange
    end
    object qryItemsBed: TBCDField
      Tag = 3
      DisplayLabel = #1605#1576#1604#1594
      FieldName = 'Bed'
      Precision = 19
    end
    object qryItemsItemNote_L1: TStringField
      Tag = 3
      DisplayLabel = #1588#1585#1581' '#1579#1576#1578
      FieldName = 'ItemNote_L1'
      Size = 1000
    end
    object qryItemsItemNote_L2: TStringField
      DisplayLabel = 'Note'
      FieldName = 'ItemNote_L2'
      Size = 1000
    end
    object qryItemsCustomerID: TIntegerField
      Tag = 3
      DisplayLabel = #1705#1583' '#1588#1582#1589
      FieldName = 'CustomerID'
    end
    object qryItems_BudjectName: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '#1576#1608#1583#1580#1607
      FieldKind = fkLookup
      FieldName = '_BudjectName'
      LookupDataSet = _qryBudject
      LookupKeyFields = 'BudgetTopicID'
      LookupResultField = 'BudgetCaption_L1'
      KeyFields = 'BudgetTopicID'
      LookupCache = True
      ReadOnly = True
      Size = 100
      Lookup = True
    end
    object qryItemsBes: TSmallintField
      Tag = 3
      DisplayLabel = #1606#1608#1593' '#1605#1576#1604#1594
      FieldName = 'Bes'
      OnGetText = qryItemsBesGetText
      OnSetText = qryItemsBesSetText
    end
    object qryItemsCompanyCode: TIntegerField
      Tag = 3
      DisplayLabel = #1705#1583' '#1605#1581#1604' '#1578#1575#1605#1740#1606
      FieldName = 'CompanyCode'
      Required = True
    end
    object qryItemsCashtype: TWordField
      Tag = 3
      DisplayLabel = #1606#1602#1583#1740'/'#1594#1740#1585#1606#1602#1583#1740
      FieldName = 'Cashtype'
      OnChange = qryItemsCashtypeChange
      OnGetText = qryItemsCashtypeGetText
      OnSetText = qryItemsCashtypeSetText
    end
    object strngfldItemsAidDate: TStringField
      DisplayLabel = #1578#1575#1585#1740#1582' '#1705#1605#1705#1740
      FieldName = 'AidDate'
      Size = 10
    end
    object strngfldItemsAidNum: TStringField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1705#1605#1705#1740
      FieldName = 'AidNum'
    end
    object strngfldItems_Companyname: TStringField
      DisplayLabel = #1593#1606#1608#1575#1606' '#1605#1581#1604' '#1578#1575#1605#1740#1606
      FieldKind = fkLookup
      FieldName = '_Companyname'
      LookupDataSet = qryComponyName
      LookupKeyFields = 'CompanyCode'
      LookupResultField = 'CompanyName_L1'
      KeyFields = 'CompanyCode'
      ReadOnly = True
      Size = 50
      Lookup = True
    end
    object qryItemsCtype: TWordField
      Tag = 3
      DisplayLabel = #1606#1608#1593' '#1578#1593#1607#1583
      FieldName = 'Ctype'
      OnChange = qryItemsCtypeChange
      OnGetText = qryItemsCtypeGetText
      OnSetText = qryItemsCtypeSetText
    end
    object qryItemsCostType: TWordField
      Tag = 3
      DisplayLabel = #1606#1608#1593' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
      FieldName = 'CostType'
      OnChange = qryItemsCostTypeChange
      OnGetText = qryItemsCostTypeGetText
      OnSetText = qryItemsCostTypeSetText
    end
    object qryItemsCostCode: TStringField
      DisplayLabel = #1588#1606#1575#1587#1607' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
      FieldName = 'CostCode'
      Size = 70
    end
    object qryItems_CustomerName: TStringField
      Tag = 3
      DisplayLabel = #1606#1575#1605' '#1588#1582#1589
      FieldKind = fkLookup
      FieldName = '_CustomerName'
      LookupDataSet = _qryCustomers
      LookupKeyFields = 'CustID'
      LookupResultField = 'CustName'
      KeyFields = 'CustomerID'
      Size = 50
      Lookup = True
    end
    object qryItems_reciptNumber: TIntegerField
      FieldKind = fkLookup
      FieldName = '_reciptNumber'
      LookupDataSet = _qryCost
      LookupKeyFields = 'KeyID'
      LookupResultField = 'reciptnumber'
      KeyFields = 'CostCode'
      Lookup = True
    end
    object qryItems_costName: TStringField
      DisplayLabel = #1606#1608#1593' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
      FieldKind = fkLookup
      FieldName = '_costName'
      LookupDataSet = _qryCost
      LookupKeyFields = 'KeyID'
      LookupResultField = 'Caption'
      KeyFields = 'CostCode'
      Size = 1000
      Lookup = True
    end
    object qryItemsCostTypeName: TStringField
      DisplayLabel = #1606#1608#1593' '#1593#1575#1605#1604' '#1607#1586#1740#1606#1607
      FieldKind = fkLookup
      FieldName = '_CostTypeName'
      LookupDataSet = qrylookup
      LookupKeyFields = 'ReciptType'
      LookupResultField = 'ReciptCaption'
      KeyFields = 'CostType'
      Size = 70
      Lookup = True
    end
  end
  object srcItems: TDataSource
    DataSet = qryItems
    OnStateChange = srcItemsStateChange
    OnDataChange = srcItemsDataChange
    Left = 330
    Top = 235
  end
  object _qryBudject: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT     BudgetTopicID, BudgetCaption_L1, BudgetCaption_L2'
      'FROM         acc.BudgetTopics')
    Left = 656
    Top = 233
  end
  object qryComponyName: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'CompanyCodeFrom'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 0
      end
      item
        Name = 'CompanyCodeTo'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 999999999
      end>
    SQL.Strings = (
      'SELECT     CompanyCode, CompanyName_L1, CompanyName_L2'
      'FROM         acc.Companies'
      
        'WHERE   (CompanyCode BETWEEN :CompanyCodeFrom  AND :CompanyCodeT' +
        'o )'
      '')
    Left = 148
    Top = 5
  end
  object qryInterface: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    AfterScroll = qryInterfaceAfterScroll
    Parameters = <>
    SQL.Strings = (
      
        'select InterfaceID, InterfaceCaption_L1, InterfaceCaption_L2 ,Cu' +
        'stGroups'
      'from Acc.Interfaces where BudgetEffect=2')
    Left = 816
    Top = 96
    object qryInterfaceInterfaceID: TIntegerField
      FieldName = 'InterfaceID'
    end
    object strngfldInterfaceInterfaceCaption_L1: TStringField
      FieldName = 'InterfaceCaption_L1'
      Size = 50
    end
    object strngfldInterfaceInterfaceCaption_L2: TStringField
      FieldName = 'InterfaceCaption_L2'
      Size = 50
    end
    object qryInterfaceCustGroups: TStringField
      FieldName = 'CustGroups'
      Size = 250
    end
  end
  object srcInterface: TDataSource
    DataSet = qryInterface
    Left = 704
    Top = 96
  end
  object _qryCustomers: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'CustomerGrps'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 14
        Value = #39'2,3'#39
      end>
    SQL.Strings = (
      'select CustID,CustName  from dbo.Customers '
      
        'where (CustID<>0) and ('#39','#39'+ltrim(CustomerGrpID)+'#39','#39' like :Custom' +
        'erGrps)')
    Left = 552
    Top = 280
    object _qryCustomersCustID: TIntegerField
      FieldName = 'CustID'
    end
    object _qryCustomersCustName: TStringField
      FieldName = 'CustName'
      Size = 150
    end
  end
  object _qryCost: TADOQuery
    Connection = DmF.adcBSell
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'YearId'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'recipttype'
        DataType = ftWord
        Precision = 3
        Size = 1
        Value = Null
      end>
    SQL.Strings = (
      
        'select recipts.reciptnumber, stores.c_storename, customers.custn' +
        'ame,'
      
        ' Ltrim(recipts.yearid) +'#39'_'#39'+ Ltrim(recipts.serverid) +'#39'_'#39'+ Ltrim' +
        '(recipts.reciptid) as KeyID,'
      
        ' stores.c_storename +'#39'__'#39'+ customers.custname + case when ((look' +
        'ups.name is not null) and (lookups.name<>'#39#39'))'
      ' then '#39'__'#39'+ lookups.name else '#39#39' end as Caption'
      
        ' from customers inner join recipts on customers.custid = recipts' +
        '.personid1 inner join stores'
      ' on recipts.storeid = stores.n_storeid left outer join lookups'
      
        'on recipts.secondtype = lookups.lookupid where (recipts.yearid =' +
        ' :YearId)and(recipts.recipttype = :recipttype)')
    Left = 424
    Top = 304
    object _qryCostKeyID: TStringField
      FieldName = 'KeyID'
      ReadOnly = True
      Size = 44
    end
    object _qryCostreciptnumber: TIntegerField
      FieldName = 'reciptnumber'
    end
    object _qryCostCaption: TStringField
      FieldName = 'Caption'
      ReadOnly = True
      Size = 274
    end
  end
  object qryHistory: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'ID'
        DataType = ftWideString
        Size = 4
        Value = '1101'
      end
      item
        Name = 'ID2'
        DataType = ftWideString
        Size = 4
        Value = '1101'
      end
      item
        Name = 'ID3'
        DataType = ftWideString
        Size = 4
        Value = '1101'
      end
      item
        Name = 'ID4'
        DataType = ftWideString
        Size = 4
        Value = '1101'
      end
      item
        Name = 'ID5'
        DataType = ftWideString
        Size = 4
        Value = '1101'
      end>
    SQL.Strings = (
      
        'select id,status,SUM(cash)as Cash,SUM(chashout) as CashOut,SUM(s' +
        'umbed) as SumCash from ('
      
        'select 1 as id,'#39#1575#1593#1578#1576#1575#1585#1740#39' as status,isnull(case Cashtype when 1 t' +
        'hen SUM(bed) end,0) as cash,'
      'isnull(case Cashtype when 2 then SUM(bed) end,0) as chashout,'
      'isnull(case Cashtype when 1 then SUM(bed) end,0)+'
      'isnull(case Cashtype when 2 then SUM(bed) end,0) as sumbed'
      'from acc.BudgetItems'
      'where (BudgetTopicID=:ID)and(Ctype=1)'
      'group by Cashtype'
      ''
      'union all'
      ''
      
        'select 2 as id,'#39#1601#1575#1602#1583' '#1575#1593#1578#1576#1575#1585#39' as status,isnull(case Cashtype when' +
        ' 1 then SUM(bed) end,0) as cash,'
      'isnull(case Cashtype when 2 then SUM(bed) end,0) as chashout,'
      'isnull(case Cashtype when 1 then SUM(bed) end,0)+'
      'isnull(case Cashtype when 2 then SUM(bed) end,0) as sumbed'
      'from acc.BudgetItems'
      'where (BudgetTopicID=:ID2)and(Ctype=2)'
      'group by Cashtype'
      ''
      'union All'
      ''
      
        'select 3 as id,'#39#1576#1585#1585#1587#1740' '#1588#1583#1607#39' as status,isnull(case BI.Cashtype whe' +
        'n 1 then SUM(BI.bed) end,0) as cash,'
      
        'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as chasho' +
        'ut,'
      'isnull(case BI.Cashtype when 1 then SUM(BI.bed) end,0)+'
      'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as sumbed'
      'from acc.BudgetItems BI inner join acc.Budgets B'
      'on BI.BudgetID=b.BudgetID'
      'where ((BI.BudgetTopicID=:ID3) and (B.Status=1))'
      'group by Cashtype'
      ''
      'union All'
      ''
      
        'select 4 as id,'#39#1576#1585#1585#1587#1740' '#1606#1588#1583#1607#39' as status,isnull(case BI.Cashtype wh' +
        'en 1 then SUM(BI.bed) end,0) as cash,'
      
        'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as chasho' +
        'ut,'
      'isnull(case BI.Cashtype when 1 then SUM(BI.bed) end,0)+'
      'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as sumbed'
      'from acc.BudgetItems BI inner join acc.Budgets B'
      'on BI.BudgetID=b.BudgetID'
      'where ((BI.BudgetTopicID=:ID4) and (B.Status=2))'
      'group by Cashtype'
      ''
      'union All'
      ''
      
        'select 5 as id,'#39#1578#1575#1740#1740#1583' '#1588#1583#1607#39' as status,isnull(case BI.Cashtype whe' +
        'n 1 then SUM(BI.bed) end,0) as cash,'
      
        'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as chasho' +
        'ut,'
      'isnull(case BI.Cashtype when 1 then SUM(BI.bed) end,0)+'
      'isnull(case BI.Cashtype when 2 then SUM(BI.bed) end,0) as sumbed'
      'from acc.BudgetItems BI inner join acc.Budgets B'
      'on BI.BudgetID=b.BudgetID'
      'where ((BI.BudgetTopicID=:ID5) and (B.Status=3))'
      'group by Cashtype'
      ''
      ') as temp'
      'group by status,id'
      'order by id')
    Left = 120
    Top = 400
    object qryHistoryid: TIntegerField
      FieldName = 'id'
      ReadOnly = True
    end
    object qryHistorystatus: TStringField
      DisplayLabel = #1608#1590#1593#1740#1578
      FieldName = 'status'
      ReadOnly = True
      Size = 11
    end
    object qryHistoryCash: TBCDField
      DisplayLabel = #1606#1602#1583#1740
      FieldName = 'Cash'
      ReadOnly = True
      Precision = 19
    end
    object qryHistoryCashOut: TBCDField
      DisplayLabel = #1594#1740#1585' '#1606#1602#1583#1740
      FieldName = 'CashOut'
      ReadOnly = True
      Precision = 19
    end
    object qryHistorySumCash: TBCDField
      DisplayLabel = #1580#1605#1593' '#1705#1604
      FieldName = 'SumCash'
      ReadOnly = True
      Precision = 19
    end
  end
  object srcHistory: TDataSource
    DataSet = qryHistory
    Left = 80
    Top = 392
  end
  object ppDBPipeline1: TppDBPipeline
    DataSource = srcMaster
    OpenDataSource = False
    UserName = 'DBPipeline1'
    Left = 552
    Top = 208
  end
  object ppDBPipeline2: TppDBPipeline
    DataSource = srcItems
    OpenDataSource = False
    UserName = 'DBPipeline2'
    Left = 744
    Top = 376
    MasterDataPipelineName = 'ppDBPipeline1'
  end
  object ppReport1: TppReport
    AutoStop = False
    DataPipeline = ppDBPipeline1
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.Duplex = dpNone
    PrinterSetup.Orientation = poLandscape
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.SaveDeviceSettings = False
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 210000
    PrinterSetup.mmPaperWidth = 297000
    PrinterSetup.PaperSize = 9
    Template.FileName = 'D:\Projects\Fara2010\Finance\Exe\Report\Commitments.rtm'
    ArchiveFileName = '($MyDocuments)\ReportArchive.raf'
    DeviceType = 'Screen'
    DefaultFileDeviceType = 'PDF'
    EmailSettings.ReportFormat = 'PDF'
    EmailSettings.ConnectionSettings.MailService = 'SMTP'
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RedirectURI = 'http://localhost'
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RedirectPort = 0
    EmailSettings.ConnectionSettings.WebMail.GmailSettings.OAuth2.RefreshTokenLifeSpan = 365
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RedirectURI = 'http://localhost'
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RedirectPort = 0
    EmailSettings.ConnectionSettings.WebMail.Outlook365Settings.OAuth2.RefreshTokenLifeSpan = 365
    EmailSettings.ConnectionSettings.EnableMultiPlugin = False
    EmailSettings.ConnectionSettings.ConnectionStatusInfo = [csiStatusBar]
    LanguageID = 'Default'
    OpenFile = False
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = True
    ThumbnailSettings.Enabled = True
    ThumbnailSettings.Visible = True
    ThumbnailSettings.DeadSpace = 30
    ThumbnailSettings.PageHighlight.Width = 3
    ThumbnailSettings.ThumbnailSize = tsSmall
    PDFSettings.EmbedFontOptions = [efUseSubset]
    PDFSettings.EncryptSettings.AllowCopy = True
    PDFSettings.EncryptSettings.AllowInteract = True
    PDFSettings.EncryptSettings.AllowModify = True
    PDFSettings.EncryptSettings.AllowPrint = True
    PDFSettings.EncryptSettings.AllowExtract = True
    PDFSettings.EncryptSettings.AllowAssemble = True
    PDFSettings.EncryptSettings.AllowQualityPrint = True
    PDFSettings.EncryptSettings.Enabled = False
    PDFSettings.EncryptSettings.KeyLength = kl40Bit
    PDFSettings.EncryptSettings.EncryptionType = etRC4
    PDFSettings.DigitalSignatureSettings.SignPDF = False
    PDFSettings.FontEncoding = feAnsi
    PDFSettings.ImageCompressionLevel = 25
    PDFSettings.PDFAFormat = pafNone
    PDFSettings.Layers = True
    PDFSettings.Outline = True
    PreviewFormSettings.PageBorder.mmPadding = 0
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    RTFSettings.AppName = 'ReportBuilder'
    RTFSettings.Author = 'ReportBuilder'
    RTFSettings.DefaultFont.Charset = DEFAULT_CHARSET
    RTFSettings.DefaultFont.Color = clWindowText
    RTFSettings.DefaultFont.Height = -13
    RTFSettings.DefaultFont.Name = 'Arial'
    RTFSettings.DefaultFont.Style = []
    RTFSettings.Title = 'Report'
    TextFileName = '($MyDocuments)\Report.pdf'
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = True
    XLSSettings.AppName = 'ReportBuilder'
    XLSSettings.Author = 'ReportBuilder'
    XLSSettings.Subject = 'Report'
    XLSSettings.Title = 'Report'
    XLSSettings.WorksheetName = 'Report'
    CloudDriveSettings.DropBoxSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.DropBoxSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.DropBoxSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.DropBoxSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.DropBoxSettings.DirectorySupport = True
    CloudDriveSettings.DropBoxSettings.SharedResources = True
    CloudDriveSettings.GoogleDriveSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.GoogleDriveSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.GoogleDriveSettings.DirectorySupport = False
    CloudDriveSettings.GoogleDriveSettings.SharedResources = False
    CloudDriveSettings.OneDriveSettings.OAuth2.AuthStorage = [oasAccessToken, oasRefreshToken]
    CloudDriveSettings.OneDriveSettings.OAuth2.RedirectURI = 'http://localhost'
    CloudDriveSettings.OneDriveSettings.OAuth2.RedirectPort = 0
    CloudDriveSettings.OneDriveSettings.OAuth2.RefreshTokenLifeSpan = 365
    CloudDriveSettings.OneDriveSettings.DirectorySupport = True
    CloudDriveSettings.OneDriveSettings.SharedResources = True
    Left = 496
    Top = 264
    Version = '23.0'
    mmColumnWidth = 0
    DataPipelineName = 'ppDBPipeline1'
    object ppHeaderBand1: TppHeaderBand
      Border.mmPadding = 0
      mmBottomOffset = 0
      mmHeight = 45773
      mmPrintPosition = 0
      object ppShape1: TppShape
        DesignLayer = ppDesignLayer2
        UserName = 'Shape1'
        Brush.Style = bsClear
        mmHeight = 31750
        mmLeft = 12700
        mmTop = 6879
        mmWidth = 256382
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel1: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label1'
        OnGetText = ppLabel1GetText
        Border.mmPadding = 0
        Caption = 'Label1'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 11
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 4763
        mmLeft = 136261
        mmTop = 10583
        mmWidth = 12435
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel2: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label2'
        Border.mmPadding = 0
        Caption = #1711#1586#1575#1585#1588' '#1605#1589#1608#1576#1575#1578' '#1576#1608#1583#1580#1607
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 11
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 4763
        mmLeft = 127265
        mmTop = 18521
        mmWidth = 30163
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel3: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label3'
        Border.mmPadding = 0
        Caption = #1606#1575#1605' '#1588#1585#1705#1578' :'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 250032
        mmTop = 26723
        mmWidth = 13229
        BandType = 0
        LayerName = Foreground1
      end
      object ppDBText1: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText1'
        AutoSize = True
        Border.mmPadding = 0
        DataField = '_ComponyName'
        DataPipeline = ppDBPipeline1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3969
        mmLeft = 219605
        mmTop = 26723
        mmWidth = 26458
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel4: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label4'
        Border.mmPadding = 0
        Caption = #1606#1608#1593' '#1605#1589#1608#1576' :'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4022
        mmLeft = 250063
        mmTop = 33073
        mmWidth = 15579
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel5: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label5'
        Border.mmPadding = 0
        Caption = #1588#1605#1575#1585#1607' '#1605#1589#1608#1576' :'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 38100
        mmTop = 26723
        mmWidth = 17992
        BandType = 0
        LayerName = Foreground1
      end
      object ppDBText3: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText3'
        AutoSize = True
        Border.mmPadding = 0
        DataField = 'BudgetSerial'
        DataPipeline = ppDBPipeline1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3969
        mmLeft = 14817
        mmTop = 26723
        mmWidth = 20108
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel6: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label6'
        Border.mmPadding = 0
        Caption = #1578#1575#1585#1740#1582' '#1605#1589#1608#1576' :'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 38100
        mmTop = 33073
        mmWidth = 17463
        BandType = 0
        LayerName = Foreground1
      end
      object ppDBText4: TppDBText
        DesignLayer = ppDesignLayer2
        UserName = 'DBText4'
        AutoSize = True
        Border.mmPadding = 0
        DataField = 'BudgetDate'
        DataPipeline = ppDBPipeline1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBPipeline1'
        mmHeight = 3969
        mmLeft = 16404
        mmTop = 33073
        mmWidth = 18521
        BandType = 0
        LayerName = Foreground1
      end
      object ppShape2: TppShape
        DesignLayer = ppDesignLayer2
        UserName = 'Shape2'
        Brush.Style = bsClear
        mmHeight = 6085
        mmLeft = 12700
        mmTop = 39688
        mmWidth = 256382
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel7: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label7'
        Border.mmPadding = 0
        Caption = #1705#1583#1576#1608#1583#1580#1607
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 258234
        mmTop = 41010
        mmWidth = 8731
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel8: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label8'
        Border.mmPadding = 0
        Caption = #1606#1575#1605' '#1576#1608#1583#1580#1607
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 237067
        mmTop = 41010
        mmWidth = 9790
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel9: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label9'
        Border.mmPadding = 0
        Caption = #1605#1581#1604' '#1578#1575#1605#1740#1606
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 201348
        mmTop = 41010
        mmWidth = 10848
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel10: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label10'
        Border.mmPadding = 0
        Caption = #1606#1602#1583#1740'/'#1594#1740#1585#1606#1602#1583#1740
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 171186
        mmTop = 41010
        mmWidth = 14552
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel11: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label11'
        Border.mmPadding = 0
        Caption = #1606#1608#1593' '#1605#1576#1604#1594
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 159809
        mmTop = 41010
        mmWidth = 9260
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel12: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label12'
        Border.mmPadding = 0
        Caption = #1605#1576#1604#1594
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 144198
        mmTop = 41010
        mmWidth = 4498
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel13: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label13'
        Border.mmPadding = 0
        Caption = #1588#1585#1581
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 38629
        mmTop = 41010
        mmWidth = 5027
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine1: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line1'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 255853
        mmTop = 39688
        mmWidth = 1058
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine2: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line2'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 227013
        mmTop = 39688
        mmWidth = 2381
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine3: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line3'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 186796
        mmTop = 39688
        mmWidth = 2381
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine4: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line4'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 170392
        mmTop = 39688
        mmWidth = 2381
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine5: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line5'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 158750
        mmTop = 39688
        mmWidth = 2381
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine6: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line6'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 131498
        mmTop = 39688
        mmWidth = 2381
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine13: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line13'
        Border.mmPadding = 0
        Weight = 0.750000000000000000
        mmHeight = 1323
        mmLeft = 12700
        mmTop = 24606
        mmWidth = 256382
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel14: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label14'
        Border.mmPadding = 0
        Caption = #1705#1583' '#1588#1582#1589
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3598
        mmLeft = 119327
        mmTop = 41010
        mmWidth = 10075
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel15: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label15'
        Border.mmPadding = 0
        Caption = #1606#1575#1605' '#1588#1582#1589
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 96309
        mmTop = 41010
        mmWidth = 10319
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel16: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label16'
        Border.mmPadding = 0
        Caption = #1606#1608#1593' '#1578#1593#1607#1583
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 70115
        mmTop = 41010
        mmWidth = 8996
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine7: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line7'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 116417
        mmTop = 39688
        mmWidth = 1852
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine8: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line8'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 82021
        mmTop = 39688
        mmWidth = 1852
        BandType = 0
        LayerName = Foreground1
      end
      object ppLine9: TppLine
        DesignLayer = ppDesignLayer2
        UserName = 'Line9'
        Border.mmPadding = 0
        Position = lpLeft
        Weight = 0.750000000000000000
        mmHeight = 6085
        mmLeft = 67733
        mmTop = 39688
        mmWidth = 1588
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel17: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label17'
        OnGetText = ppLabel17GetText
        Border.mmPadding = 0
        Caption = 'Label17'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 9
        Font.Style = [fsBold]
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3810
        mmLeft = 136531
        mmTop = 26723
        mmWidth = 11896
        BandType = 0
        LayerName = Foreground1
      end
      object ppLabel18: TppLabel
        DesignLayer = ppDesignLayer2
        UserName = 'Label18'
        OnGetText = ppLabel18GetText
        Border.mmPadding = 0
        Caption = 'Label18'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Badr'
        Font.Size = 10
        Font.Style = []
        FormFieldSettings.FormSubmitInfo.SubmitMethod = fstPost
        FormFieldSettings.FormFieldType = fftNone
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 233628
        mmTop = 33073
        mmWidth = 12435
        BandType = 0
        LayerName = Foreground1
      end
    end
    object ppDetailBand1: TppDetailBand
      Border.mmPadding = 0
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 7408
      mmPrintPosition = 0
      object ppSubReport1: TppSubReport
        DesignLayer = ppDesignLayer2
        UserName = 'SubReport1'
        ExpandAll = False
        NewPrintJob = False
        OutlineSettings.CreateNode = True
        TraverseAllData = False
        DataPipelineName = 'ppDBPipeline2'
        mmHeight = 7408
        mmLeft = 0
        mmTop = 0
        mmWidth = 284300
        BandType = 4
        LayerName = Foreground1
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        mmMinHeight = 0
        object ppChildReport1: TppChildReport
          AutoStop = False
          DataPipeline = ppDBPipeline2
          PrinterSetup.BinName = 'Default'
          PrinterSetup.DocumentName = 'Report'
          PrinterSetup.Duplex = dpNone
          PrinterSetup.Orientation = poLandscape
          PrinterSetup.PaperName = 'A4'
          PrinterSetup.PrinterName = 'Default'
          PrinterSetup.SaveDeviceSettings = False
          PrinterSetup.mmMarginBottom = 6350
          PrinterSetup.mmMarginLeft = 6350
          PrinterSetup.mmMarginRight = 6350
          PrinterSetup.mmMarginTop = 6350
          PrinterSetup.mmPaperHeight = 210000
          PrinterSetup.mmPaperWidth = 297000
          PrinterSetup.PaperSize = 9
          Version = '23.0'
          mmColumnWidth = 0
          DataPipelineName = 'ppDBPipeline2'
          object ppDetailBand2: TppDetailBand
            Border.mmPadding = 0
            mmBottomOffset = 0
            mmHeight = 6085
            mmPrintPosition = 0
            object ppShape3: TppShape
              DesignLayer = ppDesignLayer1
              UserName = 'Shape3'
              Brush.Style = bsClear
              mmHeight = 6085
              mmLeft = 12700
              mmTop = 0
              mmWidth = 256287
              BandType = 4
              LayerName = Foreground
            end
            object ppLine10: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line10'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 255853
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppLine11: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line11'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 227013
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppLine12: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line12'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 186796
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppLine14: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line101'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 170392
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppLine15: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line15'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 158750
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppLine16: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line16'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 67733
              mmTop = 0
              mmWidth = 1852
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText5: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText5'
              AutoSize = True
              Border.mmPadding = 0
              DataField = 'BudgetTopicID'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 260615
              mmTop = 1058
              mmWidth = 7673
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText6: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText6'
              AutoSize = True
              Border.mmPadding = 0
              DataField = '_BudjectName'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 231775
              mmTop = 1058
              mmWidth = 20902
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText7: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText7'
              AutoSize = True
              Border.mmPadding = 0
              DataField = '_Companyname'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 196057
              mmTop = 1058
              mmWidth = 23283
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText8: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText8'
              OnGetText = ppDBText8GetText
              Border.mmPadding = 0
              DataField = 'Cashtype'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 171450
              mmTop = 1058
              mmWidth = 14552
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText9: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText9'
              OnGetText = ppDBText9GetText
              Border.mmPadding = 0
              DataField = 'Bes'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 159809
              mmTop = 1058
              mmWidth = 9790
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText10: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText10'
              AutoSize = True
              Border.mmPadding = 0
              DataField = 'Bed'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 145257
              mmTop = 1058
              mmWidth = 5556
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText11: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText11'
              Border.mmPadding = 0
              DataField = 'ItemNote_L1'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 14023
              mmTop = 1058
              mmWidth = 52917
              BandType = 4
              LayerName = Foreground
            end
            object ppLine17: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line17'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 131498
              mmTop = 0
              mmWidth = 2381
              BandType = 4
              LayerName = Foreground
            end
            object ppLine18: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line18'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 116417
              mmTop = 0
              mmWidth = 2381
              BandType = 4
              LayerName = Foreground
            end
            object ppLine19: TppLine
              DesignLayer = ppDesignLayer1
              UserName = 'Line19'
              Border.mmPadding = 0
              Position = lpLeft
              Weight = 0.750000000000000000
              mmHeight = 6085
              mmLeft = 82021
              mmTop = 0
              mmWidth = 2381
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText12: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText12'
              AutoSize = True
              Border.mmPadding = 0
              DataField = 'CustomerID'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 119592
              mmTop = 1058
              mmWidth = 11113
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText13: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText13'
              AutoSize = True
              Border.mmPadding = 0
              DataField = '_CustomerName'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3598
              mmLeft = 91546
              mmTop = 1058
              mmWidth = 24130
              BandType = 4
              LayerName = Foreground
            end
            object ppDBText14: TppDBText
              DesignLayer = ppDesignLayer1
              UserName = 'DBText14'
              OnGetText = ppDBText14GetText
              Border.mmPadding = 0
              DataField = 'Ctype'
              DataPipeline = ppDBPipeline2
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Badr'
              Font.Size = 9
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBPipeline2'
              mmHeight = 3704
              mmLeft = 68792
              mmTop = 1058
              mmWidth = 12171
              BandType = 4
              LayerName = Foreground
            end
          end
          object ppDesignLayers1: TppDesignLayers
            object ppDesignLayer1: TppDesignLayer
              UserName = 'Foreground'
              LayerType = ltBanded
              Index = 0
            end
          end
        end
      end
    end
    object ppDesignLayers2: TppDesignLayers
      object ppDesignLayer2: TppDesignLayer
        UserName = 'Foreground1'
        LayerType = ltBanded
        Index = 0
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
  object PopPrint: TPopupMenu
    Left = 352
    Top = 625
    object N1: TMenuItem
      Caption = #1585#1705#1608#1585#1583' '#1580#1575#1585#1740
      OnClick = N1Click
    end
    object N2: TMenuItem
      Tag = 1
      Caption = #1607#1605#1607
      OnClick = N1Click
    end
  end
  object qrylookup: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT ReciptType, ReciptCaption FROM ReciptTypes WHERE (ReciptT' +
        'ype IN ($))')
    Left = 400
    Top = 200
    object qrylookupReciptType: TWordField
      FieldName = 'ReciptType'
    end
    object qrylookupReciptCaption: TStringField
      FieldName = 'ReciptCaption'
      Size = 70
    end
  end
  object srcLookUp: TDataSource
    DataSet = qrylookup
    Left = 456
    Top = 200
  end
end
