inherited CTopic2_AllRelationF: TCTopic2_AllRelationF
  Left = 292
  Top = 149
  Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1705#1583' '#1578#1601#1589#1740#1604#1740'3'
  ClientHeight = 371
  ClientWidth = 524
  OnResize = FormResize
  ExplicitWidth = 540
  ExplicitHeight = 410
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 314
    Width = 516
    ExplicitTop = 326
    ExplicitWidth = 532
  end
  inherited Panel1: TPanel
    Width = 524
    Color = clWindow
    ExplicitWidth = 532
    inherited Image1: TImage
      Left = 478
      Picture.Data = {
        
        1515}
      ExplicitLeft = 478
    end
    inherited lblTopic0: TLabel
      Left = 320
      Width = 138
      Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1578#1601#1589#1610#1604#1610'2'#39' 2'
      ExplicitLeft = 320
      ExplicitWidth = 138
    end
    inherited lblTopic1: TLabel
      Left = 162
      Width = 283
      Caption = #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583'.'
      ExplicitLeft = 162
      ExplicitWidth = 283
    end
    object Image3: TImage
      Left = 128
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 104
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 334
    Width = 524
    ExplicitTop = 346
    ExplicitWidth = 532
    inherited btnClose: TBitBtn
      Caption = #1575#1606#1589#1585#1575#1601
      Glyph.Data = {00000000}
      ModalResult = 2
      TabOrder = 5
    end
    object BitBtn1: TBitBtn
      Left = 454
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      ModalResult = 1
      TabOrder = 0
    end
    object BitBtn6: TBitBtn
      Left = 377
      Top = 6
      Width = 75
      Height = 25
      Action = actAll
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      TabOrder = 1
    end
    object BitBtn7: TBitBtn
      Left = 300
      Top = 6
      Width = 75
      Height = 25
      Action = actNotAll
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      TabOrder = 2
    end
    object BitBtn4: TBitBtn
      Left = 162
      Top = 6
      Width = 75
      Height = 25
      Action = actSearch
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 3
    end
    object BitBtn3: TBitBtn
      Left = 85
      Top = 6
      Width = 75
      Height = 25
      Action = actSort
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 4
    end
  end
  object DBGrid1: TDBGrid [3]
    Left = 0
    Top = 73
    Width = 524
    Height = 261
    Align = alClient
    Color = clCream
    DataSource = src_CTopic2
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnKeyPress = DBGrid1KeyPress
    Columns = <
      item
        Expanded = False
        FieldName = '_Checked'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CTopicCode2'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CTopicName2_L1'
        Width = 293
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CTopicName2_L2'
        Width = 5
        Visible = True
      end>
  end
  inherited ActionList1: TActionList
    object actAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      OnExecute = actAllExecute
    end
    object actNotAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      OnExecute = actNotAllExecute
    end
    object actSearch: TAction
      Caption = #1580#1587#1578#1580#1608
      OnExecute = actSearchExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
  end
  object qry_CTopic2: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      
        'SELECT acc.CenterTopics2.CTopicCode2, acc.CenterTopics2.CTopicNa' +
        'me2_L1,'
      '          acc.CenterTopics2.CTopicName2_L2'
      'FROM acc.CenterTopics2 LEFT OUTER JOIN'
      
        '         acc.CTopics2FORUse ON acc.CenterTopics2.CTopicCode2 = a' +
        'cc.CTopics2FORUse.PrvTopicCode'
      
        'WHERE (acc.CenterTopics2.CTopicCode2 <> 0) AND (acc.CTopics2FORU' +
        'se.PrvTopicCode IS NULL)'
      'ORDER BY acc.CenterTopics2.CTopicCode2'
      '')
    Left = 24
    Top = 96
  end
  object prv_Ctopic2: TDataSetProvider
    DataSet = qry_CTopic2
    Left = 23
    Top = 149
  end
  object cli_CTopic2: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'prv_Ctopic2'
    Left = 22
    Top = 205
    object cli_CTopic2_Checked: TBooleanField
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      DisplayWidth = 6
      FieldKind = fkInternalCalc
      FieldName = '_Checked'
    end
    object cli_CTopic2CTopicCode2: TIntegerField
      DisplayLabel = #1705#1583
      FieldName = 'CTopicCode2'
    end
    object cli_CTopic2CTopicName2_L1: TStringField
      DisplayLabel = #1593#1606#1608#1575#1606' '#1605#1585#1705#1586#1607#1586#1740#1606#1607' 2'
      FieldName = 'CTopicName2_L1'
      Size = 120
    end
    object cli_CTopic2CTopicName2_L2: TStringField
      DisplayLabel = 'CenterTopic2 Name'
      FieldName = 'CTopicName2_L2'
      Size = 120
    end
  end
  object src_CTopic2: TDataSource
    DataSet = cli_CTopic2
    Left = 24
    Top = 261
  end
  object qry_CenterTopicAllRange: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
      item
        Name = 'FilterCode'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      'SELECT  *  FROM  Acc.CenterTopicAllRange'
      'WHERE   (TopicCode = :FilterCode)')
    Left = 408
    Top = 152
    object qry_CenterTopicAllRangeTopicCode: TLargeintField
      FieldName = 'TopicCode'
    end
    object qry_CenterTopicAllRangeCTopicCode2: TIntegerField
      FieldName = 'CTopicCode2'
    end
  end
end
