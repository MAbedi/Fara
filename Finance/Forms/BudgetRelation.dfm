inherited BudgetRelationF: TBudgetRelationF
  Left = 397
  Top = 179
  Caption = #1575#1606#1578#1582#1575#1576' '#1576#1608#1583#1580#1607
  ClientHeight = 327
  ClientWidth = 424
  ExplicitWidth = 440
  ExplicitHeight = 366
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 270
    Width = 416
    ExplicitTop = 282
    ExplicitWidth = 432
  end
  inherited Panel1: TPanel
    Width = 424
    ExplicitWidth = 432
    inherited Image1: TImage
      Left = 378
      ExplicitLeft = 378
    end
    inherited lblTopic0: TLabel
      Left = 290
      Width = 68
      Caption = #1575#1606#1578#1582#1575#1576' '#1576#1608#1583#1580#1607
      ExplicitLeft = 290
      ExplicitWidth = 68
    end
    inherited lblTopic1: TLabel
      Left = 55
      Width = 307
      Caption = 
        #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1576#1606#1711#1575#1607' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583 +
        '.'
      ExplicitLeft = 55
      ExplicitWidth = 307
    end
    object Image3: TImage
      Left = 120
      Top = 8
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 96
      Top = 8
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 290
    Width = 424
    ExplicitTop = 302
    ExplicitWidth = 432
    object BitBtn1: TBitBtn
      Left = 352
      Top = 8
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      ModalResult = 1
      TabOrder = 1
    end
    object BitBtn2: TBitBtn
      Left = 272
      Top = 8
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1575#1606#1589#1600#1600#1600#1585#1575#1601
      ModalResult = 2
      TabOrder = 2
    end
  end
  object DBGrid1: TDBGrid [3]
    Left = 0
    Top = 73
    Width = 424
    Height = 217
    Align = alClient
    Color = clCream
    DataSource = srcSelect
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    Columns = <
      item
        Expanded = False
        FieldName = '_Checked'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BudgetTopicID'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BudgetCaption_L1'
        Width = 189
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BudgetCaption_L2'
        Visible = True
      end>
  end
  object cliSelect: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DataSetProvider1'
    Left = 40
    Top = 144
    object cliSelectBudgetTopicID: TIntegerField
      DisplayLabel = #1603#1583' '#1576#1608#1583#1580#1607
      FieldName = 'BudgetTopicID'
    end
    object cliSelect_Selected: TBooleanField
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      FieldKind = fkInternalCalc
      FieldName = '_Checked'
    end
    object cliSelectBudgetCaption_L1: TStringField
      DisplayLabel = #1593#1606#1608#1575#1606
      FieldName = 'BudgetCaption_L1'
      Size = 100
    end
    object cliSelectBudgetCaption_L2: TStringField
      DisplayLabel = 'Budget  Caption'
      FieldName = 'BudgetCaption_L2'
      Size = 100
    end
  end
  object srcSelect: TDataSource
    DataSet = cliSelect
    Left = 32
    Top = 208
  end
  object DataSetProvider1: TDataSetProvider
    DataSet = qry_Select
    Left = 104
    Top = 184
  end
  object qry_Select: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT     dbo.BudgetTopics.BudgetTopicID, dbo.BudgetTopics.Budg' +
        'etCaption_L1, dbo.BudgetTopics.BudgetCaption_L2'
      'FROM         dbo.BudgetTopics LEFT OUTER JOIN'
      
        '                      dbo.BudgetTopicsForUse ON dbo.BudgetTopics' +
        '.BudgetTopicID = dbo.BudgetTopicsForUse.PrvBudgetTopicID'
      'WHERE     (dbo.BudgetTopicsForUse.PrvBudgetTopicID IS NULL)')
    Left = 40
    Top = 80
    object qry_SelectBudgetTopicID: TIntegerField
      FieldName = 'BudgetTopicID'
    end
    object qry_SelectBudgetCaption_L1: TStringField
      FieldName = 'BudgetCaption_L1'
      Size = 100
    end
    object qry_SelectBudgetCaption_L2: TStringField
      DisplayLabel = 'Budget  Caption'
      FieldName = 'BudgetCaption_L2'
      Size = 100
    end
  end
  object qry_ProjectsRange: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
      item
        Name = 'ProjectID'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Value = 11
      end>
    SQL.Strings = (
      'SELECT  *  FROM   ProjectsRange      '
      'WHERE     (ProjectsRange.ProjectID = :ProjectID)')
    Left = 296
    Top = 176
    object qry_ProjectsRangeProjectID: TIntegerField
      FieldName = 'ProjectID'
    end
    object qry_ProjectsRangeBudgetTopicID: TIntegerField
      FieldName = 'BudgetTopicID'
    end
  end
end
