inherited Topic_RelationF: TTopic_RelationF
  Left = 286
  Top = 184
  ActiveControl = DBGrid1
  Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1581#1587#1575#1576
  ClientHeight = 371
  ClientWidth = 541
  OnResize = FormResize
  ExplicitWidth = 557
  ExplicitHeight = 410
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 314
    Width = 533
    ExplicitTop = 326
    ExplicitWidth = 549
  end
  inherited Panel1: TPanel
    Width = 541
    ExplicitWidth = 549
    inherited Image1: TImage
      Left = 495
      Picture.Data = {
        
        1515}
      ExplicitLeft = 495
    end
    inherited lblTopic0: TLabel
      Left = 364
      Width = 111
      Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1581#1587#1575#1576
      ExplicitLeft = 364
      ExplicitWidth = 111
    end
    inherited lblTopic1: TLabel
      Left = 179
      Width = 283
      Caption = #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583'.'
      ExplicitLeft = 179
      ExplicitWidth = 283
    end
    object Image3: TImage
      Left = 128
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 104
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 334
    Width = 541
    ExplicitTop = 346
    ExplicitWidth = 549
    inherited btnClose: TBitBtn
      Left = 7
      ModalResult = 2
      TabOrder = 6
      ExplicitLeft = 7
    end
    object BitBtn1: TBitBtn
      Left = 468
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      Default = True
      ModalResult = 1
      TabOrder = 0
    end
    object BitBtn2: TBitBtn
      Left = 391
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Cancel = True
      Caption = #1575#1606#1589#1600#1600#1600#1585#1575#1601
      ModalResult = 2
      TabOrder = 1
    end
    object Button1: TButton
      Left = 314
      Top = 6
      Width = 75
      Height = 25
      Action = actSlelectAll
      TabOrder = 2
    end
    object Button2: TButton
      Left = 237
      Top = 6
      Width = 75
      Height = 25
      Action = actSlelectInverse
      TabOrder = 3
    end
    object Button3: TButton
      Left = 83
      Top = 6
      Width = 75
      Height = 25
      Action = actSort
      TabOrder = 5
    end
    object Button4: TButton
      Left = 160
      Top = 6
      Width = 75
      Height = 25
      Action = actSearch
      TabOrder = 4
    end
  end
  object DBGrid1: TDBGrid [3]
    Tag = 111
    Left = 0
    Top = 73
    Width = 541
    Height = 261
    Align = alClient
    Color = clCream
    DataSource = src_AccCode
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnKeyPress = DBGrid1KeyPress
    Columns = <
      item
        Expanded = False
        FieldName = '_Checked'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'TopicCode'
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MoeenName_L1'
        Width = 240
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MoeenName_L2'
        Width = 186
        Visible = True
      end>
  end
  inherited ActionList1: TActionList
    object actSlelectInverse: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      OnExecute = actSlelectInverseExecute
    end
    object actSlelectAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      OnExecute = actSlelectAllExecute
    end
    object actSearch: TAction
      Caption = #1580#1587#1578#1580#1608
      OnExecute = actSearchExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
  end
  object qry_AccCode: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltReadOnly
    AfterOpen = qry_AccCodeAfterOpen
    Parameters = <
      item
        Name = 'YaerID'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT  cast(acc.Categories.TopicCode as int) as TopicCode , acc' +
        '.Categories.MoeenName_L1 ,acc.Categories.MoeenName_L2'
      'FROM         acc.Categories LEFT OUTER JOIN'
      
        '                   acc.CategoriesForUse ON acc.Categories.TopicC' +
        'ode = acc.CategoriesForUse.PrvTopicCode'
      
        'WHERE     (acc.CategoriesForUse.PrvTopicCode IS NULL)  And (Topi' +
        'cCode not in (SELECT DISTINCT TopicCode  FROM Acc.Documents  WHE' +
        'RE (DetailCode <> 0) AND (YearID =:YaerID)))'
      ''
      '')
    Left = 80
    Top = 88
  end
  object prv_AccCode: TDataSetProvider
    DataSet = qry_AccCode
    Options = [poAutoRefresh]
    Left = 39
    Top = 141
  end
  object cli_AccCode: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'TopicCode'
        Attributes = [faReadonly]
        DataType = ftInteger
      end
      item
        Name = 'MoeenName_L1'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'MoeenName_L2'
        DataType = ftString
        Size = 100
      end
      item
        Name = '_Checked'
        DataType = ftBoolean
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'prv_AccCode'
    StoreDefs = True
    Left = 56
    Top = 181
    object cli_AccCode_Checked: TBooleanField
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      FieldKind = fkInternalCalc
      FieldName = '_Checked'
    end
    object cli_AccCode_HasTopicCode: TBooleanField
      FieldKind = fkLookup
      FieldName = '_HasTopicCode'
      LookupDataSet = qry_DetailRange
      LookupKeyFields = 'TopicCode'
      LookupResultField = 'TopicCode'
      KeyFields = 'TopicCode'
      Lookup = True
    end
    object cli_AccCodeTopicCode: TIntegerField
      DisplayLabel = #1603#1583' '#1605#1593#1740#1606
      FieldName = 'TopicCode'
      ReadOnly = True
    end
    object cli_AccCodeMoeenName_L1: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576
      FieldName = 'MoeenName_L1'
      Size = 100
    end
    object cli_AccCodeMoeenName_L2: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576' 2'
      FieldName = 'MoeenName_L2'
      ReadOnly = True
      Size = 100
    end
  end
  object src_AccCode: TDataSource
    DataSet = cli_AccCode
    Left = 64
    Top = 237
  end
  object qry_DetailRange: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
      item
        Name = 'FilterCode'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 1
      end
      item
        Name = 'CompanyCode'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 1
      end>
    SQL.Strings = (
      'SELECT  *  FROM  acc.DetailRange'
      'WHERE  (DetailCode = :FilterCode)'
      '        AND CompanyCode=:CompanyCode')
    Left = 256
    Top = 152
  end
end
