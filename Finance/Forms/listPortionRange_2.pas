...
type
  TlistPortionRange_2F = class(Ttemplate2MDIF)
    Panel1: TPanel;
    DBGrid1: TDBGrid;
    ...
    DBEdit1: TDBEdit;
    SumGrid1: TSumGrid;
    qryCtopicPortionCTopicName2_L1: TStringField;
    qryCtopicPortionCTopicCode2: TIntegerField;
    qryCtopicPortionCTopicName2_L2: TStringField;
...
var
  listPortionRange_2F: TlistPortionRange_2F;
...
procedure TlistPortionRange_2F.FormCreate(Sender: TObject);
begin
  inherited;
  createToolButton;
...
procedure TlistPortionRange_2F.FormDestroy(Sender: TObject);
begin
  inherited;
  SaveColWidth(DBGrid1);
end;

procedure TlistPortionRange_2F.FormResize(Sender: TObject);
begin
  inherited;
  SetColSize(DBGrid1, 5, true);
end;

procedure TlistPortionRange_2F.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qrylistPortion);
end;

procedure TlistPortionRange_2F.actExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TlistPortionRange_2F.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qrylistPortion);
end;

procedure TlistPortionRange_2F.UpdateList;

begin
  with qrylistPortion Do
  begin
    Active := False;
...
procedure TlistPortionRange_2F.FormShow(Sender: TObject);
begin
  inherited;
  myParams.Clear;
  actFilter.Execute;
...
procedure TlistPortionRange_2F.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TlistPortionRange_2F.ToolBar1CustomDrawButton(Sender: TToolBar;
...
procedure TlistPortionRange_2F.updateQry;
begin
  makeqry;
  UpdateList;
end;

procedure TlistPortionRange_2F.createToolButton;
var
  i: Integer;
const
...
procedure TlistPortionRange_2F.makeqry;
var
  i: Integer;
  SqlSum, SubSqlSum: string;
begin
  i := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while not Eof do
        begin
          qrylistPortion.SQL.Add(',sum(case when acc.PortionTable.FormInfoID=' +
            qryFormsinfo.FieldByName('FormInfoID').AsString +
            ' and Acc.PortionTableItems.CTopicCode2 =' +
            FieldByName('CTopicCode2').AsString +
            ' and acc.PortionTable.PortionCunt=' + IntToStr(counter) +
            ' then acc.PortionTableItems.CofficientItem  else 0 end) AS [' +
            '÷—Ì» ' + FieldByName('CTopicName2_L1').AsString +
            qryFormsinfo.FieldByName('FormInfoID').AsString + ']');

          SubSqlSum := SubSqlSum + ',sum(case when acc.PortionTable.FormInfoID='
            + qryFormsinfo.FieldByName('FormInfoID').AsString +
            ' and Acc.PortionTableItems.CTopicCode2=' +
            FieldByName('CTopicCode2').AsString +
            ' and acc.PortionTable.PortionCunt=' + IntToStr(counter) +
            ' then acc.PortionTableItems.CofficientItem else 0 end) AS C' +
            IntToStr(i);
          SqlSum := SqlSum + ',sum(C' + IntToStr(i) + ')';

          qrylistPortion.SQL.Add
            (',Round(sum(case when acc.PortionTable.FormInfoID=' +
            qryFormsinfo.FieldByName('FormInfoID').AsString +
            ' and Acc.PortionTableItems.CTopicCode2=' +
            FieldByName('CTopicCode2').AsString +
            ' and acc.PortionTable.PortionCunt=' + IntToStr(counter) +
            ' then acc.PortionTableItems.CofficientItem else 0 end)' +
            '*(remain)' + '/sum(case when (acc.PortionTable.FormInfoID= ' +
            qryFormsinfo.FieldByName('FormInfoID').AsString +
            ' and acc.PortionTable.PortionCunt=' + IntToStr(counter) +
            ') then  acc.PortionTableItems.CofficientItem else -1  end),0) as ['
          ...
          SubSqlSum := SubSqlSum +
            ...
            qryFormsinfo.FieldByName('FormInfoID').AsString +
            ' and acc.PortionTable.PortionCunt=' + IntToStr(counter) +
            ') then acc.PortionTableItems.CofficientItem else -1  end),0) as r'
            + IntToStr(i);
    ...
    SQL.Add(' FROM  Acc.PortionTableItems INNER JOIN  Acc.PortionTable ON Acc.PortionTableItems.CofficientID = Acc.PortionTable.CofficientID RIGHT OUTER JOIN ');
    SQL.Add('( SELECT  acc.Documents.YearID , acc.Documents.TopicCode, acc.Documents.DetailCode, acc.Documents.CTopicCode, acc.Documents.CTopicCode2,');
    ...
    SQL.Add(')subSql ON Acc.PortionTable.FormInfoID = subSql.FormInfoID AND Acc.PortionTable.YearID = subSql.YearID ');
    ...
    SQL.Add('(subSql.cTopicCode2 between :cTopicCode2From and :cTopicCode2To )and');
    SQL.Add('(DetailCode between :DetailCodeFrom and :DetailCodeTo)and');
    SQL.Add('(acc.PortionTable.PortionCunt =:PortionCunt)');

    SQL.Add('GROUP BY TopicCode,DetailCode,subSql.CTopicCode,subSql.CTopicCode2, InfoID,');
...
procedure TlistPortionRange_2F.DBGrid1ColEnter(Sender: TObject);
begin
  inherited;
...
procedure TlistPortionRange_2F.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TlistPortionRange_2F._actSearchExecute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qrylistPortion);
end;
