...
uses
  ...
  ppBands, ppClass, ppCtrls, ppReport, ppStrtch, ppSubRpt, ppVar, ppPrnabl,
  ppCache, ppDB, ppProd, ppComm, ppRelatv, ppDBPipe, ppModule, raCodMod,
  SumDBGrid, ppTypes, ppParameter, Math, ppDesignLayer, System.ImageList,
  System.Actions, AccFunctions, Filter_ADO_Const, DBGridEhGrouping, ToolCtrlsEh,
  DBGridEhToolCtrls, DynVarsEh, EhLibVCL, GridsEh, DBAxisGridsEh, DBGridEh,
  CedarDbGrid;

type
  TRpt_CtopicsOnDetailsBookF = class(Ttemplate2MDIF)
    qry_Doc: TADOQuery;
...
var
  Rpt_CtopicsOnDetailsBookF: TRpt_CtopicsOnDetailsBookF;
  cloned: TADOQuery;

implementation

uses Dm, GlobalPro, FilterClass_ADO, filter_ADO,
  Document, search2, sort2, searchCode_L1_L2, Resource, FormFunctions,
  FaraConsts;

{$R *.dfm}

procedure TRpt_CtopicsOnDetailsBookF.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.actFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.FormShow(Sender: TObject);
begin
  inherited;
  if showFilter then
  begin
...
procedure TRpt_CtopicsOnDetailsBookF.qry_AccCodeAfterScroll(DataSet: TDataSet);
...
procedure TRpt_CtopicsOnDetailsBookF.qry_AccCodeBeforeOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.qry_CompaniesAfterScroll
  (DataSet: TDataSet);
var
...
procedure TRpt_CtopicsOnDetailsBookF.qry_DocAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DBGridCurrencyKind(DBGrid1, myParams,qry_Doc)
end;

procedure TRpt_CtopicsOnDetailsBookF.FormDestroy(Sender: TObject);
begin
  inherited;
  cloned.Free;
...
procedure TRpt_CtopicsOnDetailsBookF.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
  actShowForm.Execute
end;

// function TRpt_CtopicsOnDetailsBookF.CalcSumFileds(
...
procedure TRpt_CtopicsOnDetailsBookF.InitForm;
begin
  PnlCompany.Visible := gv_MultiCompany and opta.ActivePartCompany;

end;

procedure TRpt_CtopicsOnDetailsBookF.UpdateFilter;
begin
  with qry_Doc do
  begin
    Active := false;
...
//procedure TRpt_CtopicsOnDetailsBookF.Updateremain;
//var
//  c, bes, bed: Currency;
//  amount, i: Integer;
//begin
...
procedure TRpt_CtopicsOnDetailsBookF.BitBtn1Click(Sender: TObject);
var
  s: boolean;
...
procedure TRpt_CtopicsOnDetailsBookF.BitBtn2Click(Sender: TObject);
var
  s: boolean;
  Results: array [0 .. 2] of String;
  Txt: String;
begin
  ...
  Txt := 'select * from (SELECT  LTRIM(rTRIM(STR(acc.Documents.CTopicCode))) + '' - '' + LTRIM(RTRIM(STR(acc.Documents.DetailCode))) + '' - '' + RTRIM(LTRIM(STR(acc.Documents.TopicCode))) as acccode,'
    + '  acc.Categories.MoeenName_l1+ '' - ''+acc.Details.DetailName_L1+ '' - ''+acc.CenterTopics.CTopicName_l1 as AccName_l1'
    + ' ,acc.Categories.MoeenName_l2+ '' - ''+acc.Details.DetailName_L2+ '' - ''+acc.CenterTopics.CTopicName_l2 as AccName_l2'
    ...
    + ' acc.Categories ON acc.Documents.TopicCode = acc.Categories.TopicCode INNER JOIN '
    + ' acc.DocGroups ON acc.Documents.Serial = acc.DocGroups.Serial' +
  ...
  // ' AND (acc.Documents.DetailCode  BETWEEN '+GetcFrom(myParams.ParamValues['DetailCode'],ftString)+' AND '+GetcTo(myParams.ParamValues['DetailCode'],ftString)+ ') '+
  ...
  // and ( acc.DocGroups.Status<>'+IntToStr(param1)+')  AND (acc.DocGroups.Status<>'+IntToStr(param2)+')'+
    ' GROUP BY acc.CenterTopics.CTopicName_L1, acc.Documents.DetailCode, acc.Documents.CTopicCode,acc.Details.CompanyCode,'
    + ' acc.Documents.TopicCode,acc.Categories.MoeenName_L1, acc.Details.DetailName_L1,acc.CenterTopics.CTopicName_L2 ,acc.Categories.MoeenName_L2, acc.Details.DetailName_L2'
    + ')fun ';
...
procedure TRpt_CtopicsOnDetailsBookF.actShowFormExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.actSendExcelExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TRpt_CtopicsOnDetailsBookF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qry_Doc);
end;

procedure TRpt_CtopicsOnDetailsBookF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qry_Doc);
end;

procedure TRpt_CtopicsOnDetailsBookF.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.DBGrid1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  // if ((Key=vk_DOWN) and (Shift=[SSshift]))  then chooseColumnDown
  // else
...
procedure TRpt_CtopicsOnDetailsBookF.DBGrid1KeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  // if (Key=vk_UP) and (Shift=[SSshift])  then   chooseColumnUp
  // else
  // if (Key=vk_DOWN)or(Key=vk_up) then flag:=true
  // else flag:=false;
end;
...
//
// procedure TRpt_CtopicsOnDetailsBookF.chooseColumnDown;
// var
// i:integer;
...
// Prior;
// end;
/// /    StatusBar3.Panels[5].Text:=' تعداد سطرهاي انتخابي = '+IntToStr(DBGrid1.SelectedRows.Count+1);
/// /    StatusBar3.Panels[3].Text:='مقدار = '+IfThen(DBGrid1.Columns[6].Visible ,CurrToStrF(sumAmount,ffGeneral,0),'0');
...
// procedure TRpt_CtopicsOnDetailsBookF.chooseColumnUp;
// var
// i:integer;
// bed,bes,remain,amount:Currency;
// begin
...
procedure TRpt_CtopicsOnDetailsBookF.DBGrid1MouseUp(Sender: TObject;
...
procedure TRpt_CtopicsOnDetailsBookF.ppLblSeconNo_DocNoGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := 'از شماره موقت ' +
...
procedure TRpt_CtopicsOnDetailsBookF.ppLabel4GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := 'از شماره سند ' +
...
procedure TRpt_CtopicsOnDetailsBookF.ppLblDateGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.ppLblCompanyNameGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.ppLblPrintDateGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TRpt_CtopicsOnDetailsBookF.ppSysVarPageNoGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.N10Click(Sender: TObject);
begin
  inherited;
  try
...
procedure TRpt_CtopicsOnDetailsBookF.N11Click(Sender: TObject);
begin
  inherited;
  try
    qry_Companies.DisableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.N12Click(Sender: TObject);
begin
  inherited;
  try
    qry_Companies.DisableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop2Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetailsLan', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_3Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_secondery_budject', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_1Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_budject', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_4Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_secondery_aid', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_2Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_aid', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_5Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_secondery_lan_budject',
...
procedure TRpt_CtopicsOnDetailsBookF.pop_6Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_lan_budject', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_8Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_secondery_lan_aid', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.pop_7Click(Sender: TObject);
begin
  inherited;
  try
    qry_Doc.DisableControls;
    qry_AccCode.DisableControls;
    InitReportFile(ppReport1, 'Rpt_CtopicsOnDetails_lan_aid', DBGrid1,
      ppDBPipeline1);
  finally
    qry_Doc.EnableControls;
    qry_AccCode.EnableControls;
...
procedure TRpt_CtopicsOnDetailsBookF.actPrintExecute(Sender: TObject);
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.setVisibleColumns;
var
  id: Integer;
begin
  inherited;
...
procedure TRpt_CtopicsOnDetailsBookF.ppDBCalcnum2alphabetGetText
