...
uses
  ...
  ExtCtrls, DB, ADODB, Grids, DBGrids, ComCtrls, DBClient, Provider, Menus;

type
  TChooseItemBudgectFormF = class(TTemplate5F)
...
var
  ChooseItemBudgectFormF: TChooseItemBudgectFormF;

implementation

uses StrUtils, DM, GlobalPro, search, sort, Companies, mmessage, 
  sort2, search2;

{$R *.dfm}

function TChooseItemBudgectFormF.SelectChecks(var checkeds: string; const Fcode,Fcaption_l1,Fcaption_l2,tblName:String; Where:string='';
         ADC:  TADOConnection=nil): Boolean;
var
  r:  Int64;
begin
...
procedure TChooseItemBudgectFormF.FormResize(Sender: TObject);
begin
  inherited;
  SetColSize(DBGrid1,2);
end;

procedure TChooseItemBudgectFormF.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TChooseItemBudgectFormF.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TChooseItemBudgectFormF.qryCustomersAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TChooseItemBudgectFormF.DBGrid1CellClick(Column: TColumn);
begin
  inherited;
//  if Column.Index=0 then ToggleCheck;
end;

procedure TChooseItemBudgectFormF.ToggleCheck(CalculateSummary: boolean=True);
begin
  with cliCustomers do begin
    Edit;
...
procedure TChooseItemBudgectFormF.DBGrid1DblClick(Sender: TObject);
begin
  inherited;
  ToggleCheck;
end;

procedure TChooseItemBudgectFormF.DBGrid1KeyPress(Sender: TObject; var Key: Char);
//var
//  n:  Int64;
begin
  inherited;
...
procedure TChooseItemBudgectFormF.actSearchExecute(Sender: TObject);
begin
  inherited;
  searchF.ShowSearch(cliCustomers);
end;

procedure TChooseItemBudgectFormF.actSortExecute(Sender: TObject);
begin
  inherited;
  sortF.ShowSort(cliCustomers);
end;

procedure TChooseItemBudgectFormF.actSelectAllExecute(Sender: TObject);
begin
  inherited;
  with cliCustomers do begin
    DisableControls;
...
procedure TChooseItemBudgectFormF.actSlelectInverseExecute(Sender: TObject);
begin
  inherited;
  with cliCustomers do begin
    DisableControls;
...
procedure TChooseItemBudgectFormF.ReadSelect;
begin
  cliCustomers.Active:=False;
  cliCustomers.Active:=True ;
...
procedure TChooseItemBudgectFormF.SaveSelect;
begin
...
procedure TChooseItemBudgectFormF.FormDestroy(Sender: TObject);
begin
  inherited;
