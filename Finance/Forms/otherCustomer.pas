...
uses
  ...
  template2MDI, ActnList, StdCtrls, ExtCtrls,
  Buttons, DBCtrls, DBGrids, zAPIBalloon, DB, ppCtrls,
  ppBands, ppVar, ppReport,
  ppDBPipe, ADODB, ppParameter, ppPrnabl, ppClass, ppCache, ppProd, ppComm,
  ppRelatv, ppDB, Mask, Grids, ImgList, DBActns, ComCtrls;
...
type
  TotherCustomerF = class(Ttemplate2MDIF)
    ...
    Label10: TLabel;
    DBEdit16: TDBEdit;
    qryOtherFirstBalance: TBCDField;
...
begin
  inherited;
  qryOther.FieldByName('CustName').ReadOnly := opta.EditCustNameLevelID;
  ...
  qryOther.Active := True;
end;

procedure TotherCustomerF.FormResize(Sender: TObject);
begin
  inherited;
...
procedure TotherCustomerF.FormDestroy(Sender: TObject);
begin
  inherited;
...
begin
  ...
  if not UnicFieldName(DataSet.FieldByName('CustID').AsString,
    DataSet.FieldByName('CustName').AsString, 'Customers', 'CustID',
    'CustName',qryOtherCustomerGrpID.AsInteger) then
    Abort;
end;
...
begin
  ...
  if opta.RecoverPittedCode in [0] then
    with DMf.qryTmpTmp do
    begin
      Active := False;
      SQL.Text :=
  ...
  if opta.AccDetailCodeEqualCustID>0 then
    qryOther.FieldByName('acc_DetailCode').AsInteger :=
      DataSet.FieldByName('CustID').AsInteger;
  DBEdit1.SetFocus;
end;
...
begin
  inherited;
  if opta.AccDetailCodeEqualCustID>0 then
    qryOther.FieldByName('acc_DetailCode').AsInteger := Sender.AsInteger;
