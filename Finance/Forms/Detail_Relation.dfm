inherited Detail_RelationF: TDetail_RelationF
  Left = 318
  Top = 167
  Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1578#1601#1589#1610#1604#1610
  ClientHeight = 371
  ClientWidth = 524
  OnResize = FormResize
  ExplicitWidth = 540
  ExplicitHeight = 410
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 314
    Width = 516
    ExplicitTop = 326
    ExplicitWidth = 532
  end
  inherited Panel1: TPanel
    Width = 524
    ExplicitWidth = 532
    inherited Image1: TImage
      Left = 478
      Picture.Data = {
        
        1515}
      ExplicitLeft = 478
    end
    inherited lblTopic0: TLabel
      Left = 340
      Width = 118
      Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1578#1601#1589#1610#1604#1610
      ExplicitLeft = 340
      ExplicitWidth = 118
    end
    inherited lblTopic1: TLabel
      Left = 162
      Width = 283
      Caption = #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583'.'
      ExplicitLeft = 162
      ExplicitWidth = 283
    end
    object Image3: TImage
      Left = 128
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 104
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 334
    Width = 524
    ExplicitTop = 346
    ExplicitWidth = 532
    inherited btnClose: TBitBtn
      Caption = #1575#1606#1589#1585#1575#1601
      Glyph.Data = {00000000}
      ModalResult = 2
      TabOrder = 5
    end
    object BitBtn1: TBitBtn
      Left = 455
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      ModalResult = 1
      TabOrder = 0
    end
    object BitBtn3: TBitBtn
      Left = 85
      Top = 6
      Width = 75
      Height = 25
      Action = actSort
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 4
    end
    object BitBtn4: TBitBtn
      Left = 162
      Top = 6
      Width = 75
      Height = 25
      Action = actSearch
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 3
    end
    object BitBtn7: TBitBtn
      Left = 301
      Top = 6
      Width = 75
      Height = 25
      Action = actSelectnot
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      TabOrder = 2
    end
    object BitBtn6: TBitBtn
      Left = 378
      Top = 6
      Width = 75
      Height = 25
      Action = actSelectAll
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      TabOrder = 1
    end
  end
  object DBGrid1: TDBGrid [3]
    Tag = 111
    Left = 0
    Top = 73
    Width = 524
    Height = 261
    Align = alClient
    Color = clCream
    DataSource = src_Details
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnKeyPress = DBGrid1KeyPress
    Columns = <
      item
        Expanded = False
        FieldName = '_Checked'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CompanyCode'
        Width = 57
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DetailCode'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DetailName_L1'
        Width = 327
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DetailName_L2'
        Width = 4
        Visible = True
      end>
  end
  inherited ActionList1: TActionList
    object actSelectAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      OnExecute = actSelectAllExecute
    end
    object actSelectnot: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      OnExecute = actSelectnotExecute
    end
    object actSearch: TAction
      Caption = #1580#1587#1578#1580#1608
      OnExecute = actSearchExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
  end
  object qry_Details: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      'Select * from acc.Details'
      'where detailcode<>0'
      'order by detailcode')
    Left = 24
    Top = 96
  end
  object prv_Detail: TDataSetProvider
    DataSet = qry_Details
    Left = 23
    Top = 141
  end
  object cli_Details: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'prv_Detail'
    Left = 22
    Top = 189
    object cli_Details_Checked: TBooleanField
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      FieldKind = fkInternalCalc
      FieldName = '_Checked'
    end
    object cli_DetailsDetailCode: TIntegerField
      Tag = 3
      DisplayLabel = #1603#1583
      FieldName = 'DetailCode'
    end
    object cli_DetailsDetailName_L1: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576' '#1578#1601#1589#1610#1604#1610
      FieldName = 'DetailName_L1'
      Size = 150
    end
    object cli_DetailsDetailName_L2: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576' '#1578#1601#1589#1610#1604#1610' 2'
      FieldName = 'DetailName_L2'
      Size = 150
    end
    object cli_DetailsCompanyCode: TIntegerField
      DisplayLabel = #1603#1583' '#1588#1585#1603#1578
      FieldName = 'CompanyCode'
    end
  end
  object src_Details: TDataSource
    DataSet = cli_Details
    Left = 24
    Top = 237
  end
  object qry_DetailRange: TADOQuery
    Connection = DmF.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
      item
        Name = 'FilterCode'
        Attributes = [paSigned]
        DataType = ftLargeint
        Precision = 19
        Size = 10
        Value = Null
      end>
    SQL.Strings = (
      'SELECT  *  FROM  acc.DetailRange'
      'WHERE     (TopicCode = :FilterCode)')
    Left = 408
    Top = 144
    object qry_DetailRangeTopicCode: TLargeintField
      FieldName = 'TopicCode'
    end
    object qry_DetailRangeDetailCode: TIntegerField
      FieldName = 'DetailCode'
    end
    object qry_DetailRangeAuditReferenceNo: TIntegerField
      FieldName = 'AuditReferenceNo'
    end
    object qry_DetailRangeAuditReferenceTxt: TStringField
      FieldName = 'AuditReferenceTxt'
      Size = 30
    end
    object qry_DetailRangeCompanyCode: TIntegerField
      FieldName = 'CompanyCode'
    end
  end
end
