inherited DetailTopic_RelationF: TDetailTopic_RelationF
  Left = 286
  Top = 184
  Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1578#1601#1589#1610#1604#1610
  ClientHeight = 383
  ClientWidth = 549
  OldCreateOrder = True
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel1: TBevel
    Top = 326
    Width = 549
  end
  inherited Panel1: TPanel
    Width = 549
    inherited Image1: TImage
      Left = 495
      Picture.Data = {
        
        1515}
    end
    inherited lblTopic0: TLabel
      Left = 357
      Width = 118
      Caption = #1575#1606#1578#1582#1575#1576' '#1603#1583#1607#1575#1610' '#1578#1601#1589#1610#1604#1610
    end
    inherited lblTopic1: TLabel
      Left = 179
      Width = 283
      Caption = #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583'.'
    end
    object Image3: TImage
      Left = 128
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 104
      Top = 40
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 346
    Width = 549
    inherited btnClose: TBitBtn
      Left = 7
    end
    object BitBtn1: TBitBtn
      Left = 468
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      ModalResult = 1
      TabOrder = 1
    end
    object BitBtn2: TBitBtn
      Left = 391
      Top = 6
      Width = 75
      Height = 25
      Hint = ' '
      Caption = #1575#1606#1589#1600#1600#1600#1585#1575#1601
      ModalResult = 2
      TabOrder = 2
    end
    object Button1: TButton
      Left = 314
      Top = 6
      Width = 75
      Height = 25
      Action = actSlelectAll
      TabOrder = 3
    end
    object Button2: TButton
      Left = 237
      Top = 6
      Width = 75
      Height = 25
      Action = actSlelectInverse
      TabOrder = 4
    end
    object Button3: TButton
      Left = 83
      Top = 6
      Width = 75
      Height = 25
      Action = actSort
      TabOrder = 5
    end
    object Button4: TButton
      Left = 160
      Top = 6
      Width = 75
      Height = 25
      Action = actSearch
      TabOrder = 6
    end
  end
  object DBGrid1: TDBGrid [3]
    Left = 0
    Top = 73
    Width = 549
    Height = 273
    Align = alClient
    Color = clCream
    DataSource = src_AccCode
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnKeyPress = DBGrid1KeyPress
    Columns = <
      item
        Expanded = False
        FieldName = '_Checked'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'TopicCode'
        Width = 64
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MoeenName_L1'
        Width = 240
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MoeenName_L2'
        Width = 144
        Visible = True
      end>
  end
  object Button5: TButton [4]
    Left = 168
    Top = 184
    Width = 75
    Height = 25
    Caption = 'Button5'
    TabOrder = 3
    OnClick = Button5Click
  end
  inherited ActionList1: TActionList
    object actSlelectInverse: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      OnExecute = actSlelectInverseExecute
    end
    object actSlelectAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      OnExecute = actSlelectAllExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      OnExecute = actSortExecute
    end
    object actSearch: TAction
      Caption = #1580#1587#1578#1580#1608
      OnExecute = actSearchExecute
    end
  end
  object qry_AccCode: TADOQuery
    Connection = Dmf.adcAccounting
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      
        'SELECT     dbo.Categories.TopicCode, dbo.Categories.MoeenName_L1' +
        ' ,dbo.Categories.MoeenName_L2'
      'FROM         dbo.Categories LEFT OUTER JOIN'
      
        '                      dbo.CategoriesForUse ON dbo.Categories.Top' +
        'icCode = dbo.CategoriesForUse.PrvTopicCode'
      'WHERE     (dbo.CategoriesForUse.PrvTopicCode IS NULL)'
      'Order By   Ltrim(Str(dbo.Categories.TopicCode))'
      '')
    Left = 80
    Top = 88
  end
  object prv_AccCode: TDataSetProvider
    DataSet = qry_AccCode
    Options = [poAutoRefresh]
    Left = 55
    Top = 141
  end
  object cli_AccCode: TClientDataSet
    Aggregates = <>
    FieldDefs = <>
    IndexDefs = <>
    Params = <>
    ProviderName = 'prv_AccCode'
    StoreDefs = True
    Left = 56
    Top = 189
    object cli_AccCode_Checked: TBooleanField
      Tag = 3
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      FieldKind = fkInternalCalc
      FieldName = '_Checked'
    end
    object cli_AccCodeTopicCode: TLargeintField
      Tag = 3
      DisplayLabel = #1603#1583' '#1605#1593#1740#1606
      FieldName = 'TopicCode'
    end
    object cli_AccCode_HasTopicCode: TBooleanField
      FieldKind = fkLookup
      FieldName = '_HasTopicCode'
      LookupDataSet = qry_DetailRange
      LookupKeyFields = 'TopicCode'
      LookupResultField = 'TopicCode'
      KeyFields = 'TopicCode'
      Lookup = True
    end
    object cli_AccCodeMoeenName_L1: TStringField
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576
      FieldName = 'MoeenName_L1'
      Size = 100
    end
    object cli_AccCodeMoeenName_L2: TStringField
      DisplayLabel = #1593#1606#1608#1575#1606' '#1581#1587#1575#1576' 2'
      FieldName = 'MoeenName_L2'
      Size = 100
    end
  end
  object src_AccCode: TDataSource
    DataSet = cli_AccCode
    Left = 64
    Top = 237
  end
  object qry_DetailRange: TADOQuery
    Connection = Dmf.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <
      item
        Name = 'FilterCode'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 1
      end>
    SQL.Strings = (
      'SELECT  *  FROM  DetailRange'
      'WHERE     (DetailCode = :FilterCode)')
    Left = 408
    Top = 144
  end
end
