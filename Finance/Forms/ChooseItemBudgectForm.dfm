inherited ChooseItemBudgectFormF: TChooseItemBudgectFormF
  Left = 297
  Top = 142
  Align = alLeft
  Caption = #1575#1606#1578#1582#1575#1576' '
  ClientHeight = 512
  ClientWidth = 522
  OldCreateOrder = True
  OnDestroy = FormDestroy
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 522
    inherited Image1: TImage
      Left = 468
    end
    inherited lblTopic0: TLabel
      Left = 411
      Width = 37
      Caption = #1575#1606#1578#1582#1575#1576' '
    end
    inherited lblTopic1: TLabel
      Left = 144
      Top = 40
      Width = 355
      AutoSize = False
      Caption = #1576#1585#1575#1610' '#1575#1606#1578#1582#1575#1576' '#1575#1586' '#1583#1603#1605#1607' Space '#1610#1575' Double Click '#1605#1608#1587' '#1575#1587#1578#1601#1575#1583#1607' '#1603#1606#1610#1583'.'
    end
    object Image3: TImage
      Left = 208
      Top = 16
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FAFF}
      Visible = False
    end
    object Image2: TImage
      Left = 184
      Top = 16
      Width = 16
      Height = 16
      AutoSize = True
      Picture.Data = {
        
        FBFF}
      Visible = False
    end
  end
  inherited pnlDown: TPanel
    Top = 475
    Width = 522
    inherited btnClose: TBitBtn
      Caption = #1575#1606#1589#1585#1575#1601
      ModalResult = 2
      TabOrder = 5
      Glyph.Data = {00000000}
    end
    object BitBtn1: TBitBtn
      Left = 440
      Top = 6
      Width = 75
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #1578#1571#1610#1610#1600#1600#1600#1583
      Default = True
      ModalResult = 1
      TabOrder = 0
    end
    object BitBtn2: TBitBtn
      Left = 162
      Top = 6
      Width = 75
      Height = 25
      Action = actSearch
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 3
    end
    object BitBtn3: TBitBtn
      Left = 85
      Top = 6
      Width = 75
      Height = 25
      Action = actSort
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 4
    end
    object BitBtn6: TBitBtn
      Left = 363
      Top = 6
      Width = 75
      Height = 25
      Action = actSelectAll
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      TabOrder = 1
    end
    object BitBtn7: TBitBtn
      Left = 286
      Top = 6
      Width = 75
      Height = 25
      Action = actSlelectInverse
      Anchors = [akTop, akRight]
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      TabOrder = 2
    end
  end
  object DBGrid1: TDBGrid [2]
    Left = 0
    Top = 73
    Width = 522
    Height = 402
    Align = alClient
    Color = clCream
    DataSource = srcCustomers
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnKeyPress = DBGrid1KeyPress
    Columns = <
      item
        Expanded = False
        FieldName = '_checked'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'code'
        Title.Caption = #1603#1583
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'caption_L1'
        Width = 227
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Caption_L2'
        Width = 170
        Visible = True
      end>
  end
  inherited ActionList1: TActionList
    object actSlelectInverse: TAction [1]
      Caption = #1575#1606#1578#1582#1575#1576' '#1605#1593#1603#1608#1587
      Hint = ' '
      ShortCut = 16457
      OnExecute = actSlelectInverseExecute
    end
    object actSelectAll: TAction
      Caption = #1575#1606#1578#1582#1575#1576' '#1607#1605#1607
      Hint = ' '
      ShortCut = 16449
      OnExecute = actSelectAllExecute
    end
    object actSearch: TAction
      Caption = #1580#1587#1578#1580#1608
      Hint = ' '
      ShortCut = 114
      OnExecute = actSearchExecute
    end
    object actSort: TAction
      Caption = #1578#1585#1578#1610#1576
      Hint = ' '
      ShortCut = 117
      OnExecute = actSortExecute
    end
  end
  object qryCustomers: TADOQuery
    AutoCalcFields = False
    Connection = Dmf.adcAccounting
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    AfterOpen = qryCustomersAfterOpen
    Parameters = <>
    SQL.Strings = (
      'SELECT DGID As Code , DGName_L1  As Caption_l1 ,  '
      'DGName_L2   As Caption_l2'
      ' FROM DetailGroups'
      ''
      ' ORDER BY code ')
    Left = 144
    Top = 192
    object qryCustomersCode: TIntegerField
      FieldName = 'Code'
    end
    object qryCustomersCaption_l1: TStringField
      FieldName = 'Caption_L1'
      FixedChar = True
      Size = 10
    end
    object qryCustomersCaption_l2: TStringField
      FieldName = 'Caption_L2'
      FixedChar = True
      Size = 10
    end
  end
  object srcCustomers: TDataSource
    DataSet = cliCustomers
    Left = 320
    Top = 264
  end
  object DataSetProvider1: TDataSetProvider
    DataSet = qryCustomers
    Left = 232
    Top = 216
  end
  object cliCustomers: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DataSetProvider1'
    Left = 80
    Top = 216
    object cliCustomers_checked: TBooleanField
      DisplayLabel = #1575#1606#1578#1582#1575#1576
      FieldKind = fkInternalCalc
      FieldName = '_checked'
    end
    object cliCustomerscode: TSmallintField
      Tag = 3
      FieldName = 'code'
    end
    object cliCustomerscaption: TStringField
      Tag = 3
      DisplayLabel = #1593#1606#1608#1575#1606' '
      FieldName = 'caption_L1'
      FixedChar = True
      Size = 100
    end
    object cliCustomersCaption_L2: TStringField
      Tag = 3
      DisplayLabel = 'caption'
      FieldName = 'Caption_L2'
      Size = 100
    end
    object cliCustomers_has: TStringField
      FieldKind = fkLookup
      FieldName = '_has'
      LookupDataSet = CompaniesF.qryDetailGroup
      LookupKeyFields = 'DGID'
      LookupResultField = 'DGName_L1'
      KeyFields = 'code'
      Size = 0
      Lookup = True
    end
  end
  object PopMnuForm: TPopupMenu
    BiDiMode = bdRightToLeft
    ParentBiDiMode = False
    Left = 275
    Top = 128
    object MenuItem3: TMenuItem
      Action = actSelectAll
    end
    object MenuItem4: TMenuItem
      Action = actSlelectInverse
    end
    object N9: TMenuItem
      Caption = '-'
    end
    object N1: TMenuItem
      Action = actSearch
    end
    object N2: TMenuItem
      Action = actSort
    end
    object N5: TMenuItem
      Caption = '-'
    end
    object N4: TMenuItem
      Action = DataSetPost1
    end
    object N6: TMenuItem
      Action = DataSetCancel1
    end
  end
  object qryDGCompaniesRange: TADOQuery
    Connection = Dmf.adcAccounting
    Parameters = <
      item
        Name = 'FormType'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 125
      end>
    SQL.Strings = (
      'Select * from DGCompaniesRange'
      'WHERE     (CompanyCode =:FormType)')
    Left = 440
    Top = 192
    object qryDGCompaniesRangeDGIDCompanies: TIntegerField
      FieldName = 'DGIDCompanies'
    end
    object qryDGCompaniesRangeCompanyCode: TIntegerField
      FieldName = 'CompanyCode'
    end
  end
end
