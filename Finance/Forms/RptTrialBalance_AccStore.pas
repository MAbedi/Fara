...
uses
  ...
  DBCtrls, Menus, ppVar, ppParameter, ppDesignLayer, Filter_ADO_Const,
  System.ImageList, System.Actions, DBGridEhGrouping, ToolCtrlsEh,
  DBGridEhToolCtrls, DynVarsEh, EhLibVCL, GridsEh, DBAxisGridsEh, DBGridEh,
  CedarDbGrid;

type
  TRptTrialBalance_AccStoreF = class(Ttemplate2MDIF)
    qry_Init: TADOQuery;
    ...
    actSell: TAction;
    qryCross: TADOQuery;
    qryCrossCrossCode: TWordField;
    qryCrossCrossName: TStringField;
...
var
  RptTrialBalance_AccStoreF: TRptTrialBalance_AccStoreF;
  cloned: TADOQuery;
...
uses Dm, searchCode_ADO, filter_ADO, FilterClass_ADO, GlobalPro,
  ...
  RptTrialBalance_AccCode_Arzi, RptTrialBalance_CTopics3, VarianceBookBsell;

{$R *.dfm}
{ TRptTrialBalance_AccStoreF }

procedure TRptTrialBalance_AccStoreF.FormCreate(Sender: TObject);
begin
  inherited;
  OrgSQL := qry_TrialBalance.SQL.Text;
...
procedure TRptTrialBalance_AccStoreF.FormShow(Sender: TObject);
begin
  inherited;
  if ShowFilter then
  begin
...
procedure TRptTrialBalance_AccStoreF.actFilterExecute(Sender: TObject);
var
...
procedure TRptTrialBalance_AccStoreF.InitForm;
begin
  with qry_Init do
  Begin
    Active := False;
  ...
  Caption := ' مغایرت تراز تفصیلی با انبار  ' + qry_InitLevelCaption.AsString;
...
procedure TRptTrialBalance_AccStoreF.FormPaint(Sender: TObject);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.actPrintExecute(Sender: TObject);
begin
  inherited;
  if formtype = 1 then
  begin
...
procedure TRptTrialBalance_AccStoreF.actSendExeclExecute(Sender: TObject);
begin
  inherited;
  SendToExcel(DBGrid1);
end;

procedure TRptTrialBalance_AccStoreF.actSearch_Execute(Sender: TObject);
begin
  inherited;
  search2F.ShowSearch(qry_TrialBalance);
end;

procedure TRptTrialBalance_AccStoreF.actSellExecute(Sender: TObject);
begin
  inherited;
  CreateMDIForm3(True, TVarianceBookBsellF, VarianceBookBsellF, Self, 1);
  with VarianceBookBsellF.qry_Doc do
  begin
    Active := False;
    ...
    Parameters.ParamByName('RType').Value := 1;
  ...
  with VarianceBookBsellF.qry_AccCode do
  begin
    Active := False;
    Parameters.ParamByName('VarianceCodeFrom').Value :=
      qry_TrialBalance.FieldByName('VarianceCode').AsInteger;
...
procedure TRptTrialBalance_AccStoreF.actSortExecute(Sender: TObject);
begin
  inherited;
  sort2F.ShowSort(qry_TrialBalance);
end;

procedure TRptTrialBalance_AccStoreF.actStoreExecute(Sender: TObject);
begin
  inherited;
  CreateMDIForm3(True, TVarianceBookBsellF, VarianceBookBsellF, Self, 2);
  with VarianceBookBsellF.qry_Doc do
  begin
    Active := False;
    Parameters.ParamByName('FromYearId').Value := APPBank.Year;
    ...
    Parameters.ParamByName('FromYearId2').Value := APPBank.Year;
    Parameters.ParamByName('ToYearId2').Value := APPBank.Year;
    Parameters.ParamByName('RType').Value := 2;
    if myParams.FindParam('Year') <> nil then
    begin
      Parameters.ParamByName('FromYearID').Value := APPBank.Year;
      Parameters.ParamByName('ToYearID').Value := APPBank.Year;
...
procedure TRptTrialBalance_AccStoreF.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.UpdateFilter;
var
  s: string;
begin
  with qryCross do
  ...
  begin
    ...
    // SQL.Text := 'SELECT r.storeid ,s.c_StoreName, v.VarianceCode ';
    // SQL.Add(',t.TopicCode ,c.MoeenName_L1, max(t.Debt) Debt, max(t.Credit ) Credit');
    // SQL.Add(',cast(sum(case when (r.effecttype in(2,6)) then totalinputprice + totaloutputprice else 0 end) as money) as InPutPrice');
    // SQL.Add(',cast(sum(case when (r.effecttype in(4,7)) then totalinputprice + totaloutputprice else 0 end) as money) as OutPutPrice');
    // SQL.Add(',cast(sum(case when (r.effecttype in(2,6)) then totalinputprice + totaloutputprice else 0.0 end)-max( t.debt) as money) as');
    // SQL.Add('DifDebt');
    // SQL.Add(',cast(sum(case when (r.effecttype in(4,7)) then totalinputprice + totaloutputprice else 0.0 end)-max(t.credit) as money) as');
    // SQL.Add('DifCredit');

    while not qryCross.Eof do
    begin
      s := s + ',cast(SUM(CASE WHEN( R.ReciptType = ' +
        qryCrossCrossCode.AsString +
        ') THEN TotalInputPrice + TotalOutputPrice ELSE  0.0 END) as money) AS Cross'
        + qryCrossCrossCode.AsString.Trim;

      qryCross.Next;
    end;

    SQL.Text := ReplaceStr(OrgSQL, '%CrossTabForm%', s);

    // SQL.Add('FROM (select distinct topiccode,variancecode from acc.variancerelations) as v');
    // SQL.Add('LEFT join Acc.AccTrialBalance( :LenPrvAccCode,');
    // SQL.Add(':DocTypeCode1_Not,:DocTypeCode2_Not,:DocTypeCode3_Not, :DocTypeCodeFrom , :DocTypeCodeTo ,');
    // SQL.Add(' :CompanyCodeFrom , :CompanyCodeTo, :LenAccCode,:AccCodeFrom ,:AccCodeTo,');
    // SQL.Add(':SecondaryDocNoFrom , :SecondaryDocNoTo , :PrimaryDocNoFrom , :PrimaryDocNoTo ,');
    // SQL.Add(':DocDateFrom , :DocDateTo,');
    // SQL.Add(':Status1,:Status2,:FromYearId,:ToYearID , :CTopicCode3From , :CTopicCode3To ) T LEFT JOIN');
    // SQL.Add('Acc.Categories c ON t.TopicCode = c.TopicCode');
    // SQL.Add('on t.topiccode = v.topiccode');
    // SQL.Add('LEFT join');
    // SQL.Add('reciptitems_stock as r on v.variancecode = r.storeid');
    // SQL.Add('left join Stores s on s.n_StoreID = v.VarianceCode');
    //
    // SQL.Add('GROUP BY r.storeid ,t.TopicCode ,c.MoeenName_L1,s.c_StoreName, v.VarianceCode');
    // SQL.Add('order by v.VarianceCode,t.TopicCode');

    Parameters.ParamByName('DocTypeCodeFrom').Value := 0;

    Parameters.ParamByName('DocTypeCodeTo').Value := 9;

    Parameters.ParamByName('CompanyCodeFrom').Value := 0;

    Parameters.ParamByName('CompanyCodeTo').Value := 9;
...
procedure TRptTrialBalance_AccStoreF.actShowFormExecute(Sender: TObject);
begin
  inherited;
  // Call rpt_AccbookF
  ...
  CreateMDIForm3(False, TRpt_AccBookF, Rpt_AccBookF, RptTrialBalance_AccStoreF,
    formtype);
...
procedure TRptTrialBalance_AccStoreF.actAfterLevelExecute(Sender: TObject);
var
  // TopTopice:Currency;
...
procedure TRptTrialBalance_AccStoreF.actBeforeLevelExecute(Sender: TObject);
var
  LevelID: integer;
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.ppLabel4GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.ppLabel5GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TRptTrialBalance_AccStoreF.mnuDetailClick(Sender: TObject);
var
  TopTopice: integer;
...
procedure TRptTrialBalance_AccStoreF.mnuDetail_arziClick(Sender: TObject);
var
  TopTopice: integer;
  // PrvCodeLength,CodeLength:Integer;
  // s:string;
begin
  ...
  CreateMDIForm3(False, TRptTrialBalance_AccCode_ArziF,
    RptTrialBalance_AccCode_ArziF, RptTrialBalance_AccStoreF, formtype);
  // ------------------SET Parameters in  TRptTrialBalance_DetailsF  form
...
procedure TRptTrialBalance_AccStoreF.mnuCtopic1Click(Sender: TObject);
var
  TopTopice: integer;
  // CodeLength:Integer;
...
begin
  ...
  CreateMDIForm3(False, tRptTrialBalance_CTopicsF, RptTrialBalance_CTopicsF,
    MainF); // RptTrialBalance_AccStoreF
...
procedure TRptTrialBalance_AccStoreF.actUpdateExecute(Sender: TObject);
begin
  inherited;
  qry_TrialBalance.Requery();
end;

procedure TRptTrialBalance_AccStoreF.qry_TrialBalanceAfterOpen
  (DataSet: TDataSet);
var
  i: integer;
begin
  ...
  with qry_TrialBalance do
  begin
    FieldByName('storeid').DisplayLabel := 'کد انبار';
    FieldByName('c_StoreName').DisplayLabel := 'انبار';
    FieldByName('VarianceCode').DisplayLabel := 'کد مغایرت انبار';
    FieldByName('TopicCode').DisplayLabel := 'کدحساب';
    FieldByName('MoeenName_L1').DisplayLabel := 'عنوان معین';
    FieldByName('Debt').DisplayLabel := 'بدهکار';
    FieldByName('Credit').DisplayLabel := 'بستانکار';
    FieldByName('InPutPrice').DisplayLabel := 'جمع| ورودی';
    FieldByName('OutPutPrice').DisplayLabel := 'جمع| خروجی';
    FieldByName('DifDebt').DisplayLabel := 'تفاوت| بدهکار';
    FieldByName('DifCredit').DisplayLabel := 'تفاوت| بستانکار';

    FieldByName('vTopicCode').DisplayLabel := 'کدحساب مغایرت';
    FieldByName('CTopicCode').DisplayLabel := 'تفصیلی 2';
    FieldByName('vCTopicCode').DisplayLabel := 'تفصیلی 2 مغایرت';

    FieldByName('VarianceName').DisplayLabel := 'عنوان  مغایرت';

    qryCross.First;
    while not qryCross.Eof do
    begin
      FieldByName('Cross' + qryCrossCrossCode.AsString.Trim).DisplayLabel :=
        'ریز|' + qryCrossCrossName.AsString;
      FieldByName('Cross' + qryCrossCrossCode.AsString.Trim).Tag := 3;
      qryCross.Next;
    end;
...
procedure TRptTrialBalance_AccStoreF.ppLBCompanyNameGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := APPBank.CompanyName;
...
procedure TRptTrialBalance_AccStoreF.ppDocDateGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  Text := GetPrintDate;
...
procedure TRptTrialBalance_AccStoreF.ppSystemVariable2GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.ppLabel13GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.ppLabel12GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.MenuItem2Click(Sender: TObject);
begin
  inherited;
  try
    qry_TrialBalance.DisableControls;
    InitReportFile(ppReport1, 'RepRptTrialBalance_AccStore', DBGrid1,
      ppDBPipeline1);
  finally
...
procedure TRptTrialBalance_AccStoreF.MenuItem1Click(Sender: TObject);
begin
  inherited;
  try
    qry_TrialBalance.DisableControls;
    InitReportFile(ppReport1, 'RepRptTrialBalance_AccStore_Topic', DBGrid1,
      ppDBPipeline1);
  finally
    qry_TrialBalance.EnableControls;
  end;
...
procedure TRptTrialBalance_AccStoreF.ppLabel3GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  case formtype of
...
procedure TRptTrialBalance_AccStoreF.MenuItem4Click(Sender: TObject);
begin
  inherited;
  try
    qry_TrialBalance.DisableControls;
    InitReportFile(ppReport1, 'RepRptTrialBalance_AccStore_L', DBGrid1,
      ppDBPipeline1);
  finally
    qry_TrialBalance.EnableControls;
  end;
...
procedure TRptTrialBalance_AccStoreF.MenuItem3Click(Sender: TObject);
begin
  inherited;
  try
    qry_TrialBalance.DisableControls;
    InitReportFile(ppReport1, 'RepRptTrialBalance_AccStore_Topic_L', DBGrid1,
      ppDBPipeline1);
  finally
    qry_TrialBalance.EnableControls;
  end;
...
procedure TRptTrialBalance_AccStoreF.mnuCtopic2Click(Sender: TObject);
var
  TopTopice: integer;
  // CodeLength:Integer;
begin
...
procedure TRptTrialBalance_AccStoreF.mnuCtopic3Click(Sender: TObject);
var
  TopTopice: integer;
  // CodeLength:Integer;
begin
...
procedure TRptTrialBalance_AccStoreF.ShowBeforeAccCode(LevelID: integer);
var
  AccCode: integer;
begin
  CreateMDIForm3(False, TRptTrialBalance_AccStoreF, RptTrialBalance_AccStoreF,
    Self, LevelID);
  ...
  with RptTrialBalance_AccStoreF do
  Begin
    with qry_TrialBalance do
    begin
      Active := False;
    ...
    // end;
  end; // with_RptTrialBalance_AccStoreF
end;

procedure TRptTrialBalance_AccStoreF.ShowAfterAccCode(prvLevelID, CodeLength,
...
begin
  ...
  end; // with_RptTrialBalance_AccStore2F

end;

procedure TRptTrialBalance_AccStoreF.N1Click(Sender: TObject);
begin
  inherited;
  DBGrid2PrintF.showGrid2Print(DBGrid1, 0, LblShowLimitPlace1.Caption);
end;

procedure TRptTrialBalance_AccStoreF.N2Click(Sender: TObject);
begin
  inherited;
  TreeChartF.showChart(qry_TrialBalance, Self);
end;

procedure TRptTrialBalance_AccStoreF.N3Click(Sender: TObject);
begin
  inherited;
  UpdateAllQry(Self);
end;

procedure TRptTrialBalance_AccStoreF.actOtherExecute(Sender: TObject);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.N4Click(Sender: TObject);
begin
  inherited;
...
procedure TRptTrialBalance_AccStoreF.N5Click(Sender: TObject);
var
  txt: String;
